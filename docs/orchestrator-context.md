...
**Last updated:** 2026-09-24 (22-B merged + closed at v40; the section-of-record rewritten, the 09-15 text moved verbatim to the archive §"Appended 2026-09-24"). Prior: 2026-09-15 (D32+D50 F4 disposition complete, D57 closed, D56 merged + closed). Prior: 2026-09-10 (22-A4 merged + live-migrated to schema v38; worktrees torn down; the section-of-record rewritten and this file trimmed at its size trigger). **This line is a POINTER, not a ledger** — it regressed to a 5,298-char wall of superseded per-session history dated 2026-05-24 and was compacted here, the same pathology and the same fix as the CLAUDE.md line-3 restructure. Current state lives in §"Currently in-flight work" below; the displaced history is verbatim in [`docs/orchestrator-context-archive.md`](orchestrator-context-archive.md) §"Appended 2026-09-10".
...
### Currently in-flight: PHASE 22 ACTIVE — **22-B MERGED + CLOSED at v40; the D67 + D69 riders DISPATCHED** (2026-09-24)

**Re-derive every fact here before relying on it; this section decays by default.** The newest `docs/orchestrator-handoff-*.md` and the arc ledgers are the detailed record; this is the pointer.

**`main`** is well ahead of `origin/main` and NOT pushed — the push is the operator's. **Schema v40 LIVE** (`schema_version` 40, read `mode=ro` 2026-09-24). Phase 22 migrations since v38: **0039** (22-A2 proof machinery — the tier-2 admission class, fail-closed read-time replay, the four cohort readers' named exclusion; merge `57187164`) and **0040** (22-B — the third `entry_intent` value `unintended_execution` with its evidence-bearing assignment surface; merge `297a9566`). Also merged since 09-15: D51 (the schema-manifest comparator, `91ef1684`) + D51b (`6050317d`), and the `cell_depth` build/model rider (`3b98ca55`). Last pre-v40 pipeline run **183** (`complete`, export ok); **the 17:30 HST run of 09-24 is the first on v40 — compare its per-class warning Counter against run 183** (the run-171/172 method from 22-A4).

**22-B** is CLOSED on the witness by CHARC (`341aa7ca`); RD's witness step 5 PASSED and is transcribed as the exec ledger's closing section (`docs/superpowers/plans/2026-09-23-phase22-arc-b-unintended-execution.exec-ledger.md`). Trade 20 (AMN) carries `unintended_execution` with attestation 1. Declared limitations AL-6 and AL-7. The 22-B standing grant is SPENT.

**In flight (2026-09-24):** the **D67 + D69 riders** — brief [`docs/d67-d69-riders-brief.md`](d67-d69-riders-brief.md) @ `321689e2`, one `implementer-sonnet-high` cell in `.worktrees/d67-d69-riders`, Reviewer A `strong`, Reviewer B (orchestrator) at QA. D67 = the web conftest wrapper's hand-copied signature (reproduced isolated: 3 failed / 12 passed); D69 = `backfill-intent`'s raw-ticker echo + uncontained post-commit output/close. CHARC rules any shape fork.

**Done 2026-09-24 (orchestrator queue):** the stale CLAUDE.md "`swing db-migrate` writes TWO backups" gotcha retired (`53143892`, verbatim to the archive); the `_is_head` rename found ALREADY landed (`e8a4d54e`); `%TEMP%/pytest-of-rwsmy` (6.3 GB) deleted. **Held for the operator:** `cell_depth --live` "showing more than the newest cell" — the code already lists every cell in the window; the intended change is unconfirmed.

**QUEUE, owners named.** **CHARC:** D68 (the exit route's any-exception rebuild), D70 (AL-6/AL-7 reopen preconditions), D71 (the trend SERIES legend overlap), the entry command's c2 masking and B-04's runner-wide durability verdict (both D39 sweep), the mistaken-attestation reversal surface; then the standing list (B-2, `ReservedJournalFieldError`'s base, D42, D46, D45, D44/D47, D55, D58, the D56 residual). CHARC's ratified order: 22-C/22-D when a live case exists; sweeps + harness compaction at phase close. **RD/operator:** the post-merge display gap (no stored-trade page renders `entry_intent`); RD's October monthly read on v40. **The next arc is the operator's to commission.**

**Trading reality:** the admitting gate stays open — ZERO `aplus` in recent evaluation runs. Real money; re-read open trades from the DB rather than carrying a list.
...
1. **RUN `python scripts/cell_depth.py --live <h>` BEFORE EVERY DISPATCH (the census) AND `python scripts/cell_depth.py --agent <id>` AT EVERY ROUND GATE (the named cell, read at any age — `--live` selects by transcript mtime, so a cell PARKED at a gate drops out of the window while a newer cell's row stands alone; mechanism inferred from the code, not an incident; CHARC ruling (a), 2026-09-24).** It is a precondition, not a caveat. **A cell cannot see its own depth** — no hook fires for a subagent — so a cell's self-estimate is not a record and the ledger's depth column is mine to fill. Past the 400K cap the loop re-dispatches to a FRESH cell off the committed plan plus the per-round preserved evidence; that is a normal outcome, not a failure. *(First real use, 2026-09-07: the 22-A4 planner measured 779,749 — nearly 2× the cap — and the gauge flagged it OVER before I dispatched. Every round in that cell had been paid at ~1.9× a round at 400K, and neither the cell nor I could see it.)*
