...
### Currently in-flight: PHASE 22 ACTIVE — **D56 MERGED + CLOSED; NO ACTIVE ARC, NO CELL IN FLIGHT** (2026-09-15)

**Nothing is dispatched and no cell is running; no worktree exists but `main`.** The next arc is the operator's to commission. Every fact in this section was re-derived from disk on 2026-09-15; re-derive rather than carry them forward.

**`main` @ `dde335c2`, PUSHED (`origin/main` equal, 0 ahead / 0 behind).** Schema **v38** live (`schema_version` = 38; `EXPECTED_SCHEMA_VERSION` = 38 at `swing/data/db.py:102`) — Phase 22 has added no migration since 0038. Tree clean except the untracked coa-chess finding `docs/cli-autoupdate-run-poisoning-finding-coa.md` (not ours). Last pipeline run **176**, `state=complete`, **`export_status=ok`** — note D42's export failure did NOT recur on 176 (175 failed); do not carry "failed on every run since 160" forward without re-reading. Latest evaluation run 162: 47 `skip`, 6 `watch`, 5 `excluded`, **ZERO `aplus`** — the admitting gate stays open. Open trades 23 CADL (partial_exited), 24 RHI, 25 OII, 26 NRIX, 28 PBF. Real money.

**Closed this sitting (2026-09-15), all merged, all accepted by CHARC:**
- **D32+D50** backup gates (`a3b2f90c`) — D50 CLOSED; **D32's disposition F4 is now COMPLETE**: step (a) moved 31 root pre-images to `~/swing-data/backups/pre-images/`, step (b) deleted the **21 twin CLI copies one pair at a time** on a delete-time re-hash of BOTH members plus a sidecar check printed into the ledger beside each delete (rows 46-50 by the operator's hand; 52-68 delegated to the orchestrator mid-sitting), **4,617,224,192 bytes freed, 21 of 21 proven, zero stops**. The 3 withheld true twins and everything else are KEPT. D32 itself still closes on 22-B's first real migration (the production proof). Ledger: [`docs/phase22-arc-d32-d50-ledger.md`](phase22-arc-d32-d50-ledger.md).
- **D57** wake-on-mail (`c8279a93`) — CLOSED at `c1ef3a56` on a witness this seat WAS: CHARC's ping woke a `swing-`prefixed orchestrator seat as a new turn in ~2 s with zero Monitor calls, and the mail post alone produced no record. **The ping convention is live: after EVERY `role_mail post`, run the `ping ->` line(s) it prints.**
- **D56** the error-region arc (`d12aa7b6`, `--no-ff`) — the pattern-review form and the exemplars page's four inline forms now route every error response to a sibling / page-level region instead of replacing the controls the message names, and ONE trough-1 helper feeds both the DBW corrected-start pre-fill (C3) and the refusal text that names it. Merged-head suite **12,469 passed / 13 skipped / 0 failed**, ruff clean. **Live witness 3 of 3 with a MEASURED zero-write proof** (`pattern_exemplars` 35/35 before and after every step, plain `sqlite3` `mode=ro`). Ledger: [`docs/phase22-arc-d56-ledger.md`](phase22-arc-d56-ledger.md).

**What D56 is worth remembering for.** Reviewer B passed the CODE and failed a TEST: the three-row-kind pin CHARC ruled had been written over the two row kinds where the old and new values COINCIDE, so reverting the fix would have left it green — a pin that reads as coverage and discriminates nothing. Separately, Reviewer A's round 1 was disqualified because its own prompt wrote the verdict tokens line-initially and the anchored grep matched the prompt's echo (the recipe's documented hazard, met in the wild); its findings were still acted on, per "a failing round may inform you but may not END the loop." And the register row that commissioned the arc cited the WRONG route's line numbers — CHARC's own, corrected at `ac3f6a80` when the round-0 census read the code instead of the citation.

**QUEUE, owners named.** **Orchestrator's:** the `_is_head` version-mirror rename rider (trigger fired, still unlanded — its own commit, never beside a version bump); `scripts/cell_depth.py --sessions` printing each session's Claude Code build; the non-ASCII em-dash in `swing/cli.py`'s pre-18 WARN line. **CHARC's:** ~~D53.1's close check at run 177~~ **DONE — D53.1 CLOSED**: run 177 completed 2026-09-15T17:43:28 (`state=complete`, `export_status=ok`) and its two non-zero DBW rows carry `window_start_date == evidence.trough_1_date` (7860 LILAK 2026-08-20; 7865 NEOG 2026-08-06 — orchestrator re-read `mode=ro`, both MATCH); CHARC closed it at his rollover. Remaining: the **Gotchas cap triage** (§Gotchas at the ~55K soft cap, so the next gotcha commit carries a compression pass) — and inside it, **the `swing db-migrate` "TWO backups" gotcha is now FALSE** (D32 made it one, echoed); D55, D58, the newly banked **D56 residual** (the exemplars route's 500 path at `routes/patterns.py:264,273` has NO test — found by grepping for the raise's message text while correcting a docstring that claimed it was covered); then B-2, `ReservedJournalFieldError`'s bare-`Exception` base, D42, D46, D45, D44/D47, the D39 sweep.

**Seat state:** **CHARC ROLLED 2026-09-16 at 432,788** (state committed `372aa758`, `docs/charc-state.md` overwritten; that seat has STOPPED draining). His successor launches as `swing-charc-<stamp>`; `comms/.sessions.json` still names the retired seat until the launcher rewrites it, so trust the `ping ->` line `role_mail post` prints, never a remembered name. The successor owns: the D32 production proof at the 22-B witness, D51 before 22-B, the D58 shape ruling, and the harness compaction at Phase-22 close. RD has NOT rolled and is still on the interim Monitor rule. This orchestrator seat was at ~350K.
...
|---|---|---|---|
| A | 2026-04-27 | (2026-04-27) Subject-only grep regex amended to ERE + POSIX digit class | §"Binding conventions" of this file (the ERE + POSIX form is stated there operatively) |
...
| B | 2026-04-25 | (2026-04-25) Next-move post-Tranche-C: parallel operational + applied-research parity check. | completed ACTION — the parity check it queued was run and closed; [`research/studies/harness-vs-production-parity.md`](../research/studies/harness-vs-production-parity.md) |

**DELIBERATELY KEPT, having FAILED the owner test** (their content has no live home, so archiving them would lose it): *10% return target math* (2026-04-25) · *Bug 7 family confirmed closed in web layer* (2026-04-25) · *`/prices/refresh` anchor consistency closed Bug-7 family in this layer* (2026-04-26). **Also kept, for a different reason —** *Filter-rule activation-rate sanity check* binds FUTURE studies, so it is not closed at all and fails archivability on forward force rather than ownership.
...
- **The orchestrator role STARTS at Opus 5 / `high`** (operator-ruled 2026-08-03, recalibrated for the Fable 5 / Opus 5 / Sonnet 5 generation; the Opus-4.x-era `xhigh` default is **RETIRED** — generation uplift covers it). **TWO named `xhigh` escalations: (1) the merge-integration / composition step** — the ONLY place the cross-arc composition class is caught (`harness-architecture.md` §5.1) — **and (2) phase-close QA.** Directors START at **Fable 5.1 / medium** (operator-ruled 2026-09-13, was `high` from 2026-09-02; the launcher's `$RoleLaunch` table agrees; the START configuration for all three roles is now declared at the top of each `scripts/*_bootstrap.md`). Full allocation + grounding: [`docs/implementer-dispatch-recipe.md`](implementer-dispatch-recipe.md) §2A and the brief [`docs/harness-model-effort-recalibration-brief.md`](harness-model-effort-recalibration-brief.md).
