...
**Last updated:** 2026-04-26 (chart-pattern flag-v1 Phase 2 complete; Phase 3 dispatch queued with disjoint-task-partitioning discipline)
...
**Chart-pattern flag-v1 Phase 2 (persistence layer) complete (2026-04-26).** Phase 1 (algorithm core, 26-commit chain `dffb723..574869f`, 1002→1029 fast tests, +27 unit tests) and Phase 2 (persistence layer, 16-commit chain `8eca791..158efd2` + `3d6e03d` retro, 1029→1052 fast tests, +23 tests, schema 8→10 with migrations 0009+0010 + `pattern_classifications` repo + Trade dataclass extension + 4-case repo-layer cross-column invariant) both shipped. Spec at `docs/superpowers/specs/2026-04-26-chart-pattern-flag-v1-design.md`; plan at `docs/superpowers/plans/2026-04-26-chart-pattern-flag-v1-plan.md`. Carve-out clean — no consumer yet (cache table empty until Phase 3; trade columns NULL until Phase 5). **Next dispatch queued: `copowers:executing-plans` for Phase 3 (pipeline integration — `_step_charts` calls `classify_flag` + persists row in same fenced_write; classifier-exception handling; `render_chart` `pattern_overlay` kwarg as no-op).** Operator decision (2026-04-26): try disjoint-task-partitioning brief discipline FIRST as a cheaper mitigation against subagent collision; fall back to worktree isolation if collision recurs.
...
- **Brief artifacts must be committed immediately; untracked briefs are vulnerable to subagent activity.** Lesson from chart-pattern flag-v1 Phase 2 dispatch (2026-04-26). Orchestrator drafted the Phase 2 dispatch brief and left it untracked, expecting the dispatch chain to commit it (as happened with Phase 1's `9f0a778`). Some operation during Phase 2 execution (likely a subagent's `git clean` or working-tree cleanup) deleted the untracked file; the brief is lost (not in working tree, not in git history). **Pattern:** orchestrator MUST commit briefs in the orchestrator-thread immediately after writing them, not rely on the dispatch chain to commit them. Untracked files in the working tree are not safe across a dispatch cycle, especially when subagents may be doing housekeeping.
