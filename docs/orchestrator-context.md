...
### Currently in-flight: PHASE 18 ACTIVE (data-collection integrity) — 18-A/B/C CLOSED; sub-agent dispatch model ADOPTED (2026-06-14)

Phase 18 is half-built. **18-A/18-B/18-C CLOSED** (schema v29→**v30** at 18-C; ~8258 fast green; all three-eye-gate-passed): 18-A temporal-log NaN writer fix + the shared `swing/data/ohlcv_finiteness.py` predicate (`c45d8752`); 18-B OHLC write-path consolidation killing the #24-#26 two-path-divergence class (`5cce72db`); 18-C yfinance call audit + migration 0030, operator-witnessed live-migration verified (`2b381461`). **The orchestrator-spawned sub-agent dispatch model was ADOPTED this generation** — dispatch is now via the `.claude/agents/implementer-<model>-<effort>` cell library + the SPOF [`docs/implementer-dispatch-recipe.md`](implementer-dispatch-recipe.md); full procedure in §"Dispatch execution — orchestrator-spawned implementer sub-agents" above (under Operating processes). **QUEUED (operator sequences; no commission until "go"):** 18-D/18-E (monitors consuming `yfinance_calls`), 18-F (GUI stoplights), 18-G (brief sweep), R1 (`requests` dep, own dispatch), 18-H (catch-all) — CHARC owes the arch passes for D/E/F. Generational handoff: [`docs/orchestrator-handoff-2026-06-14-phase18-mid.md`](orchestrator-handoff-2026-06-14-phase18-mid.md). Trailing (organic): TROX age-off, dividend marker, RD QAs.
