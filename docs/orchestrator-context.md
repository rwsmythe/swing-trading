...
**QUEUE, owners named.** **Orchestrator's:** the `_is_head` version-mirror rename rider (trigger fired, still unlanded — its own commit, never beside a version bump); `scripts/cell_depth.py --sessions` printing each session's Claude Code build; the non-ASCII em-dash in `swing/cli.py`'s pre-18 WARN line. **CHARC's:** **D53.1's close check at run 177** (non-zero DBW rows must carry `window_start_date == evidence.trough_1_date`) — run 177 is the 17:30 HST scheduled task and had NOT run at write time; the **Gotchas cap triage** (§Gotchas at the ~55K soft cap, so the next gotcha commit carries a compression pass) — and inside it, **the `swing db-migrate` "TWO backups" gotcha is now FALSE** (D32 made it one, echoed); D55, D58, the newly banked **D56 residual** (the exemplars route's 500 path at `routes/patterns.py:264,273` has NO test — found by grepping for the raise's message text while correcting a docstring that claimed it was covered); then B-2, `ReservedJournalFieldError`'s bare-`Exception` base, D42, D46, D45, D44/D47, the D39 sweep.

**Seat state at write time:** CHARC's seat is PAST the 400K rollover trigger and rolls on the operator's word — his successor will be `swing-charc-<stamp>` in `comms/.sessions.json`, and the printed `ping ->` line is the only name to trust. This orchestrator seat was at ~350K.
