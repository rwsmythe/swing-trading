...
### Currently in-flight: PHASE 20 — all production arcs (20-A/B/C) SHIPPED; riders R1/R3 + close remain (2026-07-15)

**PHASE 20 (Reconciliation Integrity & Correction Paths) SCOPED + operator-approved** (`docs/phase20-scope-charc.md`; the trading-ledger mirror of Phase-19's research-chain hardening). Origin: the 2026-07-10 forensic reconciliation pinned **D25** — the tier-1 auto-corrector corrupted 3 fill rows (wrong-leg matching on same-qty pairs; AMN H1 outcome −$9.60 vs true +$1.17; a phantom SATL trade); the equity_delta instrument caught it in 24h and both directors nearly dismissed it. **Arcs:** **20-A** (D25: fix the corrector THEN the data — Half A merges before/atomic-with Half B, RD-BINDING order; the URGENT headline, deadline pre-Monday-open + before RD's Aug read; brief `docs/reconciliation-corrector-fix-20a-commissioning-brief.md`; cells writing-plans opus-xhigh / executing opus-high; `swing/trades` carve-out + a schema-STOP on the SATL void) · **20-B** (D22 gate the ungated `discrepancy resolve`, after 20-A) · **20-C** (D23/D24 coherence-UX, parallelizable/file-disjoint) · riders R1 stop-hook import / R2 AGENTS.md thin-pointer (operator-resolved) / R3 dead-gen tidy. **20-A (D25) CLOSED 2026-07-11** — Half-A corrector guards (A1 same-qty multi-candidate → tier-2; A2 side/date/2.0%-band plausibility; A3 dedicated `ReCorrectionContradictionError` re-correction alarm; A4 fills↔trades watchdog) + Half-B data, merged to main **`d5999345`** (schema v31, no migration; 18 exec commits; review-strong 12 rounds + codex-auto-review converged; merged-head **9092/5/0**). Writing-plans caught a scope-premise correction ratified into A5: the root-cause matcher lives in `swing/trades/schwab_reconciliation.py` (ticker+qty-only, break-at-first, no side/date) — NOT the classifier the brief named. Live-DB Half-B ran **operator/RD-witnessed** via a dry-run-first driver: **B1** the 3 audited tier-3 overrides (fills 17/28/37 → 13.00/24.53/35.65 via `override-correction` #28/#30/#34), **B2** the SATL trade-11 void (no-schema `trade_events` note; tuition 16→15, D19 audit-visible), **#33 stamp WITHDRAWN by RD** (the cross-discrepancy flag caught his premise-miss — the per-discrepancy chains are each valid and the two per-fill heads agree on 35.65; `_effective_fill_correction_head` resolves latest-applied-wins by design). **B3 RD-witnessed PASS**: AMN −$9.60 → **+$1.17** exact; H1 epoch re-derivation ledger↔broker **to the cent** (1555.60 + $0.20 penny-drift = 1555.80); **operator badge-death witnessed** — a fresh recon (run 78, `swing schwab fetch --all`) re-evaluated equity_delta under tolerance → cash-review badge cleared (re-eval, NOT the D24 acknowledge), and **proved the guards in anger**: zero re-corruption on the exact #34 causal geometry, fills intact. Worktree + branch cleaned. **Banked post-20-A ride-alongs (CHARC-registered):** the A-4 dedicated `discrepancy_type` (migration 0032 + the #11 one-commit discipline); two V2 candidates — execution↔fill identity linking (the same-side/same-qty swap blind spot), and the strict-ALL stats-only aggregation choke point (RD's D-B2 bound-b). **20-B (D22) SHIPPED 2026-07-15** — the resolve-gate on `discrepancy_resolve_cmd` (cli.py): a subject-rows-exist `pending_ambiguity_resolution` row REFUSES (naming resolve-ambiguity + `--force`); FK-orphans pass ungated via the REUSED 19-F `orphaned_affected_target` (no forked predicate); `--force` records the conscious bypass on the audit surface AND still enforces the reason-required contract; non-pending byte-identical. 4-round convergence (codex-auto-review surfaced all 4 majors incl. the `--force`-defeats-reason hole); CLI-only, no carve-out, no schema. **20-C (D23/D24) SHIPPED 2026-07-15** — web-only coherence-UX (the principle: coherence findings route to the DATA FIX, not the acknowledge): the untracked-position page gains the out-of-framework guidance block (V1 = guidance, NOT a config write-path — a web `user-config.toml` write is a V2 candidate); the equity_delta badge now links in BOTH lit states to a NEW read-only NETTED-basis diagnostic view (ledger vs NLV vs declared-OOF vs swing-NLV vs netted delta; the raw `equity_delta_dollars` labeled OOF-inclusive per RD's stored-vs-emit note); NO new base-VM field (standalone VM); malformed-JSON-500 + badge-TOCTOU fixed. **Both operator+CHARC-witnessed** via a reversible `[20BC-SEED]` helper (seeded the lit-badge/pending/untracked states; teardown exact — max discrepancy_id back to 86), then merged (20-B `f7fb1aa2`, merged-head 9102/5/0; 20-C `ec348221`, final merged-head **9114/5/0**); worktrees cleaned. **PHASE-20 PRODUCTION ARC LIST COMPLETE (20-A/B/C).** REMAINING: riders R1 (`comms_unread_hook` bare-import hardening) + R3 (dead-gen registry tidy, incl. the two accidental 15:43Z spin-ups `849fa6a6`/`bcc74462`); R2 already operator-resolved. Then the close ritual + the CHARC close audit. **Banked ride-alongs (CHARC-registered):** the A-4 dedicated `discrepancy_type` (migration 0032 + #11); two V2 candidates — execution↔fill identity linking; the strict-ALL stats-only choke point. Main HEAD `ec348221`; schema v31; suite 9114/5/0.

### (superseded) Currently in-flight: PHASE 19 CLOSED — all six arcs + the close housekeeping done; awaiting the next commission (2026-07-06)

**PHASE 19 (Launch-Context Robustness & Measurement Parity) CLOSED 2026-07-06** at schema **v31** (zero migrations; ~9033 fast green). All six arcs shipped + gated: **19-A** coverage_gaps CALIBRATION-C trailing-delisting fix (`fadc193d`; + D21/R3/R4) · **19-B** the **#5 drumbeat-false-RED ROOT FIX — the multi-week saga ENDED** (`3234f19a`) + the comms-belt narrow-fix (`44b3bbd9`) · **19-C** weeknight Windows Scheduled Task (`e3f1a442`; first real fire = run 125, 2026-07-06, `trigger='scheduled'`, exit 0) · **19-D** shadow-engine hardening (research/harness-only ADR-floor 0.15 + epsilon OHLC clamp 1.0; RD's pre-committed **T4 confirmatory re-read FINAL**; VSTS +27R killed, `invalid_ohlc` 38→22) · **19-E** Day-3-5 partial-trim advisory (add-alongside; NYSE-sessions day-counting; AMN witnessed the ruling live at session-day 3) · **19-F** orphan-tolerant discrepancy resolution (disc 73 healed via a new GUI acknowledge path). **Close housekeeping DONE:** H1 `comms_stop_hook` fail-open hardening + H2 `harness_probe` research-size ceilings (`722358a5`) · H3 the runbook capture-timing softening (`4cb584d9`) · H4 drift disposition (`AGENTS.md` tracked, `.codex/` ignored, `diag_5` retired) · the §4.3 archival sweep (14 dead artifacts → `docs/archive/phase19/`; top-level `docs/*.md` 39→28) · the CLAUDE.md line-3 re-compaction (streak 3908; +`monitoring/` in §Architecture). **Two items carried:** (1) the shadow-expectancy track-vs-ignore convention — an H4 full-ignore was REVERTED (`88bfdb6d`; it broke the study reproducibility test `test_gitignore_allowlists_artifact_dir`) and routed to **CHARC+RD**; (2) **D22** the ungated `discrepancy resolve` pending-state gate — DEFERRED to **Phase-20** (production code). **Full per-arc record:** [`docs/phase19-scope-charc.md`](phase19-scope-charc.md); the dead dispatch artifacts (briefs/plans/the 2026-07-04 parallel-batch handoff) are archived at [`docs/archive/phase19/`](archive/phase19). **No active phase — awaiting the operator's next commission** + the CHARC close audit on the post-housekeeping HEAD. Schema v31.
...
The information-vs-authority line: the mailbox carries *information* (status, queries, return reports) between roles. **Dispatch-direction** (commissioning briefs, implementer dispatch prompts, approvals) carries *authority*, which is the operator's alone. **Amended 2026-06-26 (operator):** a director MAY post a dispatch directly to the orchestrator's per-generation inbox (`comms/orchestrator/<sid>/inbox`, added G6 Arc A) **once the operator has pre-authorized the action** — a director's action-bearing inbox message carries the operator's IMPLIED approval (authority granted before the director sends; the director is the courier, not the source of authority; the operator reviews via the comms GUI). Treat such a dispatch as you would an operator-hand-carried prompt. `decision_request` stays the operator's type only; `role_mail.py` refuses to route it to a non-operator (the L1 lock). Bootstrap a fresh orchestrator generation with `scripts/orchestrator_bootstrap.md`.
