...
### Currently in-flight: PHASE 18 ACTIVE (data-collection integrity) — 18-A/B/C/E/F CLOSED; 18-D + trailing remain (2026-06-15)

Phase 18 is mostly built. **18-A/18-B/18-C/18-E/18-F CLOSED** (schema **v30** at 18-C, unchanged since; ~8380 fast green; all gated): 18-A finiteness predicate (`c45d8752`); 18-B OHLC write-path consolidation killing the #24-#26 divergence class (`5cce72db`); 18-C yfinance audit + migration 0030, operator-witnessed (`2b381461`); 18-E operational tool-health monitor — `swing/monitoring/` + `scripts/tool_health.py` + the §3 status envelope (`65de5a7f`; the operator CLI witness caught a false-RED weather-ticker bug the byte-tests + 4 Codex rounds missed → CLAUDE.md gotcha); 18-F GUI health stoplights — two topbar stoplights (tool-health live, research GREY-until-18-D) via a Starlette context-processor base-injection + two read-only drill-downs (`f075e7b5`; review-strong converged R1-R5, 4 research false-green vectors closed; the FIRST codex-auto-review A/B ran → **NOT adopted**, review-strong gpt-5.5/high stays the binding gate). **Banked this generation:** the universal before-AND-after full-suite discipline (`ec8f2d68`, §Process hygiene; CHARC recipe-mirror PENDING) + the review-tier mechanism (`-p review-fast` / `-p review-strong`, recipe §3). The sub-agent dispatch model (cells + the SPOF [`docs/implementer-dispatch-recipe.md`](implementer-dispatch-recipe.md)) is the dispatch mechanism (§"Dispatch execution" above). **QUEUED (operator sequences; no commission until "go"):** **18-D** (RD research data-collection-health monitor — it WRITES `exports/research/health/latest.json`, the §3 envelope 18-F's research stoplight reads → building it lights the grey stoplight; the natural next arc; CHARC owes its arch pass), **18-G** (brief sweep), **R1** (`requests` dep, own dispatch), **18-H.1/.2/.3** (Schwab tokens-absent→yellow · 404 error-surface unification · drill-down stoplight icon). Generational handoff: [`docs/orchestrator-handoff-2026-06-15-phase18-ef-shipped.md`](orchestrator-handoff-2026-06-15-phase18-ef-shipped.md). Trailing (organic): TROX age-off, dividend marker, RD QAs.
