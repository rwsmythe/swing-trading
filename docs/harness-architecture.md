...
**PRESERVE THE QUOTE, CORRECT THE SURROUNDING TEXT, FLAG IT UP (adopted 2026-07-29 — implementer instinct, orchestrator-proposed, RD-ratified, CHARC-canonicalized).** When a review catches an **overstatement in a director's QUOTED words**, the implementer **preserves the quote verbatim**, corrects the document's OWN surrounding sentence, and **flags it up** — it does not silently tighten the director's language. **Why it belongs here rather than in a style guide:** it is the DOWNSTREAM half of premise-sourcing. A position is sourced from the role that holds it; that must include *not editing it downstream*. And the failure would be **invisible** — a softened quote still reads like a quote — so the record would degrade without any signal. If a director wants the claim tightened, the director tightens it (as RD did the same day: *archive lag is ONE divergence condition, not the only one*).
