...
## Amendment 1 -- Two-Codex-chain default refinement (orchestrator-side; 2026-05-27 PM; post-dispatch operator-identified lesson)

Per operator-identified meta-lesson during G2 dispatch + NEW CLAUDE.md gotcha #36 banked at the same housekeeping commit: applied research dispatches default to **TWO** Codex MCP chains, not one. G2 brief Sec 7 + Sec 8 originally prescribed a SINGLE chain after Slice 5 ship. This Amendment supersedes that prescription.

**Why:** historical applied research arcs (V2 executing-plans, D1, R2-A, R2-D) defaulted to a single Codex chain invoked AFTER the smoke artifact + findings draft were shipped. This means (a) the smoke artifact was generated by IMPLEMENTATION code that Codex had not yet reviewed; (b) Codex-caught implementation defects required re-emission of the smoke artifact OR propagation of wrong numbers into findings narratives; (c) single chain reviewed implementation + methodology + numerical accuracy + narrative claims at the same time, overloading the review surface. V2-mechanic arc (merge `64e0099`) shipped the two-Codex-chain split via Brief Amendment 1 and surfaced 2 CRITICAL + 12 MAJOR + 3 MINOR in implementation BEFORE smoke artifact emission; chain #2 then reviewed methodology with implementation already validated. Total 21 issues closed in-place across both chains; minimal re-runs required. The two-chain pattern is materially better than single-chain-at-end.

**Updated chain structure for G2:**

**Chain #1 -- Implementation review (post-Slice-4; BEFORE smoke artifact)**
- Trigger: after all 3 ruleset modules (G + H + I) + scorecard.py + their tests ship + harness extension at run.py
- Review surface: ruleset entry/stop/target/failure logic + scorecard arithmetic + L2 LOCK + cohort-validity discipline (gotcha #33 banned-terms LOCK + gotcha #35 prior-arc-anchor citation discipline) + harness byte-stability of A-F
- Expected rounds: 2-5; converge NO_NEW_CRITICAL_MAJOR
- Goal: validate IMPLEMENTATION correctness BEFORE smoke artifact emission so the smoke is generated by validated code

**Chain #2 -- Results / methodology review (post-Slice-6; AFTER smoke + findings)**
- Trigger: after smoke artifact emitted (Slice 5) + findings doc draft shipped (Slice 6)
- Review surface: 9-metric scorecard numerical accuracy + cross-ruleset narrative claims + cross-cohort consistency interpretation + gotcha #35 prior-arc-anchor citation discipline at narrative layer + L-style methodological caveats + gotcha #31 narrative artifact path/fact lag
- Expected rounds: 1-3; converge NO_NEW_CRITICAL_MAJOR
- Goal: validate METHODOLOGY + NARRATIVE before final ship

The 44th cumulative C.C lesson #6 validation slot SPANS BOTH chains (one slot; two reviews; mirrors V2-mechanic 43rd slot precedent).

**Updated slice cadence (supersedes Sec 8):**

- Slice 1: G_bulkowski + tests
- Slice 2: H_oneil_double_bottom_base + tests
- Slice 3: I_edwards_magee_classical + tests
- Slice 4: scorecard.py + harness extension at run.py + their tests + L2 LOCK + cohort-validity discipline tests
- **Codex chain #1 invocation** (implementation review)
- Codex chain #1 fix bundles (1-3 commits per round)
- Slice 5: Smoke artifact emission (against the now-validated implementation)
- Slice 6: Findings doc draft + narrative_synthesis.md
- **Codex chain #2 invocation** (methodology review)
- Codex chain #2 fix bundles
- Slice 7: Return report + post-fix narrative sweep per gotcha #31

Estimated total commits revised: 10-16 (vs prior 8-14 single-chain estimate; +2 for the additional Codex chain).

**Implementer action:** if you are at or before Slice 4 when reading this Amendment, proceed per the updated structure. If you are past Slice 4 (have already shipped Slice 5+ smoke) but have NOT yet invoked Codex: invoke chain #1 NOW against the implementation diff; do NOT skip the implementation-review step on the assumption that single-chain-at-end will catch everything. If you are past Slice 5 + have already invoked Codex single-chain: document the deviation in the return report Sec 8 and note that the future-arc default supersedes single-chain.

---

*End of Amendment 1. The G2 brief's substantive content (Sec 0-Sec 11 above) is preserved verbatim; Amendment 1 captures the orchestrator-side two-Codex-chain refinement applied retroactively to G2 + forward-binding for all future applied research dispatches via CLAUDE.md gotcha #36.*
