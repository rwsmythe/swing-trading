...
- **#3 coverage_gaps = CALIBRATION A (trailing-1 grace) + CALIBRATION C (current-vs-historical, shipped 2026-06-25).** Tolerates a TRAILING ≤1-session lag (`_COVERAGE_TRAILING_GRACE_SESSIONS=1` — the newest completed session not-yet-observed pre-nightly). **CALIBRATION C: reds ONLY on a CURRENT/ongoing coverage failure** — a trailing lag ≥2 (drumbeat behind/dead), an UNEXPLAINED interior observe hole, OR a zero-observation observe FAILURE (a completed run NAMED the session but wrote no observation for the detection). It **ACCEPTS** (surfaces in the DETAIL as `N accepted historical (missed-run/skip)`, does NOT red) historical holes the drumbeat has provably moved PAST (a later session for that detection IS observed): a **WHOLE-SESSION MISSED RUN** (no completed pipeline run observed that session — keyed on `pipeline_runs.data_asof_date`, NOT the mere absence of observations, so a zero-obs observe-failure still reds) and a per-detection hole explained by a recorded `pattern_observe` skip-warning (`no bar for observation_date` / `non_finite_ohlc`). **So a single missed manual run no longer false-reds the stoplight for weeks** — the 2026-06-24 RED that drove this calibration (a missed 06-22 run = 842 single-session gaps) now reads `842 accepted historical`. A degraded/unparseable run-ledger date → the whole-session clause is conservatively DISABLED (still reds; never accept-everything). The accept requires the BOTH-signals AND (zero global observations AND no completed-run ledger row), so an unexplained interior hole or a globally-observed-but-no-ledger hole still reds.
