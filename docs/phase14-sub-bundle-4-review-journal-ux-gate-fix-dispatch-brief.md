...
- **VERIFY EVERY FILTER OPTION END-TO-END (operator directive 2026-05-30 -- do NOT limit the fix/verification to the 'Open' case):** for EACH of the three filter dimensions -- `filter_state` (EVERY concrete state + EVERY virtual group, incl. 'Open'), `filter_pattern` (EVERY pattern token), `filter_aplus` (aplus + non_aplus) -- every selectable `<select>` option must (a) be a valid allowlist token (never trips `invalid_filter`) AND (b) actually narrow the listing to exactly the correct row set. The 'Open' bug is one instance of a value-contract class; fix + verify the WHOLE contract.
...
- Tests: **PARAMETRIZE over EVERY option of EVERY filter dimension** -- for each option assert (i) it is in its allowlist (enumerate the template's emitted option values vs the frozensets -- the byte-level contract that broke) AND (ii) it filters planted rows to exactly the expected subset (state/virtual-group expansion, pattern, aplus); plus an invalid value falls back gracefully (no 422, `invalid_filter=True`) and a valid value never sets `invalid_filter`. A single combined `<select>`-options-vs-allowlist parity test catches future drift.
