...
- **FIX 2 — construction-barrier guard (CROSSES CHARC's tripwire; swing/data/models.py; an 18-B-class WRITE-BARRIER completion, NOT monitor code; RD measurement-core + CHARC architecture pass; routed to CHARC separately):** add OHLC finiteness validation to `PatternForwardObservation.__post_init__` — RAISE on non-finite (fail-loud backstop, mirroring the existing `sessions_since_detection` guard + 18-A's serializer-raise philosophy; the graceful skip stays at the known caller). Since `insert_observation` takes that dataclass and is the SOLE insert path, a guard there makes non-finite ENTRY structurally impossible regardless of which code constructs the row — closing the gap 18-B's path-enumeration didn't reach. This is what turns the answer to "will non-finite ever be written again?" from a writer-discipline claim into a **structural prevention**.

**Combined steady-state (the point):** with BOTH landed, #1 is GREEN (the 103 baselined out + no new non-finite possible by construction) and reds ONLY if the construction-barrier guard ever REGRESSES and a non-finite slips through — i.e., #1 becomes a true, actionable runtime tripwire (green steady-state; red = a real new event), and the 18-F research stoplight reflects genuine research-measurement health. **Routing:** FIX 1 re-dispatches inside 18-D (RD-blocking, no CHARC pass); FIX 2 routes to CHARC for the architecture pass (swing/data construction barrier) — CHARC scopes whether it expands 18-D's gate or lands as a tightly-coupled companion (e.g. 18-B.1), and sequences relative to FIX 1 (complementary, either order). RD is merge-blocking on BOTH diffs.
