...
- `@contextmanager yfinance_audit_scope(*, db_path, pipeline_run_id, surface)` — `set` on enter, `reset(token)` in `finally`. The runner + CLI labeling use THIS (restore-prior-on-exit); they never call an unconditional clear.
- **Single-active-non-base-scope guard (Codex R4 MAJOR):** a `surface in {'web','cli'}` context set at app/CLI startup is a BASE (no token). `yfinance_audit_scope` (a nested non-base scope, e.g. a `pipeline` run) is allowed OVER a base, but REJECTS (raises `RuntimeError`) if ANOTHER non-base scope is already active — so a stray overlapping non-base scope fails LOUDLY rather than silently clobbering a sibling on `reset` (the non-LIFO clobber hazard). A test proves a second overlapping non-base scope raises; base→scope→restore-base still works.
...
- **Ticker normalization (Codex R4 MINOR):** the helper records `ticker.upper()` in the audit row (metadata ONLY — the `yf.download(...)` argument inside `fetch_fn` is UNCHANGED) so monitor grouping does not split `aapl` vs `AAPL`.
...
- [ ] Tests (`tests/data/test_yfinance_audit_context.py`): set → get round-trips; `reset(token)` restores the PREVIOUS context (set A → token_a; set B → token_b; reset(token_b) → A; reset(token_a) → None); `yfinance_audit_scope(...)` CM sets on enter + restores prior on exit; **nested over base** (set a `web` base; inside a `yfinance_audit_scope(surface='pipeline', run_id=7)`, get → pipeline; after the scope, get → the `web` base, NOT None — proves a pipeline scope does not disable web recording); **single-active-non-base-scope guard** (Codex R4 MAJOR — entering a second overlapping non-base scope while one is already active RAISES `RuntimeError`; a non-base scope OVER a `web`/`cli` base is allowed); default (unset) → `None`; `_set_for_test`/`_reset_for_test` seams.
