...
| `test_lazy_recorder_import_failure_does_not_break_fetch` | force the lazy `from swing.data.yfinance_audit import _record_yf_download` to fail → the chokepoint calls the raw `yf.download` directly + returns | an unisolated lazy import breaks the additive-only fetch → fails (Codex R14 MINOR) |
...
| `test_cli_archive_commands_install_audit_context` (Task 4, static) | AST over `swing/cli.py`: every command referencing `PriceFetcher`/`read_or_fetch_archive`/the labeling-emit helper installs the CLI audit context before the reachable fetch | a future archive-touching command that forgets the helper → fails (Codex R12 MINOR) |
