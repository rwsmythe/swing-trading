...
**Scope (Codex R14 NIT — stated up front):** this audit covers PRODUCTION `swing/` yfinance surfaces ONLY. Research scripts/harnesses also contain `yf.download` (e.g. `research/harness/earnings_proximity/fetchers.py`) and are intentionally OUT of scope unless promoted into an app/runtime surface — the "no site silently skipped" claim below is scoped to `swing/`.
...
- `get_yfinance_audit_context()` → `None` if `_disabled` is active (the overlay below), ELSE `_scope` if present, ELSE `_base`, ELSE `None`.
...
- **Disabled overlay (Codex R12 MAJOR):** a `yfinance_audit_disabled()` context-manager (or a disabled flag the runner sets when its scope entry degrades) takes PRECEDENCE over both `_scope` and `_base` so `get_yfinance_audit_context()` returns `None` — used by the runner to ensure a degraded (stale-scope) run records NO rows rather than MISATTRIBUTING to the stale context.
- `_set_for_test(...)` / `_reset_for_test()` — test seams; `_reset_for_test()` clears BOTH slots to `None` between tests.
- Tests: web base → pipeline scope → `set_base(cli)` while the scope is active → `get()` stays pipeline until the scope exits, THEN `get()` is the new `cli` base; a second overlapping scope raises; **the scope EXIT does NOT raise** (Codex R11 MAJOR — a scope whose exit detects a state mismatch does not raise out of the `with` block and does not alter the body's result/exception); **`yfinance_audit_disabled()` CM** (Codex R13 MAJOR — precedence over BOTH `_scope` and `_base`; `get()` is `None` inside it; token/LIFO restore on exit; nested disabled; "disabled OVER a stale active scope → `get()` is `None`, then the prior state is restored on exit" — NOT a sticky flag); **context-install-time run-linkage validation** (Codex R13 MINOR — `set_yfinance_audit_base_context(surface='pipeline', pipeline_run_id=None)` raises at INSTALL, and `cli`/`web` with a run id raises — fail loud, not a silent insert no-op). (The internal LIFO token validation has no public driver — not separately asserted; Codex R9 NIT.)

**Concurrency — the WEB⇄PIPELINE bleed is impossible by construction; general in-process overlap is an INVARIANT (Codex R3/R7/R15 MAJOR):** the global is a single current-context slot. **The WEB-triggered pipeline ALWAYS runs in a SEPARATE process from the web server** — an architectural FACT on disk: the web POST `/pipeline/run` spawns the pipeline via `subprocess.Popen` (`swing/web/routes/pipeline.py:141`); the web server NEVER calls `run_pipeline` in-process. Therefore the web's `surface='web'` BASE and the pipeline's `surface='pipeline'` scope NEVER coexist in one process — so the WEB⇄pipeline "tag a concurrent web call as pipeline" bleed is **impossible by construction** for the web-triggered path. For a DIRECT in-process `run_pipeline` caller (a public function), the plan states the INVARIANT that the pipeline scope is valid only in a process DEDICATED to that run (mixed same-process workloads under a pipeline scope are unsupported in V1 — see below); it does NOT claim absolute impossibility for that general case. The plan adds a test asserting the web app does NOT run `run_pipeline` in-process (it spawns a subprocess) so a future regression that introduces an in-process runner trips it. **Narrowed claim (Codex R12/R15 MINOR/MAJOR):** "impossible by construction" applies SPECIFICALLY to the WEB-TRIGGERED path (subprocess). `run_pipeline` is a PUBLIC function callable in-process by NON-web code; for that case the plan states an explicit INVARIANT rather than a guarantee: **`yfinance_audit_scope(surface='pipeline')` is valid ONLY in a process DEDICATED to that run** — mixed same-process workloads under a pipeline scope (a concurrent web/CLI/archive call) are UNSUPPORTED in V1 and would be mis-tagged `pipeline` for the scope window. The single-active-scope guard + the disabled overlay cover the realistic cases (stray overlap raises; degraded entry drops); a V2 thread-local/explicit-propagation design is the fix IF mixed same-process workloads become supported. The plan does NOT claim absolute impossibility for the general in-process case. The `yfinance_audit_scope` LIFO token-restore + the single-active-non-base-scope guard are belt-and-suspenders (they make a stray same-process scope-over-scope raise, and a scope-over-`None`/base restore correctly). A V2 execution-local context is the only path IF an in-process web↔pipeline runner is ever added (which the invariant + test forbid). The web base is set once at app startup with no token (the outermost / process-lifetime context).

### The shared instrumentation helper `_record_yf_download` (lives in `swing/data/yfinance_audit.py`)

A single helper used at all 4 sites brackets the RAW `yf.download` (Codex R1 MAJOR — instrument around the raw call, never wrap whole functions). **DB-free-boundary discipline (Codex R2 MAJOR):** the helper lives in `swing/data/yfinance_audit.py` (which imports the repo + `swing.data.db.connect`). `ohlcv_archive.py` must NOT import it at module level (that would break `ohlcv_archive`'s deliberate DB-free-at-import property). Instead each chokepoint, when it sees an active context, does a **function-local (lazy) import** of `_record_yf_download` (a `from swing.data.yfinance_audit import _record_yf_download` inside the function body) — so `import swing.data.ohlcv_archive` pulls in NO repo/db module at import time. The no-context fast path imports nothing. **The lazy import is ISOLATED — but NARROWLY (Codex R14/R15 MINOR):** the `try/except` wraps ONLY the lazy `from swing.data.yfinance_audit import _record_yf_download` (+ pre-dispatch metadata prep); on import failure it logs + calls the EXACT raw `yf.download` closure directly (single fetch). The `_record_yf_download(...)` dispatch is OUTSIDE that except — a blanket except around the whole call could re-call `fetch_fn` after a POST-fetch internal failure → a DUPLICATE network fetch. Once entered, `_record_yf_download`'s internal isolation governs (it calls `fetch_fn` exactly once, never re-fetches). (`labeling_bars.py` and `price_cache.py` already touch DB/CLI concerns, so a module-level import there is fine; only `ohlcv_archive.py` carries the DB-free-import constraint. A test asserts `import swing.data.ohlcv_archive` does not add `swing.data.db` / `swing.data.repos.yfinance_calls` to `sys.modules`.)
...
# The isolation try/except wraps ONLY the lazy import (Codex R15 MINOR -- a
# blanket except around the whole _record_yf_download call could re-call
# fetch_fn after a post-fetch internal failure -> a DUPLICATE network fetch):
#   ctx = get_yfinance_audit_context()
#   if ctx is None: return yf.download(...)        # no import, no DB, zero new behavior
#   try:
#       from swing.data.yfinance_audit import _record_yf_download   # lazy, DB-free boundary
#   except Exception:                              # import failed -> single raw fetch
#       log.warning(...); return yf.download(...)
#   # dispatch OUTSIDE the except: once entered, _record_yf_download's INTERNAL
#   # isolation governs (it calls fetch_fn EXACTLY ONCE, never re-fetches):
#   return _record_yf_download(ctx=ctx, call_type=..., ticker=..., ticker_count=...,
#                              fetch_fn=lambda: yf.download(...))

YFINANCE_AUDIT_BUSY_TIMEOUT_MS = 0     # Codex R3/R12: busy_timeout=0 -> a contended
                                       # BEGIN IMMEDIATE fails INSTANTLY (no wait), so
                                       # the _YF_AUDIT_WRITE_LOCK is NEVER held across a
                                       # wait -> no N x timeout convoy + sub-ms critical
                                       # section (the 30000ms project default would blow
                                       # the 3s web price-fetch deadline under contention)

# Serialization (Codex R11/R12): a yfinance-specific _YF_AUDIT_WRITE_LOCK is held ONLY
# around each SHORT start/finish BEGIN IMMEDIATE tx -- NEVER around yf.download. The
# audit connection uses busy_timeout=0, so a CONTENDED BEGIN IMMEDIATE fails INSTANTLY
# (skip audit, no wait) and the lock is NEVER held across a wait -> the critical section
# is always sub-ms. So: same-process audits serialize DETERMINISTICALLY (uncontended ->
# all rows record); cross-process contention fails instantly (best-effort drop); and
# there is NO N x timeout convoy (no worker waits while holding the lock). record_call_start
...
def _record_yf_download(*, ctx, call_type, ticker, ticker_count, fetch_fn):
    ...
    try:                             #   latency, not audit-start/classification overhead
        result = fetch_fn()
    except Exception as exc:         # the fetch raised
        ms = int((monotonic()-t0)*1000)   # Codex R16: fix the metric BEFORE any
        _finish_safe(conn, call_id, status='error', rows=None, error=_sanitize(exc), ms=ms)
        _safe_close(conn)
        raise                        # PRESERVE the raise (single + intraday paths)
    ms = int((monotonic()-t0)*1000)  # Codex R16: response_time_ms fixed HERE, BEFORE
                                     #   classification (a slow/changed-shape classify
                                     #   must not inflate the recorded yfinance latency)
    ...
    try:
        ...
        log.warning(...); _finish_safe(conn, call_id, status='success', rows=None, error=None, ms=ms)
        _safe_close(conn)
...
- **`busy_timeout=0` (Codex R3/R12 CRITICAL):** the audit connection opens with `busy_timeout_ms=YFINANCE_AUDIT_BUSY_TIMEOUT_MS` (**0** — instant fail) so a contended `BEGIN IMMEDIATE` fails IMMEDIATELY (no wait) and the start-failure path runs `fetch_fn()` un-audited — the audit can NEVER block the fetch (the 30s project default would blow the web price-fetch deadline; even a 100ms timeout would convoy N workers N×100ms — R12). A test holds an external write transaction and asserts `_record_yf_download` calls `fetch_fn` + returns promptly (near-zero wait) un-audited.
- **Serialization — short-critical-section lock + `busy_timeout=0` (Codex R9/R10/R11/R12, reconciled):** a yfinance-specific `_YF_AUDIT_WRITE_LOCK` is held ONLY around each SHORT start `BEGIN IMMEDIATE` tx and (separately) each short finish tx — NEVER around `yf.download`. The audit connection uses **`busy_timeout=0`**, so a contended `BEGIN IMMEDIATE` fails INSTANTLY (skip audit, no wait) and the lock is NEVER held across a wait — the critical section is always sub-ms. So: (a) same-process concurrent audits serialize DETERMINISTICALLY (uncontended sub-ms INSERTs → all rows record, no flaky-CI drop — Codex R11 MINOR); (b) there is NO `N × timeout` convoy (Codex R9/R12) because no worker ever waits while holding the lock; (c) ANY write-lock holder colliding with the audit `BEGIN IMMEDIATE` (cross-process AND same-process NON-audit writers — the pipeline heartbeat/lease/status writes — Codex R14 MINOR) fails instantly → best-effort drop (logged; the fetch is never delayed). Tests: N concurrent active-context fetches with NO external writer → all N rows recorded; N concurrent fetches WITH an external SQLite writer → all fetches return within deadline (wall time bounded by ONE near-zero attempt, NOT `N × timeout`; some audits best-effort drop); a heartbeat-like writer holding `BEGIN IMMEDIATE` while `_record_yf_download` runs → the fetch returns + the audit best-effort drops (no delay).
...
- `_finish_safe` calls `record_call_finish` inside its own try/except — an audit-finish failure logs + is swallowed (never breaks the fetch / the caller's control flow). **The finish uses the SAME `busy_timeout=0` connection** (Codex R8/R12 MAJOR), so a contended finish fails INSTANTLY (no wait) — the audit FINISH (which runs after `yf.download` and before the caller returns) can never push a just-in-deadline quote into stale fallback. A finish-contention test (hold an external write lock AFTER `fetch_fn` returns) asserts the caller returns promptly with the original result.
- **Stale `in_flight` rows (Codex R10 MINOR):** a rare dropped finish (the contended-finish path) leaves the row `in_flight` permanently. So the row closes on every path EXCEPT a best-effort finish drop. The monitors (18-D/18-E) MUST treat a stale `in_flight` row (no terminal status, `ts` older than a small grace) as INCOMPLETE/unknown — NOT as a hung call — and the repo read helpers document this convention. NO new status is added (the enum lock holds).
...
CREATE TABLE yfinance_calls (
    ...
    -- Run-linkage invariant (Codex R12 MAJOR): pipeline rows MUST carry a run id;
    -- cli/web rows MUST NOT (they have no pipeline run). Catches a context-install
    -- bug that creates unattributable pipeline rows.
    CHECK (
        (surface = 'pipeline' AND pipeline_run_id IS NOT NULL)
        OR
        (surface IN ('cli', 'web') AND pipeline_run_id IS NULL)
    ),
    ...
    -- ON DELETE RESTRICT (Codex R13 MAJOR + NIT): a pipeline yfinance row MUST keep
    -- its run_id (the run-linkage CHECK forbids NULL), so a pipeline_runs row CANNOT
    -- be deleted while yfinance_calls reference it. DELIBERATE divergence from
    -- schwab_api_calls' ON DELETE SET NULL: yfinance pipeline rows retain MANDATORY
    -- run provenance, a different FK lifecycle by design.
...
Notes:
- `pipeline_runs` PK is `id` (migration 0003), so the FK references `pipeline_runs(id)` (same column `schwab_api_calls` references) — but with **`ON DELETE RESTRICT`** (Codex R13), NOT schwab's `SET NULL`: the run-linkage CHECK makes a pipeline row's run_id mandatory, so SET NULL would violate the CHECK on a parent delete. Deleting a `pipeline_runs` row is restricted while yfinance audit rows reference it (mandatory run provenance — a deliberate divergence from schwab). **Forward constraint (Codex R14 MINOR):** there is NO current `pipeline_runs` deletion path; any FUTURE run-retention/pruning work MUST prune `yfinance_calls` FIRST (or the RESTRICT FK blocks it).
...
- [ ] **Migration tests** (`tests/data/test_migration_0030_yfinance_calls.py`): table/11-columns/6-indexes/CHECKs (enum + numeric + shape) via `PRAGMA table_info` + `sqlite_master`; INSERT `status='empty'` ok, `status='bogus'` → `IntegrityError`; `call_type='download_intraday'` ok, `'frobnicate'` rejected; `surface='web'` ok, `'mobile'` rejected; the numeric CHECKs (negative `response_time_ms`/`rows_returned`, `ticker_count<=0` rejected) + the shape CHECK (a `download_batch` row with `ticker` set, or a `download_single` row with `ticker_count` set, rejected); a valid `download_batch` in-flight row (`ticker_count` set, `ticker` NULL) accepted; the empty-ticker CHECK (`ticker=''` / whitespace rejected — Codex R7 MINOR); the **run-linkage CHECK** (`surface='pipeline'` + NULL run id rejected; `cli`/`web` + non-null run id rejected — Codex R12 MAJOR); **FK behavior under `PRAGMA foreign_keys=ON`** (Codex R7/R13 MINOR/MAJOR): insert-with-valid-`pipeline_run_id` ok; an invalid `pipeline_run_id` rejected; deleting a referenced `pipeline_runs` row is RESTRICTED (`ON DELETE RESTRICT` raises) while a yfinance row references it — catches a wrong FK column / wrong action + is consistent with the run-linkage CHECK; **migrate-twice no-op** (run `run_migrations` twice; version stays 30, no error); backup-gate STRICT-equality (v29 file DB → creates `swing-pre-phase18-arc-c-migration-*.db`; fresh v0 walk does NOT fire this gate); `EXPECTED_SCHEMA_VERSION == 30`.
- [ ] **Model tests** (`tests/data/test_yfinance_call_model.py`): valid `YfinanceCall` constructs; `status`/`call_type`/`surface` outside the frozensets → `ValueError`; `response_time_ms` negative / bool → `ValueError`; `rows_returned` negative → `ValueError`; **bool rejection for EVERY integer field** — `pipeline_run_id=True` / `ticker_count=True` / `response_time_ms=True` / `rows_returned=True` → `ValueError` (bool is an int subclass — Codex R17 MINOR); `error_message=None` ok; **shape-invariant mirror (Codex R6 MINOR)** — `download_batch` with `ticker` set / without `ticker_count`, and `download_single`/`download_intraday` with `ticker_count` set / without `ticker`, each → `ValueError` (the dataclass `__post_init__` mirrors the SQL shape CHECK); an empty/whitespace `ticker` → `ValueError` (mirrors the `length(trim(ticker)) > 0` CHECK — Codex R7 MINOR); the run-linkage invariant (`surface='pipeline'` with NULL run id, or `cli`/`web` with a non-null run id) → `ValueError` (mirrors the SQL run-linkage CHECK — Codex R12 MAJOR).
...
- [ ] Implement, in ONE commit: the migration SQL; the backup-gate fn + expected-tables constant + `run_migrations` registration + `EXPECTED_SCHEMA_VERSION` bump; the `_YFINANCE_VALID_STATUSES`/`_YFINANCE_VALID_CALL_TYPES`/`_YFINANCE_VALID_SURFACES` constants + `@dataclass YfinanceCall` (`__post_init__` validators: enum frozensets, bool-is-int reject for EVERY integer field [`response_time_ms`, `rows_returned`, `pipeline_run_id`, `ticker_count` — Codex R17], non-negative numeric, the batch-vs-single SHAPE invariant, the non-empty-ticker invariant, AND the run-linkage invariant — `pipeline`⇒run_id-set, `cli`/`web`⇒run_id-NULL — all mirroring the SQL CHECKs) in `swing/data/models.py`; the `swing/data/repos/yfinance_calls.py` repo (`_SELECT_COLUMNS`, `_row_to_model`, `insert_in_flight(conn, *, ts, call_type, ticker, ticker_count, pipeline_run_id, surface)` [status hardcoded `'in_flight'`; caller controls tx — NO `commit()`], `update_call_outcome(conn, *, call_id, response_time_ms, status, rows_returned, error_message)` [plain `UPDATE`, never `INSERT OR REPLACE`], `get_call`). **Enum-grep rule (Codex R5 NIT + R14 MINOR — precise):** the SCHEMA CHECK + the Python VALIDATOR copies (the `_YFINANCE_VALID_*` constants + the dataclass `__post_init__` frozensets + the read-path `_row_to_model`) must mirror atomically and be the ONLY validator/check sets — grep `swing/` to confirm NO additional validator/check set drifts. **The context-install validation in `yfinance_audit_context.py` IMPORTS the shared `_YFINANCE_VALID_SURFACES` constant (it does NOT hardcode a copy) and is INCLUDED in this grep rule.** Call sites passing enum LITERALS/constants (`call_type='download_single'`, `surface='web'`, etc.) are fine and expected; the grep is not against call-site literals. See fail → see pass.
...
- [ ] Implement the lock-guarded global (DB-free; mirroring `log_correlation.py`) — a `_base` slot + a `_scope` slot + a `_disabled` overlay — with `set_yfinance_audit_base_context(...)` (updates `_base` only; never touches `_scope`; VALIDATES surface + run-linkage at install, raising on a bad combo — Codex R13), `@contextmanager yfinance_audit_scope(...)` (sets/clears `_scope`; LIFO token-restore; single-active-scope guard; NON-THROWING production exit; same install-time validation), `@contextmanager yfinance_audit_disabled()` (token/LIFO; precedence over `_scope` AND `_base`), `get_yfinance_audit_context` (`None` if disabled, else `_scope` else `_base` else `None`), `_set_for_test`/`_reset_for_test` (clears all). See fail → see pass.
...
- [ ] Tests:
  ...
  - a web-app test: app construction sets the persistent `surface='web'` base context FROM the finalized `app.state.cfg` (assert via `get_yfinance_audit_context`). (A nested base→scope→restore push/pop assertion lives in the Task 1 CONTEXT unit tests as pure mechanics — it does NOT prove a production web/pipeline safety property; the production guarantee rests ONLY on the subprocess + static guards — Codex R17 MINOR.)
  ...
  - **non-dashboard web route coverage (Codex R15 MAJOR):** construct the web app WITHOUT pre-seeding context, invoke a NON-dashboard route/path that reaches `read_or_fetch_archive` (a chart-rendering path via `routes/patterns.py` / `routes/trades.py` / `trade_charts.py`), and assert the chokepoint sees `surface='web'` (proves the process-wide base covers ALL web yfinance paths, not just the dashboard live-price).
  - **pruning-order test (Codex R13/R15 MINOR):** under `PRAGMA foreign_keys=ON`, deleting a `pipeline_runs` row referenced by a `yfinance_calls` row raises (RESTRICT); deleting the `yfinance_calls` row(s) first then the run succeeds (the documented forward pruning order).
- [ ] Bracket `_yf_download_window_for_labeling:46` + `_fetch_live_price`'s raw `yf.download:206` via the ctx-snapshot + `_record_yf_download` pattern (these two modules already touch DB/CLI concerns, so a module-level import of the recorder is fine here). Set the context: **runner** — enter ONE OUTER `with yfinance_audit_scope(surface='pipeline', pipeline_run_id=lease.run_id, db_path=cfg.paths.db_path):` IMMEDIATELY after `set_pipeline_run_id(lease.run_id)` (`runner.py:592`), STRUCTURALLY enclosing the heartbeat create/start + the ENTIRE post-lease body through the existing `finally` — so EVERY post-lease `return RunResult(...)` is lexically inside the scope (restoration guaranteed) AND no post-lease yfinance call runs before the scope opens (Codex R3/R9 MAJOR). **No-raise production entry + DISABLED OVERLAY (Codex R10/R12 MAJOR):** the scope entry must NOT strand a lease — wrap the entry so a scope-guard/entry failure (a stale/overlapping scope) is caught + logged + degrades to no-audit (the lease + run proceed normally). Crucially (Codex R12/R13), "degrade to no-audit" enters the `yfinance_audit_disabled()` CM (a proper token/LIFO context-manager, NOT a sticky flag) which takes PRECEDENCE over both `_scope` and `_base` so `get_yfinance_audit_context()` returns `None` for that run body — otherwise the run's yfinance calls would be MISATTRIBUTED to the stale run/surface (worse than dropping). An audit-context problem must NEVER wedge a pipeline lease NOR misattribute its rows. An AST/structural test asserts every post-lease `return RunResult` is inside the `with` block (not just the finviz/evaluate examples); a test pre-seeds an active scope, runs the runner path, and asserts the run proceeds + the lease releases normally (NOT left wedged); **CLI** — install the `surface='cli'` BASE context ONCE in the TOP-LEVEL Click GROUP callback AFTER `cfg` resolution, ONLY when `ctx.invoked_subcommand not in {"db-migrate", "pipeline"}` (Codex R13/R17 MINOR — a single install point covers any future archive-touching command BY CONSTRUCTION, vs fragile per-command opt-in; the `pipeline` subgroup is EXCLUDED so it relies solely on `yfinance_audit_scope(surface='pipeline')`/the disabled overlay — a CLI base in a pipeline process is unnecessary + could misattribute a stray post-scope call). A shared `install_yfinance_cli_audit_context(cfg)` helper does the install; test harnesses reset the global between Click invocations; **web app startup** (`set_yfinance_audit_base_context(surface='web', pipeline_run_id=None, db_path=app.state.cfg.paths.db_path)` once, AFTER `app.state.cfg` is finalized — Codex R7 MINOR — the persistent outermost base, no token restore needed). The CLI base helper uses `set_yfinance_audit_base_context(surface='cli', pipeline_run_id=None, ...)`. See fail → see pass. grep: no enum drift.
...
| `test_rows_returned_is_the_RAW_frame_count` (Codex R16 MINOR) | the RAW `yf.download` frame is CHANGED by post-processing (a trailing-ragged row OR a MultiIndex column squeeze) → audit `rows_returned` == the RAW frame's row count (before squeeze/trim/column-select), while the caller's returned frame is the post-processed one | an impl that records `rows_returned` AFTER downstream processing → fails (proves the bracket records the raw `yf.download` boundary) |
| `test_finish_rejects_in_flight_status` (service) | `record_call_finish(status='in_flight')` raises; only `success`/`empty`/`error` accepted | a finish that accepts `in_flight` lets a row "finish" into a non-terminal state monitors misread → fails (Codex R16 MINOR) |
...
| `test_audit_finish_contention_does_not_delay_fetch` | hold an external write lock AFTER `fetch_fn` returns → the caller returns promptly (instant-fail finish) with the original result | a default-busy-timeout finish blocks the caller past the price-fetch deadline → fails (Codex R8 MAJOR) |
| `test_same_process_nonaudit_writer_drops_audit_not_fetch` | a heartbeat-like writer holds `BEGIN IMMEDIATE` while `_record_yf_download` runs → the fetch returns + the audit best-effort drops (no delay) | proves same-process non-audit contention is handled like cross-process (Codex R14 MINOR) |
| `test_lazy_recorder_import_failure_does_not_break_fetch` | force the lazy `from swing.data.yfinance_audit import _record_yf_download` to fail → the chokepoint calls the raw `yf.download` EXACTLY ONCE + returns | an unisolated lazy import breaks the additive-only fetch → fails (Codex R14 MINOR) |
| `test_post_fetch_internal_failure_does_not_double_fetch` | a spy `yf.download` counts calls; `_record_yf_download` fails AFTER `fetch_fn` (post-fetch) → `yf.download` was called EXACTLY ONCE (no re-fetch) | a blanket-except-around-the-whole-call impl re-calls `fetch_fn` → spy count 2 → fails (Codex R15 MINOR) |
...
| `test_audit_lock_contention_does_not_delay_fetch` | hold an external write tx; `_record_yf_download` STILL calls `fetch_fn` + returns near-instantly (`busy_timeout=0`), un-audited | a default-30s-busy-timeout impl blocks the fetch up to 30s → fails the prompt-return bound (Codex R3/R12 CRITICAL) |
...
| `test_concurrent_fetches_with_writer_return_within_deadline` | N concurrent fetches WITH an external SQLite writer → ALL fetches return within the deadline; wall time bounded by ONE near-zero attempt, NOT `N × timeout` (some audits best-effort drop) | a blocking/100ms-timeout-lock impl convoys N×timeout past the deadline → fails (Codex R9/R12 CRITICAL) |
...
| `test_runner_degraded_scope_records_no_stale_rows` (Task 4) | pre-seed a STALE active scope; run a path reaching yfinance → NO audit row is written under the stale context (the disabled overlay → `get()` is `None`) | an impl that degrades but leaves `_scope` active misattributes rows to the stale run/surface → fails (Codex R12 MAJOR) |
| `test_run_linkage_check` (Task 0) | `surface='pipeline'` with NULL `pipeline_run_id` rejected; `surface='cli'`/`'web'` with a non-null run id rejected; valid combos accepted | a missing run-linkage CHECK lets a context-bug create unattributable pipeline rows → fails (Codex R12 MAJOR) |
| `test_cli_group_callback_installs_audit_context` (Task 4) | the top-level Click GROUP callback installs the `surface='cli'` base for representative commands WITHOUT pre-seeding (the REAL guarantee — covers indirect/dynamic reaches by construction) | a missing top-level install → a command's chokepoint sees no context → fails (Codex R12/R15 MINOR). The AST scan over `swing/cli.py` is kept as an ADVISORY supplement, not the guarantee. |
...
- **Per-chokepoint short-lived `connect(db_path, busy_timeout_ms=0)`** — the chokepoints can't share the pipeline's `audit_conn` (no threaded conn). Volume is ~10–15 calls/night (§Volume), so the per-call `connect()` is negligible. The `_YF_AUDIT_WRITE_LOCK` serializes audit tx WITHIN a process (deterministic — sub-ms critical sections); ACROSS processes (web/pipeline/CLI) the `busy_timeout=0` instant-fail means a cross-process write-lock collision drops the audit row rather than blocking the fetch — recording is best-effort under cross-process contention (Codex R4/R12 MAJOR), NOT a strong cross-connection serialization guarantee.
- **Concurrency-safety** (Codex R3/R4/R7 MAJOR): the single-slot global is data-race-free (lock-guarded); the `yfinance_audit_scope` single-active-non-base-scope guard makes a stray same-process overlap raise rather than silently clobber. The web⇄pipeline bleed is impossible BY CONSTRUCTION: the WEB-TRIGGERED pipeline always runs in a subprocess (`web/routes/pipeline.py:141` `subprocess.Popen`; the web never runs `run_pipeline` in-process — a test pins this), so the `surface='web'` base and the `surface='pipeline'` scope never share a process. **No limitation for the current web-triggered architecture (Codex R16 NIT); direct in-process mixed workloads under a pipeline scope remain UNSUPPORTED in V1** (the explicit invariant above — a V2 thread-local/explicit-propagation design is the fix if they become supported).
...
**`ON DELETE RESTRICT` is a documented HARD PRODUCT DECISION (Codex R13/R15 MAJOR/MINOR):** the pipeline-run FK uses RESTRICT (not schwab's SET NULL) so a pipeline yfinance row retains MANDATORY run provenance forever (the run-linkage CHECK forbids NULL). There is NO current `pipeline_runs` deletion path, so this blocks nothing today. The FORWARD CONSTRAINT: any future run-retention/pruning work MUST prune `yfinance_calls` (for that run) BEFORE the `pipeline_runs` row, or the RESTRICT FK blocks the delete. A pruning-order test pins this. (The alternative — SET NULL + an insert-only run-linkage check — was rejected: it would lose the forever-attribution guarantee the run-linkage lock wants.)
