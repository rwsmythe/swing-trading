...
- Tests: web base → pipeline scope → `set_base(cli)` while the scope is active → `get()` stays pipeline until the scope exits, THEN `get()` is the new `cli` base; a second overlapping scope raises; an invalid/non-LIFO restore raises.
...
- [ ] Tests:
  ...
  - **the hard-invariant test (Codex R7 MAJOR):** assert the web POST `/pipeline/run` spawns the pipeline as a SUBPROCESS (`subprocess.Popen` at `web/routes/pipeline.py:141`) and does NOT call `run_pipeline` in-process — so a future regression introducing an in-process web runner (which would re-open the web↔pipeline context-bleed window) trips this test.
