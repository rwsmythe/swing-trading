...
- **Non-blocking lock acquisition (Codex R9 CRITICAL — the Python-lock convoy):** the yfinance audit uses its OWN lock (`_YF_AUDIT_WRITE_LOCK`, NOT the schwab `_AUDIT_WRITE_LOCK`) acquired NON-BLOCKING (`lock.acquire(blocking=False)`); if it cannot acquire immediately it SKIPS the audit (logs + runs `fetch_fn()` un-audited). This bounds the TOTAL audit budget (Python-lock wait + the 100ms SQLite wait) so a convoy of N concurrent executor workers can never SUM past the price-fetch deadline. (The schwab audit can afford a blocking wait — no sub-second fetch deadline — but the yfinance web path cannot, so it does NOT share the blocking-acquire semantics.) A concurrent-contention test (`max_concurrent_price_fetches` workers + an external SQLite writer) asserts the fetches still return within deadline.
...
- [ ] Implement mirroring `audit_service.py` (the `record_call_start`/`record_call_finish` halves, `in_transaction` reject, `BEGIN IMMEDIATE`/COMMIT/ROLLBACK) but with a DEDICATED `_YF_AUDIT_WRITE_LOCK` acquired NON-BLOCKING (`acquire(blocking=False)`; skip-audit-on-miss — Codex R9 CRITICAL, NOT the schwab blocking lock) + the audit connection opened with `busy_timeout_ms=100` + the `_record_yf_download` helper (§The shared instrumentation helper) + `_sanitize_error` + `_safe_close`/`_finish_safe` (BOTH the start and finish audit calls wrapped so neither can sink/delay the fetch). Module docstring documents the deliberate light-redaction posture (no auth token → no `setLogRecordFactory`) + the non-blocking-lock + low-busy-timeout deadline-protection. NO `environment` param anywhere. NO sandbox gate.
...
| `test_audit_lock_convoy_does_not_delay_concurrent_fetches` | `max_concurrent_price_fetches` workers all hit `_record_yf_download` while an external SQLite writer holds the lock → ALL fetches return within the deadline (the non-blocking `_YF_AUDIT_WRITE_LOCK` acquisition skips audit rather than queueing) | a blocking-lock impl convoys N×100ms past the deadline → fails (Codex R9 CRITICAL) |
