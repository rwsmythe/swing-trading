...
**Design:** a new DB-free `swing/data/yfinance_audit_context.py` (mirroring `log_correlation.py`'s lock-guarded global) with **push/pop (token-restore) semantics** (Codex R2 MAJOR — the persistent web base must not be clobbered by a nested pipeline run's clear):
- A lock-guarded current context `(db_path: Path, pipeline_run_id: int | None, surface: str) | None` (default `None` → recording no-ops).
- `set_yfinance_audit_context(*, db_path, pipeline_run_id, surface) -> Token` — capture the PREVIOUS context, install the new one, return an opaque token carrying the previous.
- `reset_yfinance_audit_context(token)` — restore the previous context the token captured (NOT an unconditional clear).
- `@contextmanager yfinance_audit_scope(*, db_path, pipeline_run_id, surface)` — `set` on enter, `reset(token)` in `finally`. The runner + CLI use THIS (restore-prior-on-exit); they never call an unconditional clear.
- `get_yfinance_audit_context()` → the current context or `None`.
...
**Concurrency note:** the global is a single current-context slot (not a stack of arbitrary depth) — sufficient because the realistic nesting is at most web-base → one pipeline run (the pipeline subprocess and the web server are normally separate processes; the in-process overlap is the defended edge case). The token-restore makes web-base → pipeline → restore-web correct; a pipeline `reset(token)` restores the web base rather than disabling recording. The web base is set once at app startup with no token (it is the outermost / process-lifetime context).
...
- [ ] Tests (`tests/data/test_yfinance_audit_context.py`): set → get round-trips; `reset(token)` restores the PREVIOUS context (set A → token_a; set B → token_b; reset(token_b) → A; reset(token_a) → None); `yfinance_audit_scope(...)` CM sets on enter + restores prior on exit; **nested/overlap** (set a `web` base; inside a `yfinance_audit_scope(surface='pipeline', run_id=7)`, get → pipeline; after the scope, get → the `web` base, NOT None — proves a pipeline scope does not disable web recording); default (unset) → `None`; `_set_for_test`/`_reset_for_test` seams.
- [ ] Implement the lock-guarded global mirroring `log_correlation.py` (DB-free) with `set_yfinance_audit_context` (returns a token capturing the previous), `reset_yfinance_audit_context(token)`, `@contextmanager yfinance_audit_scope(...)`, `get_yfinance_audit_context`, `_set_for_test`/`_reset_for_test`. See fail → see pass.
...
- [ ] Tests (`tests/data/test_yfinance_audit_service.py`): `record_call_start` inserts in-flight + returns id; `record_call_finish` sets terminal fields; both REJECT a caller-held tx (`CallerHeldTransactionError`); `_AUDIT_WRITE_LOCK` serialized (smoke); `_sanitize_error` collapses whitespace + truncates to 200; **`_record_yf_download` unit tests** with a fake `fetch_fn` + a real on-disk DB + a `yfinance_audit_scope` context: success (rows+timing+`success`), empty-df (`empty`, rows 0), raising `fetch_fn` (`error` + row closed + re-raises), NO context (no row + returns `fetch_fn()` result), **audit-START-failure isolation** (a bogus `db_path` → `fetch_fn()` still returns; no raise), and **audit-FINISH-failure isolation** (force `record_call_finish`/`_finish_safe` to raise on the success, empty, AND error paths → the original return/re-raise is unchanged) (Codex R2 MINOR).
...
- [ ] Tests:
  - `tests/patterns/test_labeling_bars_yfinance_audit.py`: under a `surface='cli'` scope, `autofetch_bars_for_labeling` records a `download_single` row (kwargs-spy `threads=False`); empty → `empty`; without context → no row + identical behavior.
  ...
  - `tests/pipeline/test_runner_yfinance_audit_context.py`: a run enters a `yfinance_audit_scope(surface='pipeline', run_id=<real>, db_path=...)` and RESTORES the prior context on exit (assert prior restored, not unconditionally cleared).
...
- [ ] Bracket `_yf_download_window_for_labeling:46` + `_fetch_live_price`'s raw `yf.download:206` via `_record_yf_download` (these two modules already touch DB/CLI concerns, so a module-level import of the recorder is fine here). Set the context: runner (after `set_pipeline_run_id`, wrap the run body in `yfinance_audit_scope(surface='pipeline', run_id=lease.run_id, db_path=cfg.paths.db_path)` so the prior context is restored on exit — replaces the earlier "clear in finally" wording); CLI `pattern label` emit path (`yfinance_audit_scope(surface='cli', run_id=None, ...)`); web app startup (`set_yfinance_audit_context(surface='web', run_id=None, ...)` once — the persistent outermost base, no token restore needed). See fail → see pass. grep: no enum drift.
...
| `test_ohlcv_archive_import_is_db_free` | `import swing.data.ohlcv_archive` does NOT add `swing.data.db` / `swing.data.repos.yfinance_calls` to `sys.modules` | a module-level recorder import breaks the DB-free-at-import property → fails (Codex R2 MAJOR) |
...
- **Web context is the persistent (process-lifetime) BASE, not per-request** — the web server is long-lived with no clean per-request run boundary; a once-set `surface='web'` base context tags every web-triggered yfinance call. This audits the web paths (resolving the Codex R1 #27 contradiction) at the cost of no per-request run-id (`pipeline_run_id=NULL` for web rows — correct, web has no pipeline run). A nested in-process pipeline run uses `yfinance_audit_scope` (push/pop) so it restores the web base on exit and never clobbers it (Codex R2 MAJOR). V2: a request-scoped context mirroring `SWING_WEB_REQUEST_ID`.
