...
`grep -rn "yf\.download(" swing/` returns exactly **4 invocation sites in 4 files** (every `yf.download` in `swing/` is an actual call; no commented-out call sites):
...
**Design:** a new DB-free `swing/data/yfinance_audit_context.py` (mirroring `log_correlation.py`):
- A lock-guarded immutable context `(db_path: Path, pipeline_run_id: int | None, surface: str)` + an active flag (default inactive → recording no-ops).
- `set_yfinance_audit_context(*, db_path, pipeline_run_id, surface)` — set + activate.
- `clear_yfinance_audit_context()` — deactivate (runner `finally`; mirrors how `audit_conn` is closed in `finally`).
...
A single helper used at all 4 sites brackets the RAW `yf.download` (Codex R1 MAJOR — instrument around the raw call, never wrap whole functions):

```
def _record_yf_download(*, call_type, ticker, ticker_count, fetch_fn):
    ctx = get_yfinance_audit_context()
    if ctx is None:
        return fetch_fn()            # opt-out: zero new behavior
    ...
    except Exception:                # audit start failed -> NEVER sink the fetch
...
1. **NO measurement-chain change.** `validate_bars`, the archive parquet contents, the `_trim_trailing_ragged` trim, the candidate/temporal-log shapes, the `_fetch_live_price` float/RuntimeError contract, the `_fetch_chunk` `({}, list(chunk), True)` fallback: untouched. The audit records AROUND the raw `yf.download`; it never alters the call, its kwargs (`threads=False` single / authorized `threads=True` batch / the intraday `group_by="column"`), or its return value. An audit-write failure is caught + logged, never propagated into the fetch. **Proof on disk:** the helper returns the unmodified `yf.download` result; a test asserts the returned DataFrame is identical with and without an active context, and that the `_fetch_chunk` fallback tuple is byte-identical when `yf.download` raises under an active context.
...
3. **Always-on recording (NO sandbox gate).** Unlike `_step_schwab_*`, the yfinance audit records in ALL environments — NO `environment` column, NO production-only short-circuit. Recording fires whenever a context is active, regardless of `cfg.integrations.schwab.environment`. **Proof on disk:** the helper has no `environment` read; a test under `environment='sandbox'` still records a row.
...
- `ticker` vs `ticker_count`: single/intraday set `ticker` (leave `ticker_count` NULL); batch sets `ticker_count=len(chunk)` (leave `ticker` NULL). No CHECK couples them (the dataclass `__post_init__` is the soft contract; over-constraining the SQL risks brittle migrations).
...
- [ ] **Migration tests** (`tests/data/test_migration_0030_yfinance_calls.py`): table/11-columns/4-indexes/3-CHECKs via `PRAGMA table_info` + `sqlite_master`; INSERT `status='empty'` ok, `status='bogus'` → `IntegrityError`; `call_type='download_intraday'` ok, `'frobnicate'` rejected; `surface='web'` ok, `'mobile'` rejected; **migrate-twice no-op** (run `run_migrations` twice; version stays 30, no error); backup-gate STRICT-equality (v29 file DB → creates `swing-pre-phase18-arc-c-migration-*.db`; fresh v0 walk does NOT fire this gate); `EXPECTED_SCHEMA_VERSION == 30`.
- [ ] **Model tests** (`tests/data/test_yfinance_call_model.py`): valid `YfinanceCall` constructs; `status`/`call_type`/`surface` outside the frozensets → `ValueError`; `response_time_ms` negative / bool → `ValueError`; `rows_returned` negative → `ValueError`; `error_message=None` ok.
...
- [ ] Implement, in ONE commit: the migration SQL; the backup-gate fn + expected-tables constant + `run_migrations` registration + `EXPECTED_SCHEMA_VERSION` bump; the `_YFINANCE_VALID_STATUSES`/`_YFINANCE_VALID_CALL_TYPES`/`_YFINANCE_VALID_SURFACES` constants + `@dataclass YfinanceCall` (`__post_init__` validators, bool-is-int reject for int fields) in `swing/data/models.py`; the `swing/data/repos/yfinance_calls.py` repo (`_SELECT_COLUMNS`, `_row_to_model`, `insert_in_flight(conn, *, ts, call_type, ticker, ticker_count, pipeline_run_id, surface)` [status hardcoded `'in_flight'`; caller controls tx — NO `commit()`], `update_call_outcome(conn, *, call_id, response_time_ms, status, rows_returned, error_message)` [plain `UPDATE`, never `INSERT OR REPLACE`], `get_call`). grep `swing/` for hardcoded enum copies — confirm only the migration CHECK + the model constants + the dataclass frozensets hold them. See fail → see pass.
- [ ] Commit: `feat(data): Task 0 — yfinance_calls schema 0030 + model + repo (atomic #11 enum sweep, v29->v30)`.

### Task 1 — `yfinance_audit` service + the `_record_yf_download` helper (`swing/data/yfinance_audit.py`)

- [ ] Tests (`tests/data/test_yfinance_audit_service.py`): `record_call_start` inserts in-flight + returns id; `record_call_finish` sets terminal fields; both REJECT a caller-held tx (`CallerHeldTransactionError`); `_AUDIT_WRITE_LOCK` serialized (smoke); `_sanitize_error` collapses whitespace + truncates to 200; **`_record_yf_download` unit tests** with a fake `fetch_fn` + a real on-disk DB + `_set_for_test` context: success (rows+timing+`success`), empty-df (`empty`, rows 0), raising `fetch_fn` (`error` + row closed + re-raises), NO context (no row + returns `fetch_fn()` result), and **audit-start-failure isolation** (a bogus `db_path` → `fetch_fn()` still returns; no raise).
- [ ] Implement mirroring `audit_service.py` (the `record_call_start`/`record_call_finish` halves, `_AUDIT_WRITE_LOCK`, `in_transaction` reject, `BEGIN IMMEDIATE`/COMMIT/ROLLBACK) + the `_record_yf_download` helper (§The shared instrumentation helper) + `_sanitize_error` + `_safe_close`/`_finish_safe`. Module docstring documents the deliberate light-redaction posture (no auth token → no `setLogRecordFactory`). NO `environment` param anywhere. NO sandbox gate.
- [ ] Commit: `feat(data): Task 1 — yfinance_audit service + _record_yf_download helper (reject caller-held tx; always-on; light redaction)`.

### Task 2 — the audit context global (`swing/data/yfinance_audit_context.py`)

- [ ] Tests (`tests/data/test_yfinance_audit_context.py`): set → get round-trips; clear → `None`; default (unset) → `None`; `_set_for_test`/`_reset_for_test` seams.
- [ ] Implement the lock-guarded global mirroring `log_correlation.py`. DB-free.
- [ ] Commit: `feat(data): Task 2 — yfinance audit context (lock-guarded process-global, mirrors log_correlation)`.

### Task 3 — instrument the two `ohlcv_archive.py` chokepoints (single + batch)

- [ ] Tests (`tests/data/test_ohlcv_archive_yfinance_audit.py` — the discriminators, §Testing): monkeypatch `yf.download`; under an active context assert single-success (rows/timing/`download_single`/`ticker` set), single-empty (`empty`), single-raise (`error` + closed + re-raises), batch one-row (`download_batch`, `ticker_count=N`, `ticker` NULL, `rows=len(raw)`), batch-raise (`error` row AND `_fetch_chunk` STILL returns `({}, list(chunk), True)` byte-identically), batch-empty (`empty`); under NO context no row + identical return; always-on under `environment='sandbox'`; audit-failure-does-not-break-fetch.
- [ ] Bracket the raw `yf.download` at `_yf_download_window:257` and `_fetch_chunk:503` via `_record_yf_download` (§Site-by-site bracketing). The `_fetch_chunk` `except Exception` keeps its existing fallback return; the helper's `error`-then-re-raise is caught there. See fail → see pass. grep: no enum drift.
- [ ] Commit: `feat(data): Task 3 — instrument ohlcv_archive single+batch yf.download chokepoints`.

### Task 4 — instrument labeling + price_cache; set the context at runner / CLI / web

- [ ] Tests:
  - `tests/patterns/test_labeling_bars_yfinance_audit.py`: under `surface='cli'` context, `autofetch_bars_for_labeling` records a `download_single` row; empty → `empty`; without context → no row + identical behavior.
  - extend price_cache tests: under a `surface='web'`/`'pipeline'` context, `_fetch_live_price` records a `download_intraday` row; empty `yf.download` → `status='empty'` + the existing `RuntimeError("no bars")` STILL raises to the caller (proves bracketing the raw call, not the method); all-NaN → `success` row (raw had rows) + the existing all-NaN `RuntimeError` STILL raises (the audit sees the raw frame, the NaN check is downstream).
  - `tests/pipeline/test_runner_yfinance_audit_context.py`: a run sets `surface='pipeline'` + real `pipeline_run_id` + `db_path`; CLEARS in the `finally` (None after).
  - a web-app test: app construction sets a persistent `surface='web'` context (assert via `get_yfinance_audit_context`).
- [ ] Bracket `_yf_download_window_for_labeling:46` + `_fetch_live_price`'s raw `yf.download:206` via `_record_yf_download`. Set the context: runner (after `set_pipeline_run_id`, `surface='pipeline'`, real run_id; clear in `finally`); CLI `pattern label` emit path (`surface='cli'`, run_id None; clear in `finally`); web app startup (`surface='web'`, run_id None; persistent — the long-lived server has no per-request boundary). See fail → see pass. grep: no enum drift.
- [ ] Commit: `feat(data): Task 4 — instrument labeling+price_cache yf.download; set audit context at runner/CLI/web`.
...
All in `tests/` (style-matched to siblings; ≤100-char lines, local imports). Chokepoint tests monkeypatch `yf.download` (never the network) + set a test context via `_set_for_test`.
...
| `test_audit_failure_does_not_break_fetch` | force `record_call_start` to raise (bogus `db_path`) → the fetch STILL returns its normal result | an impl letting the audit exception propagate sinks the fetch → fails; the isolated try/except passes |
| `test_always_on_records_under_sandbox` | `environment='sandbox'` + active context → a row IS recorded | a copied `_step_schwab_*` short-circuit records 0 → fails (LOCK 3) |
...
| `test_runner_sets_and_clears_context` (Task 4) | context active mid-run (real run_id, `surface='pipeline'`); `None` after `finally` | proves set + the `finally` clear (no stale context bleed across runs) |
| `test_labeling_records_download_single` / `test_web_app_sets_web_context` (Task 4) | full enumeration coverage (#27 no silent skip) | — |
...
- **Web context is persistent (process-lifetime), not per-request** — the web server is long-lived with no clean per-request run boundary; a once-set `surface='web'` context tags every web-triggered yfinance call. This audits the web paths (resolving the Codex R1 #27 contradiction) at the cost of no per-request run-id (`pipeline_run_id=NULL` for web rows — correct, web has no pipeline run). V2: a request-scoped context mirroring `SWING_WEB_REQUEST_ID`.
...
- **Per-chokepoint short-lived `connect(db_path)`** — the chokepoints can't share the pipeline's `audit_conn` (no threaded conn). Volume is ~10–15 calls/night (§Volume), so the per-call `connect()` is negligible; `_AUDIT_WRITE_LOCK` + `BEGIN IMMEDIATE` serialize writes across distinct connections (the `audit_service` docstring confirms cross-connection serialization: a single sqlite3 conn cannot run two concurrent tx, and `BEGIN IMMEDIATE` + the busy_timeout serialize across conns).
