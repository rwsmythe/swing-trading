...
- `@contextmanager yfinance_audit_scope(*, db_path, pipeline_run_id, surface)` — sets `_scope` on enter (capturing the prior `_scope` as a token), restores it in `finally` with **LIFO token validation** (a restore whose token ≠ the current `_scope` identity raises). Used by the pipeline runner (`surface='pipeline'`).
...
- Tests: web base → pipeline scope → `set_base(cli)` while the scope is active → `get()` stays pipeline until the scope exits, THEN `get()` is the new `cli` base; a second overlapping scope raises. (The internal LIFO token validation has no public driver — not separately asserted; Codex R9 NIT.)
...
# NO Python audit lock (Codex R10): each call uses its OWN short-lived connection,
# so there is no shared connection to serialize and no Python convoy to queue on;
# per-call BEGIN IMMEDIATE + busy_timeout_ms=100 give bounded SQLite serialization.
...
- **NO Python audit lock (Codex R9 CRITICAL convoy → R10 cleaner fix):** the yfinance audit uses NEITHER the schwab `_AUDIT_WRITE_LOCK` NOR a new one. The schwab lock exists to serialize a SHARED audit connection; yfinance uses a fresh short-lived connection PER call, so there is no shared connection to protect and NO Python lock to queue on (no convoy). Bounded serialization comes purely from per-call `BEGIN IMMEDIATE` + `busy_timeout_ms=100`. This avoids BOTH the R9 convoy (no Python lock to sum) AND the R10 normal-path drop (a non-blocking lock would skip audits among concurrent web misses even with SQLite free). Tests: (a) N concurrent active-context fetches with NO external writer → N rows recorded within deadline; (b) N concurrent fetches WITH an external SQLite writer → all fetches return within deadline (some audits may drop — best-effort).
...
| `test_concurrent_fetches_no_writer_record_all_rows` | N concurrent active-context fetches with NO external SQLite writer → all N audit rows recorded within deadline (no Python lock to drop them) | a non-blocking-Python-lock impl drops audits among concurrent web misses even with SQLite free → fails (Codex R10 MAJOR — always-on best-effort in the common path) |
