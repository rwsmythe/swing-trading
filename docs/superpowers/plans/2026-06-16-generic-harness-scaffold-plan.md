...
- **Direction-asymmetric `--to` resolution (§5.1, operator-decided 2026-06-16):** the POST path parses the `--to` target so that:
  - `--to charc` → the single director inbox `comms/charc/inbox` — **unchanged** (a director is singular; no ambiguity). `--to operator` → the operator's own single inbox `comms/operator/inbox` (also singular — unchanged from the swing lift; the operator UI reads/acks it, I.1). NEITHER charc nor operator is per-generation; only orchestrator is.
...
- **Orchestrator → director is SINGLE-INBOX / UNCHANGED.** `--to charc` delivers to the single director inbox `comms/charc/inbox`; `--to operator` delivers to the operator's own single inbox `comms/operator/inbox` (the swing-lift operator inbox the UI reads — I.1). Both are singular (NOT per-generation); only orchestrator is per-generation. (B.1 keeps `--to charc` / `--to operator` exactly as the lifted single-inbox posts.)
...
2. **Verify comms (PRE-step-5 — only what works WITHOUT a live orchestrator):** a CHARC-side `role_mail` round-trip — post (`--to charc`) → read → ack on the director inbox `comms/charc/inbox`, and a `--to operator` round-trip on the operator's own inbox `comms/operator/inbox` (both are single, role-owned inboxes — NOT per-generation; per B.1's resolver). NO orchestrator round-trip yet (no orchestrator is live until step 5). Verify the hook + the optional UI; **verify the registry/hook plumbing** (the `comms/sessions/` tree + the hooks load/exit-0 cleanly) — but NOT a live-orchestrator entry (none exists pre-step-5). The orchestrator round-trip, the per-generation inbox creation (`comms/orchestrator/<session_id>/inbox`), and the live registry-state check are deferred to the **POST-step-5 verification** (after the orchestrator is brought up — see step 5), consistent with the staged guarantee (orchestrator operation only exists after step 5).
...
5. **Bring up the orchestrator + the POST-step-5 verification** — launch an orchestrator session via `launch_role.ps1 -Role orchestrator` (sets `HARNESS_ROLE=orchestrator` → it registers + gets its own `comms/orchestrator/<session_id>/inbox`); the generation drains `comms/orchestrator/<session_id>/inbox`. THEN do the orchestrator-side comms verification that step 2 deferred (it needs a live orchestrator): the registry shows the generation live; its `comms/orchestrator/<session_id>/inbox` was created on register; a director→orchestrator round-trip works via `--to orchestrator:<session_id>` AND bare `--to orchestrator` (= newest-live). CHARC confirms this live state before commissioning the first arc.
...
**Acceptance check:** the bootstrap names all 5 checklist steps + the staged guarantee; the "verify comms" step is SPLIT into a PRE-step-5 phase (CHARC-side `charc`↔`operator` round-trip + the registry/hook plumbing — NO live-orchestrator dependency) and a POST-step-5 phase (the orchestrator round-trip + per-generation inbox creation + live registry-state, after step 5 brings the orchestrator up) — consistent with the staged guarantee (no step-2 check requires a live orchestrator); the interview fills seams 1-3 by name; the step-5 section CONTAINS the orchestrator bring-up prompt as a fenced block (H.3 content); **genericity guard passes** (no swing content; `HARNESS_ROLE` not `SWING_ROLE`).
...
**Content (§8):** post → read → ack across the three roles (charc/orchestrator/operator); the single-write-path + idempotency locks exercised end-to-end (an integration test over the real `role_mail` + a tmp comms root). The orchestrator leg uses the per-generation addressing (a seeded registry entry → `--to orchestrator:<session_id>` delivery + a drain of that generation's `comms/orchestrator/<session_id>/inbox`); charc + operator use their own single inboxes. This is the "comms works" half of the success criterion (§1).

**TDD (red→green):**
1. Failing test: operator→charc→ack (shared `comms/charc/inbox`), charc→orchestrator→ack (to a seeded generation's `comms/orchestrator/<session_id>/inbox` via `--to orchestrator:<session_id>`), orchestrator→operator→ack (the operator's own `comms/operator/inbox`) — all round-trip; the ack moves inbox→read; nothing deleted.

**Acceptance check:** all three role pairs round-trip (the orchestrator leg per-generation); guard passes.
