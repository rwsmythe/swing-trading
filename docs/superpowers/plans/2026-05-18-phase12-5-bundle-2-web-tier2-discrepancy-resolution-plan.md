...
**Architecture:** Three layers, single sub-bundle. (1) View-model + builder + parametric-pick helper in NEW `swing/web/view_models/reconcile.py` (read-only builder module: NO writes; NO transaction management; the builder reads via the caller-supplied `conn` per spec §5.4 — Codex R4 m#4 phrasing clarification: "pure" reserved for the `_parse_parametric_pick_count` helper + per-discrepancy-type render helpers that take no `conn`; "read-only builder" describes the `build_reconcile_discrepancy_resolve_vm` function that takes `conn` for reads). (2) Route handlers in NEW `swing/web/routes/reconcile.py` (GET form-render + POST form-parse → `apply_tier2_resolution` → 204 + HX-Redirect; full error catch-ladder). (3) `BaseLayoutVM.banner_resolve_link: str | None = None` retrofit across `BaseLayoutVM` AND the 13 standalone-field base-layout VMs (Pass A field-declaration grep + Pass B builder/route population grep); new helpers `list_pending_ambiguities_in_banner_set` + `fetch_first_pending_ambiguity_resolve_link_path` in `swing/metrics/discrepancies.py`; `base.html.j2` wraps the banner count in an `<a>` link when populated.
...
- **Template-side branches added IN T-2.6 (Codex R2 M#1 fix):** the route handler invokes `error_kind in {'anchor_mismatch', 'service_error', 'db_unavailable'}` on its 400/409/500/503 paths. T-2.5 stubbed only `not_found` + `already_resolved`; T-2.6 EXTENDS `reconcile_discrepancy_resolve_error.html.j2` with the 3 remaining branches at the SAME commit as the route handler so T-2.6 ships green standalone. T-2.10 then polishes (styling refinements + cycle-checklist mentions + dedicated template-branch tests). Acceptance: every `_render_error(error_kind=X, ...)` call site in T-2.6 has a corresponding `{% if vm.error_kind == 'X' %}...{% endif %}` block in the template after T-2.6 lands.
- Handler flow per spec §4.2 verbatim:
  ...
  4. Pre-flight checks (read-only on `conn`; route does NOT open a transaction):
     ...
     h. No-match branch: `choice_code` neither in `static_codes` nor a valid parametric pick → 400 + re-render with `valid_choices` echoed. When `disc.ambiguity_kind == 'multi_match_within_window'`, `valid_choices = sorted(static_codes) + [f'pick_schwab_record_{i+1}' for i in range(_parse_parametric_pick_count(disc.resolution_reason))]` — INCLUDES the full operator-visible menu (Codex R1 m#3 + R2 m#1 fix). Order asymmetry vs the GET form (which prepends parametric per T-2.3 step 5): error message appends. Both surface the complete set of valid codes; order is presentational only (banked V2.1 §VII.F amendment J3). When ambiguity_kind is NOT multi_match_within_window, `valid_choices = sorted(static_codes)` only.
...
- `test_post_value_error_concurrent_race_returns_409_not_400` — NEW (Codex R1 M#2 + R2 m#2 acceptance pin): the mock-style fixture **mutates via a SEPARATE sqlite3 connection** (NOT the same connection the route handler is using) + **commits the UPDATE explicitly** before the patched `apply_tier2_resolution` raises `ValueError("discrepancy is no longer pending")` — this simulates the actual concurrent-writer race semantics (one connection commits the terminal state; the route's re-read picks it up). Alternative-equivalent: use a two-`TestClient`-thread fixture where thread B issues `swing journal discrepancy resolve-ambiguity` via `CliRunner` (committing on its own connection) between thread A's pre-flight check and the service call. Submit POST; assert response is **409 + error template** (`error_kind='already_resolved'`; NOT 400 + re-render). Asserts the re-read disambiguation logic in branch 14b is wired correctly + that the disambiguation reads through a non-stale view of the discrepancy.
...
### Task T-2.10 — Error path template POLISH + dedicated branch coverage tests
...
- Modify: `swing/web/view_models/reconcile.py` — `ReconcileDiscrepancyErrorVM` already defined at T-2.5 + consumed by T-2.5 + T-2.6 with full field shape. T-2.10 makes NO signature changes + NO validator additions (Codex R3 M#1 fix: late `__post_init__` tightening would risk breaking T-2.5/T-2.6 already-green paths whose call sites do not contract for a non-empty `error_message` on every error_kind). T-2.10 VM-side scope is docstring expansion only (Codex R4 m#1 fix: deferred error_kind-constants story — the constants would only prevent string-typo drift if every route call site consumed them, which would require T-2.6 to import + reference them; landing constants WITHOUT making call sites consume them is decorative).
...
**Commit message stem:** `polish(web): reconcile error-template a11y + dataclass docstring + per-branch test coverage (Phase 12.5 #2 T-2.10)`
...
| F4 | **HX-Request propagation on embedded form.** `hx-headers='{"HX-Request": "true"}'` on the resolve form (Phase 5 R1 M1 LOCK). | CLAUDE.md HTMX trinity | T-2.4 `test_resolve_template_renders_hx_headers_attribute_semantically` (parses HTML attribute + `json.loads` equality) + complement `test_resolve_template_renders_hx_headers_attribute_present` (substring presence). |
...
| J2 | Spec §4.2 step 5 + §8.1 14th row + §8.2 race semantics state that POST-service `ValueError` maps uniformly to "400 + re-render". Plan T-2.6 step 6 (Codex R1 M#2 fix; ratified R2) SPLITS this disposition: catch + re-read; concurrent-resolve race (`disc.resolution != 'pending_ambiguity_resolution'`) → 409 + error template; handler payload-shape rejection → 400 + re-render. The split closes a real failure mode (re-rendering through the builder whose precondition is `resolution='pending_ambiguity_resolution'` would otherwise loop the operator through an internal-error state). | Codex R1 M#2 + R2 M#2 | Banked for V2.1 §VII.F amendment; spec §4.2 step 5 + §8.1 14th row should be amended to enumerate the 14a (400) + 14b (409) split. **Plan supersedes spec here**: T-2.6 step 6 is the binding contract for executing-plans. |
| J3 | Spec §4.2 step 3f no-match branch describes `valid_choices` echo without explicit treatment of parametric choices on `multi_match_within_window`. Plan T-2.6 step 3h (Codex R1 m#3 fix; ratified R2) constructs `valid_choices = sorted(static_codes) + [f'pick_schwab_record_{i+1}' for i in range(N)]` when ambiguity_kind == 'multi_match_within_window'. | Codex R1 m#3 + R2 m#1 | Banked for V2.1 §VII.F amendment; spec §4.2 step 3f should be amended to enumerate the parametric inclusion. **Plan supersedes spec here**: T-2.6 step 3h is the binding contract for executing-plans. Order asymmetry: GET menu prepends parametric (per spec §5.4 + plan T-2.3 step 5); error message appends — functional equivalence (both surface the full menu), order difference is presentational. |
