...
def run_v2_sweep(
    ...
    *,
    ...
    total_candidates=0, universe_size=0, entries=(), flipped=(),
...
- (b) Side-effect contract (Codex R2.M3 + R3.M2 RESOLVED):
  ...
  - The tracked `swing.config.toml` explicitly DROPS sensitive fields at TWO ranges (Codex R4.m1 RESOLVED — initial citation was incomplete): `swing/config.py:466-467` (`raw_finviz.pop("token", None)`; `raw_finviz.pop("screen_query", None)`) AND `swing/config.py:480-481` (Schwab sensitive `client_id`/`client_secret`/`refresh_token` drop). Operator-secret access (Finviz / Schwab API tokens stored in `user-config.toml`) requires the separate `config_overrides.py` cascade which `from_defaults` does NOT invoke.
