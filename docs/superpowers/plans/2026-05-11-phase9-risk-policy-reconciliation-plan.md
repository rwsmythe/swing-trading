...
### Task A.5: cfg-mirror cascade + Phase 5 config page integration

**Files:**
- Modify: `swing/config.py` (extend `load_config` with startup divergence check)
...
- Test: `tests/config/test_config_toml_divergence_startup.py`

**Depends on:** T-A.4.

**Acceptance criteria:**
- When Phase 5 config-page POST changes `cfg.account.risk_equity_floor`, ALSO call `swing/trades/risk_policy.py:supersede_active_policy(conn, field_updates={"capital_floor_constant_dollars": new_value}, source="cfg_cascade", ...)`. A new policy_id row is created with the updated capital_floor.
- Other cfg fields (chase_factor, chart_top_n_watch) DO NOT cascade.
- At startup, `swing/config.py:load_config` invokes `apply_toml_divergence_correction(conn, cfg)`; logs WARNING; surfaces divergence to stderr banner.
- Discriminating tests: divergent TOML at startup → in-memory cfg corrected, TOML file unchanged.
- HTMX form lessons (Phase 5 R1 M1+M2): HX-Request propagation on embedded form + HX-Redirect success-path response are PRESERVED (no regressions).
...
- [ ] **Step 1: Write failing test for cfg-mirror cascade**
- [ ] **Step 2: Run + verify fail**
- [ ] **Step 3: Implement route + config.py extension**
- [ ] **Step 4: Verify HTMX surfaces still pass (HX-Request + HX-Redirect)**
- [ ] **Step 5: Verify TOML file not written when applying divergence correction**
- [ ] **Step 6: Run + verify pass**
- [ ] **Step 7: Commit**

```bash
git add swing/config.py swing/web/routes/config.py tests/web/test_config_route_risk_policy_cascade.py tests/config/test_config_toml_divergence_startup.py
git commit -m "feat(web): Task A.5 — Phase 5 config page cascades risk_equity_floor edit to risk_policy; startup divergence warning"
```
