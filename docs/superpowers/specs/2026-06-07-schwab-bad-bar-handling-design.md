...
2. **Every dropped date has a clean alternate row** → build the returned in-memory series by **splicing those clean rows into the Schwab good bars**, then **normalize to the legacy/`to_dataframe` shape** (Codex R3 MAJOR-A): a `DatetimeIndex` named `Date`, capitalized `Open/High/Low/Close/Volume`, sorted ascending — the exact shape `OhlcvCache` consumes (`web/ohlcv_cache.py:302` calls `bars.index[-1].date()`; SMA/ADR helpers expect capitalized columns). `resolve_ohlcv_window` may be used ONLY as a **row source** (it returns RangeIndex + lowercase `asof_date` cols, `ohlcv_archive.py:426-429,474-475`); a raw `resolve_ohlcv_window` frame would mis-shape the hot path and **must not** be returned as-is. Return the normalized **contiguous** DataFrame (the hook passes a DataFrame through via its `else` branch, `runner.py:476-480` / `web/app.py:343-347`; `OhlcvCache` reads tuple slot 0, `web/ohlcv_cache.py:280`). **Result: Schwab primary, zero gap, on ALL three substrates** — substrate 1 gets the normalized spliced frame, substrates 2/3 read the persisted archives.
   - **Provider-tag semantics (Codex R3 MINOR, documented):** the spliced frame is tagged `schwab_api` as the **ladder disposition/primary provider, NOT a per-row provenance claim** (it contains ≥1 yfinance-sourced row). No hot-path consumer asserts per-row Schwab provenance (`OhlcvBundle.provider` is whole-fetch level, `web/ohlcv_cache.py:85-109`); per-row provenance lives only in the archive/`resolve_ohlcv_window` path (substrate 2, `runner.py:2621-2642`), which is unaffected.
...
**Opt-in confinement (Codex R3 MAJOR-B — no dropped-bar window may escape the guard):** the per-bar drop isolation is **OPT-IN, ladder-only.** `map_price_history_to_window` / `get_price_history` keep their **default `raise`-on-bad-bar contract** (so the direct production CLI caller `swing/cli_schwab.py:1153-1189` `fetch --verify-marketdata`, which consumes the window directly and does NOT go through the ladder coverage guard, is unchanged — a bad bar still surfaces as `SchwabBarConsistencyError`). Only `fetch_window_via_ladder` passes the opt-in flag (e.g. `isolate_bad_bars=True`) that switches the mapper to drop-and-collect; the coverage guard (steps 1-3) then runs exclusively where it is wired. This confines the new behavior to exactly the path that owns the guarantee.
...
1. **Mapper carries the drop metadata (OPT-IN; default unchanged — Codex R3 MAJOR-B).** Under the ladder's opt-in flag (§4.2 `isolate_bad_bars`), `map_price_history_to_window` skips the bad candle(s) (`continue` instead of `raise`), accumulating a structured record per drop with the **full, untruncated** context the mapper already holds: `ticker` (the arg), `asof_date`, `open/high/low/close`, the violated bound, and the gap magnitude (`|value - bound|`). Carry these out via a new metadata field on the returned `SchwabPriceHistoryWindow` (e.g. `dropped_bars: tuple[DroppedBar, ...]`, default empty). **WITHOUT the flag (the default), the mapper still raises `SchwabBarConsistencyError`** — so the direct CLI caller `get_price_history` (`cli_schwab.py:1153-1189` `fetch --verify-marketdata`) keeps today's contract. This adds **metadata to the window dataclass** — it does NOT touch / relax `OhlcvBar`. (Design note / OQ-F: window-field vs a mapper return-tuple; the field is preferred because it threads cleanly through the existing `_mapper`/`finish_hook`/ladder return shapes.)
...
- Data-integrity arc (typed `SchwabBarConsistencyError`, float-normalization) intact; `SchwabBarConsistencyError` retained as the **default (non-opt-in) mapper behavior** (preserving the direct CLI/`get_price_history` contract) AND for the ladder's all-bad / threshold-exceeded whole-window fallback (§4.3). The drop-isolation is opt-in, ladder-only (§4.2/§5.1).
...
2. Mapper per-bar isolation, **OPT-IN** (`isolate_bad_bars` flag threaded `get_price_history` → `_mapper`; ladder passes True, all other callers default False): under the flag, replace the `raise SchwabBarConsistencyError` (mappers.py:868-872) with a per-bar skip that records a `DroppedBar`; raise 204 if **all** bars drop; raise (→ whole-window fallback) if the OQ-E predicate (`dropped>10 OR dropped/total>0.05`) trips. (TDD: **default (no flag) → still raises `SchwabBarConsistencyError` on the first bad bar [CLI contract]**; flag + 1-bad + N-good fixture → N bars + 1 `dropped_bars` entry; flag + all-bad → 204; flag + over-threshold → raises. Compute the threshold both sides of the boundary per `feedback_regression_test_arithmetic`.)
3. **Ladder coverage-guard returning a NORMALIZED CONTIGUOUS in-memory series (the §4.2 safety core):** in `fetch_window_via_ladder` (passing `isolate_bad_bars=True`), after a Schwab window with `dropped_bars`: persist the Schwab good bars; obtain the clean alternate row per dropped date; all covered → splice + **normalize to the `to_dataframe` shape (DatetimeIndex `Date`, capitalized OHLCV, sorted) and return THAT DataFrame** (NOT a raw `resolve_ohlcv_window` frame [wrong index/casing] and NOT the raw gapped `window.to_dataframe()`); any uncovered → existing whole-window yfinance fallback. (TDD: covered → returned frame has the dropped date present from yfinance **AND matches the exact legacy shape**; uncovered → whole-window fallback; **swallowed-Schwab-persist + covered → returned frame still contiguous**.)
