...
2. **Discriminating cache-collision test:** plant a fixture eval-run + ticker + invoke `get_or_render_surface` from both hyp-rec route AND watchlist-expanded route with the SAME (ticker, pipeline_run_id); assert (a) cache contains exactly ONE row with byte-equivalent SVG; (b) second caller reads cache hit (no second render).
