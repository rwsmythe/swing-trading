...
**Per Codex R3 M#3 + R4 M#3 closure — `hyprec_detail` surface NAME LOCK + RENDERER-KWARGS LOCK:** with this expansion, `hyprec_detail` is now CANONICAL for "full-ticker detail chart with MA + volume" rendered in ANY UI surface (hyp-recs expansion + watchlist expansion + future detail surfaces). The historical naming (recommendation-specific) is misleading-but-grandfathered; the V1 LOCK preserves the existing enum value to avoid a v22 schema bump.

**Renderer-kwargs CACHE-COLLISION LOCK** (per Codex R4 M#3): the chart_renders cache key for run-bound non-theme2 surfaces is `(ticker, surface, pipeline_run_id)` per the v20 partial-UNIQUE indexes. A hyp-recs caller and a watchlist-expanded caller invoking `get_or_render_surface(surface='hyprec_detail', ticker=T, pipeline_run_id=R)` MUST produce BYTE-EQUIVALENT renders — otherwise the second caller's render replaces the first caller's cache entry without notice, leading to flapping visuals on dashboard re-renders. Two LOCKs prevent this:

1. **Renderer-kwargs uniformity:** both callsites pass the SAME minimal kwargs to `render_hyprec_detail_svg`. The current render at `swing/web/charts.py:render_hyprec_detail_svg(*, ticker, bars, pattern_evaluation)` (line 282 + verified at `swing/pipeline/runner.py:2378-2380`) passes `pattern_evaluation=None` from the pipeline-time write-through. For V1, BOTH JIT callsites also pass `pattern_evaluation=None` — pattern_evaluation overlay is reserved for the gold-tier pattern-review surface which doesn't go through this JIT helper. Any future caller that wants `pattern_evaluation` SHOULD use a DIFFERENT surface enum value (V2 `pattern_review_detail` surface; banked).
2. **Discriminating cache-collision test (per Codex R5 MIN#1 closure — avoid byte-stream-comparison fragility):** plant a fixture eval-run + ticker; invoke `get_or_render_surface` from the FIRST caller (e.g., hyp-rec route) → assert cache populated with one row; invoke from the SECOND caller (watchlist-expanded route) with the SAME (ticker, pipeline_run_id) → assert (a) second invocation returns the EXACT cached bytes from the FIRST caller (read-comparison, NOT independent re-render comparison); (b) cache still contains exactly ONE row (no row replacement); (c) second invocation did NOT call the matplotlib renderer (mock-the-renderer + assert call_count == 1 across both invocations). This pattern avoids the matplotlib-byte-stability concern: the test does not require renderer determinism, only that the second caller is a cache HIT.

A rename to e.g. `ticker_detail` would require migration of all existing rows + chart_renders CHECK constraint widening — banked as V2 candidate (low priority; cosmetic). Until the V2 rename, the chart_renders audit + retention tooling (Item 5 OQ-5.1) treats `hyprec_detail` rows uniformly regardless of which UI surface populated them; pipeline_run_id is the join-key that lets operators trace provenance.
...
**SQL implementation** (for `swing/metrics/cohort.py:list_trades_for_cohort` rewrite): replace `WHERE hypothesis_label = ?` with `WHERE LOWER(hypothesis_label) = LOWER(?) OR LOWER(hypothesis_label) LIKE LOWER(?) || ' %' ESCAPE '\' OR LOWER(hypothesis_label) LIKE LOWER(?) || ';%' ESCAPE '\'` (3 parameters).

**Binding parameter LOCK (per Codex R4 M#2 closure)**: the helper returns 3 BINDING PARAMETERS, NOT a single name passed thrice. Param 1 (exact-equality predicate) receives the RAW lowercased cohort name (no wildcard escaping; equality compares verbatim). Param 2 + Param 3 (LIKE predicates) receive the WILDCARD-ESCAPED lowercased cohort name (so a registry entry containing `_` or `%` does not silently broaden the LIKE match). Mixing the two would either (a) over-escape the equality predicate (registry name `foo_bar` could never match a persisted label `foo_bar` because `=` compares verbatim + escape sequences differ), or (b) under-escape the LIKE predicates (registry name `foo_bar` matches `foozbar (watch); ...` because `_` is a single-char wildcard).

Helper signature LOCK: `_label_matches_hypothesis_sql(name: str) -> tuple[str, list[object]]` returns the WHERE-clause-fragment + a 3-element binding list `[raw_lowercased_name, escaped_lowercased_name, escaped_lowercased_name]`.

**Registered-hypothesis-name non-overlap invariant (per Codex R5 MIN#2 closure):** when two hypothesis registry names share a delimiter-aware prefix relationship (e.g., `'A+ baseline'` AND `'A+ baseline extended'`), a single trade with `hypothesis_label = 'A+ baseline extended (watch); ...'` would delimiter-match BOTH (`A+ baseline ` is a prefix of `A+ baseline extended (watch); ...` AND `A+ baseline extended ` is also a prefix). The dashboard hyp-progress card would double-count that trade across both cohorts. Current registry has 4 entries with no overlap; the invariant must be PRESERVED in the future. Discriminating test: for every PAIR of registered hypothesis names, assert `_label_matches_hypothesis(name_b, name_a) == False AND _label_matches_hypothesis(name_a, name_b) == False` (no pair matches the other's delimiter-aware contract). Plants as a regression anchor; fires when a future migration adds a prefix-overlapping hypothesis name to the registry. Banked as a permanent invariant test in T-T4.SB.2.

A `_sql_escape_wildcard(name: str) -> str` helper replaces `\` → `\\`, `%` → `\%`, `_` → `\_` before binding. Discriminating test plants a synthetic registry name `'cohort_X%'` + persisted labels `'cohort_X%'` (exact) + `'cohort_X% (watch); failed: x'` (delimiter-match) + `'cohortQX9'` (would-be wildcard match without escape) + asserts only the first two match.
...
- `_label_matches_hypothesis_sql(name) -> tuple[str, list[object]]` returns the WHERE-fragment + 3-element binding params per the LOCK above.
- Discriminating test asserts Python helper + SQL helper produce IDENTICAL match sets on a fixed corpus of test labels (including the `_` / `%` wildcard-edge-case labels).
...
- `swing/metrics/cohort.py:count_per_cohort` at lines 99-119 — also uses exact-grouping (`_CLOSED_TRADE_COUNT_PER_LABEL_SQL` GROUP BY `hypothesis_label`); under suffix-bearing labels, every unique suffix becomes its own orphan cohort. Fix via the delimiter-aware match: rewrite to (a) iterate registered hypotheses + per-hypothesis count via the SAME SQL helper (3-rule delimiter-aware predicate); (b) **PRESERVE orphan-label behavior** (per Codex R4 M#1 closure) via a SECOND query that selects closed trades with `hypothesis_label NOT NULL` + does NOT delimiter-match ANY registered hypothesis, grouped by raw label. Orphan rows are returned with their RAW label as the cohort_name; the existing "unregistered cohort placeholder" semantic at `count_per_cohort:113-118` is preserved. Discriminating test plants 3 trade rows (1 matching `Sub-A+ VCP-not-formed` cohort via suffix + 1 matching `A+ baseline` exact + 1 orphan `"Free-text experimental"`); asserts 2 registered cohorts populated correctly AND orphan returned with raw label. Cross-bundle pin row 13 PARAMETRIZED OVER `count_per_cohort` IN ADDITION TO `list_trades_for_cohort` + dashboard hyp-progress card + CLI hyp-progress breakdown. (Pin parametrize set: 4 surfaces total.)
