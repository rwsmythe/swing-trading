...
- **POST /trades/entry validation 4-tier rejection ladder** + 1 extra tier (per Codex R4 MAJOR #2): use the same `_reject_anchor` helper pattern at `swing/web/routes/trades.py:896-911` (T3.SB1 precedent for Schwab `schwab_source_value_json` anchor; mirror EXACTLY): (a) malformed integer → 400 + clear pattern_evaluation_id anchor on recovery; (b) `pattern_evaluations` row not found by id → 400 + clear; (c) row found but `ticker` differs from operator-submitted ticker → 400 + clear; (d) row found but its `pipeline_run_id` differs from the entry's pipeline-context anchor → 400 + clear; **(e) ANCHOR PRESENT BUT ORIGIN MISMATCH** (Codex R4 MAJOR #2): if the submitted `origin` is `manual_off_pipeline` AND `pattern_evaluation_id` anchor is present → 400 + clear (closes the forge-with-valid-matching-anchor-on-manual-origin gap).
- **`claimed_pattern_evaluation_anchor` consistency-check gate** (per Codex R4 MAJOR #3 — mirrors `claimed_auto_fill` at trades.py:876-878 for Schwab anchor): NEW hidden form field `claimed_pattern_evaluation_anchor` (boolean string `"true"` / `"false"`) that the form-render code emits set to `"true"` when an anchor IS rendered. POST-handler consistency-check: (i) `claimed_pattern_evaluation_anchor == "true"` AND `pattern_evaluation_id` anchor MISSING → 400 + clear (claim without anchor); (ii) `claimed_pattern_evaluation_anchor == "false"` AND `pattern_evaluation_id` anchor PRESENT → 400 + clear (anchor without claim); (iii) origin = `manual_off_pipeline` AND `claimed_pattern_evaluation_anchor == "true"` → 400 + clear (manual origin contradicts claim).
...
- Modify (optional): `swing/web/routes/recommendations.py` — pass `pattern_evaluation_id` context into the entry-form GET link (as query param OR session-state hint) so the form-render at `trades.py:398` can read it. May be unnecessary if the hyp-rec → entry-form transition already carries equivalent context; verify at writing-plans phase.
...
**Acceptance criteria**:
- 25 new tests PASS (15 closure + 10 anchor-threading-validation per R4 expansion).
...
- `test_entry_post_rejects_manual_off_pipeline_origin_with_pattern_evaluation_id_anchor_400_clears` — tier (e); forged POST with valid+matching anchor on manual_off_pipeline must NOT stamp + must reject (per Codex R4 MAJOR #2).
