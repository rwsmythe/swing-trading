...
| **OQ-12** *(NEW)* trades.pattern_evaluation_id lifecycle population point | §2.2 lifecycle disposition (REVISED per Codex R1 + R2 MAJOR #4) | **NULL by default**; populated only when entry form threads through an explicit `pattern_evaluation_id` hidden anchor (operator-intent provenance). Earlier "highest-composite" + Phase 6 chart_pattern_algo coupling proposals REJECTED — Phase 6 enum is `'none'|'flag'` disjoint from Phase 13 detector enum; highest-composite is arbitrary and contaminates per-pattern-class cohort analysis. V1 conservative; V2 dispatch can extend entry form to thread anchor through. | _pending_ |
