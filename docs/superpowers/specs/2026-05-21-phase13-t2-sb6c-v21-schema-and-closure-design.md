...
- **POST /trades/entry validation 5-tier rejection ladder** (REVISED per Codex R5 MAJOR #1): use the same `_reject_anchor` helper pattern at `swing/web/routes/trades.py:896-911` (T3.SB1 precedent for Schwab `schwab_source_value_json` anchor; mirror EXACTLY): (a) malformed integer → 400 + clear pattern_evaluation_id anchor on recovery; (b) `pattern_evaluations` row not found by id → 400 + clear; (c) row found but `ticker` differs from operator-submitted ticker → 400 + clear; (d) row found but its `pipeline_run_id` differs from the entry's pipeline-context anchor → 400 + clear; **(e) ANCHOR PRESENT BUT SERVER-DERIVED `trade_origin` IS `manual_off_pipeline`** — validation MUST check the SERVER-DERIVED `trade_origin` from `swing/trades/origin.py:resolve_trade_origin` (NOT the form's UI `origin` field which is `'watchlist'`/`'hyp-recs'`/etc.; UI origin is distinct from `trade_origin`); if `resolve_trade_origin(...)` resolves to `'manual_off_pipeline'` AND `pattern_evaluation_id` anchor is present → 400 + clear. The trade_origin resolver consumes the form's UI origin + ticker + candidate-context as inputs and emits the canonical `trade_origin` enum value.
...
**Scope (REVISED per Codex R3 MAJOR #2/#3 + R4 MAJOR #2/#3 + R5 MAJOR #1/#2/#3)**: 3 SB6 closure items (B.3 label_source split + B.4 outcome distribution + B.5 metric tile) + entry-form anchor threading + 5-tier rejection validation + claim-consistency gate for `pattern_evaluation_id`; ~26 fast tests (15 closure + 11 anchor-threading-validation tests including R5 missing-claim test).
...
**Acceptance criteria**:
- 26 new tests PASS (15 closure + 11 anchor-threading-validation per R5 expansion adding the missing-claim coercion test).
