...
**Anchor-threading scope at T-A.6c.4** (REVISED per Codex R4 MAJOR #1 + #2 + #3):
- **Canonical entry route is `swing/web/routes/trades.py`** (NOT recommendations.py — recommendations.py expands/refreshes hyp-recs but does NOT own `record_entry`). The trade entry surface lives at `GET /trades/entry/form` (`swing/web/routes/trades.py:398`) + `POST /trades/entry` (`swing/web/routes/trades.py:413`); the entry form template is `swing/web/templates/partials/trade_entry_form.html.j2`.
- **GET /trades/entry/form** extends with hidden input `pattern_evaluation_id` (in the form template) populated at form-render time when the entry originates from a `pipeline_aplus` / `pipeline_watch_hyp_recs` / `pipeline_watch_manual` flow with a specific `pattern_evaluations` row visible to the operator. The `recommendations.py` route handler may need a small change to pass the `pattern_evaluation_id` context into the entry-form GET link as a query param OR session-state hint; the form-render code in `trades.py` reads it and emits the hidden input.
- **POST /trades/entry validation 5-tier rejection ladder** (REVISED per Codex R5 MAJOR #1 + R6 MAJOR #1/#2/#3): use the same `_reject_anchor` helper pattern at `swing/web/routes/trades.py:896-911` (T3.SB1 precedent for Schwab `schwab_source_value_json` anchor; mirror EXACTLY).

  **Pipeline-context anchor source (per R6 MAJOR #3)**: add NEW hidden form field `pipeline_run_id_at_form_render` that the form-render code emits when a `pattern_evaluation_id` anchor is rendered, set to the `pattern_evaluations.pipeline_run_id` value at render time. Tier (d) compares this submitted form field against the looked-up `pattern_evaluations.pipeline_run_id` from tier (b)'s row fetch.

  **Entry-path mapping fix (per R6 MAJOR #2)**: the existing POST handler at `swing/web/routes/trades.py:1095` hardcodes `entry_path=EntryPath.MANUAL_WEB_FORM` for ALL web entries. T-A.6c.4 MUST extend that line so UI `origin='hyp-recs'` → `EntryPath.HYP_RECS_BUTTON`; UI `origin='watchlist'` or manual → `EntryPath.MANUAL_WEB_FORM`. Without this fix, `derive_trade_origin` cannot distinguish `pipeline_watch_hyp_recs` from `pipeline_watch_manual`; tier (e) coverage would be weak.

  **Tiers**: (a) malformed integer on `pattern_evaluation_id` → 400 + clear; (b) `pattern_evaluations` row not found by id → 400 + clear; (c) row found but `ticker` differs from operator-submitted ticker → 400 + clear; (d) submitted `pipeline_run_id_at_form_render` differs from the row's `pipeline_run_id` → 400 + clear (and tier (d) ALSO rejects if `pipeline_run_id_at_form_render` form field is MISSING while `pattern_evaluation_id` is PRESENT — missing-anchor symmetry with the claim field's R5 fix); **(e) ANCHOR PRESENT BUT SERVER-DERIVED `trade_origin` IS `manual_off_pipeline`** (per R6 MAJOR #1 — `swing/trades/origin.py:derive_trade_origin` is the canonical resolver function name at line 52; consumes `(conn, ticker, entry_path: EntryPath)` per line 52-54; UI origin is distinct from `trade_origin`): after the entry-path mapping fix above, if `derive_trade_origin(conn, ticker, mapped_entry_path)` resolves to `'manual_off_pipeline'` AND `pattern_evaluation_id` anchor is present → 400 + clear.
- **`claimed_pattern_evaluation_anchor` consistency-check gate** (per Codex R4 MAJOR #3 + R5 MAJOR #3 — mirrors `claimed_auto_fill` at trades.py:876-878 for Schwab anchor): NEW hidden form field `claimed_pattern_evaluation_anchor` (boolean string `"true"` / `"false"`) that the form-render code emits set to `"true"` when an anchor IS rendered. POST-handler consistency-check (REVISED per R5 MAJOR #3 missing-value semantics): **missing form field coerces to `"false"`** (default-safe; tampered POSTs that omit the field cannot bypass validation by simply omitting it). After coercion: (i) `claimed_pattern_evaluation_anchor == "true"` AND `pattern_evaluation_id` anchor MISSING → 400 + clear (claim without anchor); (ii) `claimed_pattern_evaluation_anchor == "false"` (including omitted) AND `pattern_evaluation_id` anchor PRESENT → 400 + clear (anchor without claim); (iii) server-derived `trade_origin == 'manual_off_pipeline'` AND `claimed_pattern_evaluation_anchor == "true"` → 400 + clear (manual origin contradicts claim).
- **Manual-off-pipeline origin**: form does NOT emit the anchor input nor the claim flag; `pattern_evaluation_id` persists NULL via the default-None path.
...
**Paired write-path INSERT extension (REVISED per Codex R2 MAJOR #1)**: SAME schema-version-aware runtime branch shape as §2.1. Both new columns (`candidate_id` + `pattern_evaluation_id`) are added to the same `INSERT INTO trades` SQL string; the runtime branch via `PRAGMA table_info('trades')` MUST detect BOTH new columns (extended INSERT used only when BOTH `candidate_id` AND `pattern_evaluation_id` are present in the schema) — else fall back to legacy column list. Discriminating tests cover both nullable fields' roundtrip on v20-only and v21-migrated connections.
...
**Backup-gate discriminating tests** (added to T-A.6c.1 acceptance; REVISED per Codex R2 MAJOR #3):
...
- `test_run_migrations_v20_to_v21_strict_equality_pre_version_predicate` — assert gate fires when `pre_version == 20`; does NOT fire when `pre_version == 19` (consistent with existing gate behavior at `swing/data/db.py:575-581 + :643-654` — gates skip non-matching starts; they do NOT abort the migration sweep).
- `test_run_migrations_v19_to_v21_skips_sb6c_backup_uses_phase13_v20_backup_only` — multi-version jump v19→v21 applies BOTH v20 AND v21 migrations; the SB6c-specific backup file is NOT written because `pre_version == 19 != 20` at the SB6c gate; the v20 boundary's own `swing-pre-phase13-migration-<ISO>.db` backup IS written (PHASE13 gate fires at `pre_version == 19`). This is INTENTIONAL per existing gate semantics — operator who jumps v19→v21 directly accepts that no SB6c-specific backup separates the two boundaries; the v20 backup contains the pre-v20 state which is sufficient for rollback. Forward-binding lesson: cross-version-jump backup discipline is the responsibility of the OPERATOR (run migrations in single-version steps if separate backups are desired). The prior R1 spec draft claimed v19→v21 "would FAIL" — INCORRECT per Codex R2 audit of existing `run_migrations` behavior. The strict-equality discipline gates the BACKUP from firing on a wrong-version-pair, NOT the migration itself.
- `test_expected_schema_version_constant_is_21_post_sb6c` — asserts constant bump.

**Forward-binding lesson (V2 candidate banked)**: if cross-version backup safety becomes operator-binding, future migration runner enhancements could add a `--enforce-stepwise` flag to `swing db-migrate` that REFUSES multi-version jumps; current V1 accepts them.
...
5. **Write-path INSERT extension (REVISED per Codex R2 MAJOR #1)**: `INSERT INTO trades` at `swing/data/repos/trades.py:120` extended with BOTH `candidate_id` AND `pattern_evaluation_id` columns + placeholders; function signature accepting BOTH `candidate_id: int | None = None` AND `pattern_evaluation_id: int | None = None` kwargs. Runtime branch via `PRAGMA table_info('trades')` (T3.SB1 fills.py:51-53 precedent) detects whether BOTH new columns exist; extended INSERT only used when BOTH present, else legacy column list. The same atomic landing commit covers BOTH Delta A AND Delta B INSERT shapes together.
...
**Anchor-threading discriminating tests (per Codex R3 MAJOR #1 + R4 MAJOR #2/#3 server-validation requirement)**:
- `test_entry_form_renders_pattern_evaluation_id_hidden_anchor_for_pipeline_origin` — assert anchor + `claimed_pattern_evaluation_anchor="true"` present on entry form for pipeline origins.
- `test_entry_form_omits_pattern_evaluation_id_hidden_anchor_for_manual_off_pipeline` — assert anchor absent + `claimed_pattern_evaluation_anchor="false"` (or omitted) for manual_off_pipeline.
...
- `test_entry_post_rejects_pattern_evaluation_id_pipeline_run_mismatch_400_clears` — tier (d) value mismatch.
- `test_entry_post_rejects_pattern_evaluation_id_present_with_pipeline_run_id_at_form_render_omitted_400_clears` — tier (d) missing-symmetry: anchor present + pipeline_run_id_at_form_render form field omitted → reject (per Codex R6 MAJOR #3 + R5 missing-claim symmetry).
- `test_entry_post_rejects_server_derived_manual_off_pipeline_trade_origin_with_pattern_evaluation_id_anchor_400_clears` — tier (e); validation checks SERVER-DERIVED `trade_origin` (via `swing/trades/origin.py:derive_trade_origin`), NOT form UI origin. Forged POST with valid+matching anchor + UI origin='watchlist' but server-derived `trade_origin='manual_off_pipeline'` must reject (per Codex R5 MAJOR #1).
...
| **OQ-12** *(NEW)* trades.pattern_evaluation_id lifecycle population point | §2.2 lifecycle disposition (REVISED per Codex R1 + R2 MAJOR #4 + R3/R4 closure commitment) | **SB6c T-A.6c.4 THREADS the anchor for eligible pipeline-origin entry flows** + 5-tier rejection ladder + `claimed_pattern_evaluation_anchor` consistency-check gate validates server-side at POST `/trades/entry`. Manual-off-pipeline origin: anchor omitted; persists NULL. Earlier "highest-composite" + Phase 6 chart_pattern_algo coupling proposals REJECTED — Phase 6 enum is `'none'|'flag'` disjoint from Phase 13 detector enum; highest-composite is arbitrary and contaminates per-pattern-class cohort analysis. | _pending_ |
...
- **W14 (Hidden anchor 5-tier rejection ladder + claimed-anchor consistency gate)**: T3.SB1 LOCK extended per Codex R3 + R4 closure. SB6c does NOT add NEW POST routes for Q4 (deferred to T4.SB per OQ-2). SB6c DOES modify the existing `POST /trades/entry` handler at `swing/web/routes/trades.py:413` to add a NEW hidden anchor (`pattern_evaluation_id` + `claimed_pattern_evaluation_anchor`) + 5-tier rejection ladder + claim consistency-check gate per §2.2 anchor-threading scope. The existing Schwab `schwab_source_value_json` anchor + its 4-tier rejection at trades.py:896-911 are PRESERVED unchanged.
