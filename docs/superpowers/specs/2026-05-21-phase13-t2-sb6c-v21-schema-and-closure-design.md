...
| B.3 POST `/patterns/{id}/review` label_source split (closed_loop_review vs organic_trade_history) | YES — Delta A | spec §5.10 lines 785-790 |
...
2. The candidate lookup query `SELECT id FROM candidates WHERE ticker = ? AND pipeline_run_id = ? ORDER BY id DESC LIMIT 1` returns a row.
...
**Paired write-path INSERT extension**: the existing `INSERT INTO trades` at `swing/data/repos/trades.py:120` MUST be extended with `candidate_id` in the column list + `?` placeholder. Schema-version-aware INSERT pattern (T3.SB1 `swing/data/repos/fills.py:51-53` precedent) is **NOT NEEDED** here because `candidate_id` is NULLable; pre-v21 tests can simply omit it (parameter defaulting to None). Verified at writing-plans phase via `PRAGMA table_info('trades')`-driven test that asserts column position 52 exists post-migration + asserts pre-v21 fixtures still load via legacy INSERT path (no `candidate_id` keyword arg).
...
**Lifecycle population (NEW OQ-12; brainstorm recommendation)**: at trade-entry-form lock time, populate `pattern_evaluation_id` IF:
1. The trade has a populated `candidate_id` (from §2.1 logic).
2. AND a `pattern_evaluations` row exists for `(pipeline_run_id, ticker, pattern_class)` at lock time. Lookup query: `SELECT id FROM pattern_evaluations WHERE pipeline_run_id = ? AND ticker = ? AND pattern_class = ? ORDER BY composite_score DESC LIMIT 1` (highest-composite-score evaluation if multiple pattern classes evaluated for the ticker in the same run).
3. The pattern_class to query against comes from the existing `Trade.chart_pattern_algo` / `chart_pattern_operator` fields (Phase 6 chart-pattern columns at migration 0010); if both NULL, fall through to the highest-composite-score evaluation for the ticker (regardless of pattern_class) and stamp `pattern_evaluation_id` to that.
...
9. `test_pattern_evaluations_pipeline_run_id_cascade_delete_cascade_into_trade_pattern_evaluation_id_null` — chained cascade: delete a `pipeline_runs` row → `pattern_evaluations` rows cascade-delete → `trades.pattern_evaluation_id` cascade-SET-NULL via the second FK.
...
**Backup-gate per CLAUDE.md gotcha "Migration runner backup-gate equality form"**: copy Phase 9 Sub-bundle A backup-gate clause VERBATIM with strict equality: `pre_version == 20 AND target >= 21`. The `<=` form is BANNED (would fire incorrectly for multi-version jumps).

**Backup file name** (OQ-8 disposition): `swing-pre-phase13-sb6c-migration-<ISO>.db` per Phase 13 precedent (T-A.1.1 v20 used `swing-pre-phase13-migration-<ISO>.db`).
...
| **Gap B.3** POST `/patterns/{id}/review` label_source split | §5.10 lines 785-790 | YES — Delta A | POST handler at `swing/web/routes/patterns.py:patterns_review_post` (T2.SB6b T-A.6.3) extends: after constructing the `PatternExemplar(label_source=..., ...)` row, look up via `SELECT 1 FROM trades WHERE candidate_id = ? AND state IN ('entered','managing','partial_exited','closed','reviewed') LIMIT 1` (the candidate_id comes from the URL path `pattern_evaluations.id` → join via `pattern_evaluations.ticker + pipeline_run_id` → `candidates` row OR the `candidate_id` if pattern_evaluations carries it; see §3.2 cross-row lookup discipline below). If row exists AND operator decision is `confirm`, emit `label_source='organic_trade_history'`; else emit `label_source='closed_loop_review'` (T2.SB6b V1 simplification #5 closes; T2.SB6b R1 MAJOR #3 LOCK preserved against ticker-proxy regression). | RESOLVED via Delta A |
...
- **S10 (browser; closed-loop)**: review the candidate at `/patterns/{id}/review` with `confirm` decision; confirm `pattern_exemplars` row is written with `label_source='organic_trade_history'` (NOT `closed_loop_review` — closes T2.SB6b V1 simplification #5).
...
- Modify: `swing/data/db.py:_apply_migration` IF backup-gate clause needs `pre_version == 20 AND target >= 21` augmentation (verify against Phase 9 + Phase 12 C.A precedent at writing-plans phase).
...
**Acceptance criteria**:
- Migration runs cleanly against v20-shape DB; `PRAGMA user_version` = 21.
- All 17 schema-paired tests PASS.
...
- Modify: `swing/trades/entry.py` (canonical trade entry service; calls `insert_trade_with_event` at `swing/data/repos/trades.py:102`) to populate `candidate_id` + `pattern_evaluation_id` per §2.1 OQ-11 + §2.2 OQ-12 lifecycle logic. The lookup queries (candidates by `(ticker, pipeline_run_id)`; pattern_evaluations by `(pipeline_run_id, ticker, pattern_class)`) execute before the `insert_trade_with_event` call within the entry service's own transactional block (which owns its `BEGIN IMMEDIATE` per existing entry service contract).
...
| **OQ-14** *(NEW)* Volume profile data path | Gap B.2 wiring | **`swing.web.ohlcv_cache.get_or_fetch(ticker, window_days=80)`** returning the OhlcvBar window for VM to compute 30-session sum + 50d avg ratio. Aligns with existing OhlcvCache substrate; no new fetch path. | _pending_ |
