...
- **Seeding mitigation (V1, cheap, closes most of the gap):** immediately AFTER installing the wrap, `_install_web_marketdata_caches` invokes ONE wrapped `client.tokens.update_tokens()` itself. This wrapped call is exception-isolated and refreshes the token if near expiry, seeds the liveness record, and means that even if the daemon's first unwrapped tick raced and died, a fresh token is established and the dead-thread condition is immediately visible via liveness (DEGRADED/no-heartbeat). Because the daemon thread is the one that dies, the seeding call cannot resurrect it -- so the residual that remains is "the daemon thread may be dead from tick 1 under the narrow near-expiry condition," which the `swing schwab status` liveness signal surfaces (no heartbeat advance) and which the operator resolves with a `swing web` restart.
...
- (b) **Behavioral/version guard test:** assert the installed `schwabdev.Client(...).version` (or `schwabdev` version) matches the validated `"Schwabdev 2.5.1"` and fail LOUDLY on drift so a human re-validates the checker contract before the bump ships -- a mere "attribute exists + was replaced" check does NOT prove the daemon dynamically calls the replaced method, so the version assertion is the real guard.
