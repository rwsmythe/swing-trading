...
- `render_status` reads the sidecar (states evaluated in THIS priority order -- Codex R6 Major #1: explicit failure outranks STARTING so a seed call that recorded an error is never masked as STARTING):
  1. Sidecar absent -> "Checker: unknown (web server not running, or pre-N7 build)".
  2. Explicit degraded/error marker set (e.g. the seed or a daemon tick recorded a failure: `consecutive_failures > 0` / `last_error_class` present) -> "Checker: DEGRADED -- <redacted reason>" (checked BEFORE STARTING).
  3. `last_daemon_tick_ts` present and within `STALE_THRESHOLD` -> "Checker: ALIVE (last refresh <t>; <n> consecutive failures)".
  4. `last_daemon_tick_ts` present but older than `STALE_THRESHOLD` -> "Checker: DEGRADED -- stale heartbeat".
  5. No daemon tick yet (only `last_seed_ts`), no failure marker, AND `last_seed_ts`/`installed_ts` younger than `STARTUP_GRACE` -> "Checker: STARTING".
  6. No daemon tick yet, no failure marker, but OLDER than `STARTUP_GRACE` -> "Checker: DEGRADED -- no daemon heartbeat since startup" (Codex R5 Major #1: STARTING MUST expire to DEGRADED).
- ASCII-only output (gotcha #16/#32). The exact `HEARTBEAT_WRITE_INTERVAL` / `STALE_THRESHOLD` / `STARTUP_GRACE` constants are finalized at writing-plans with the invariants `STALE_THRESHOLD > HEARTBEAT_WRITE_INTERVAL` and `STARTUP_GRACE >= one checker cadence + margin` (~60-90s; long enough for a healthy first daemon tick to land, short enough that a dead-at-startup checker flips to DEGRADED promptly) held.
...
- **Liveness render test** seeds a sidecar JSON for each state and asserts the priority order (section 5.4) -- **ALIVE / STARTING (seed-only, no failure, within `STARTUP_GRACE`) / DEGRADED-seed-with-error (seed-only but `consecutive_failures>0` -- must render DEGRADED, NOT STARTING, Codex R6 Major #1) / DEGRADED-no-daemon-heartbeat (seed-only, no failure, OLDER than `STARTUP_GRACE` -- the STARTING-expiry case, Codex R5 Major #1) / DEGRADED-stale (old daemon tick) / DEGRADED-explicit / absent(unknown)**. The seed-only cases prove the false-ALIVE prevention, the explicit-failure precedence, AND the STARTING->DEGRADED expiry. **ASCII assertion scoped to the NEW line only (Codex R1 Minor #2):** `render_status` ALREADY emits an em dash in its PROVISIONAL/DEGRADED lines (`cli_schwab.py:829`/`:831`), so a whole-output `out.isascii()` would fail on pre-existing content. Assert `.isascii()` on the checker-liveness substring the test adds (mirrors the SB5 D2 lesson: scope the ASCII assertion to the new content, not the shared surface).
