...
- **`swing/web/price_cache.py:75`** + **`swing/web/ohlcv_cache.py:112`** -- READ-ONLY reuse (the hook setters already exist; A-3 calls them). Both caches already own a TTL + a sliding-window circuit breaker (`price_cache.py:271 _maybe_trip_breaker`; `ohlcv_cache.py` breaker; `cfg.web.price_cache_ttl_seconds` / `cfg.web.ohlcv_cache_ttl_seconds` default 3600s).
...
The web app must tolerate every Schwab construction failure without crashing (a dashboard with a stale tokens DB must still serve pages on the yfinance path):
- Both env-var credentials absent -> `None` SILENTLY (default posture; preserves the yfinance-only web app for operators not using Schwab).
- Partial credentials -> WARN (naming present/absent, no secret bytes) + `None`.
...
The ~120-calls/min budget is **app-wide and shared** (web + pipeline + account ops + checker). Full-parity web wiring on a demand-driven request surface is the new variable. Three layered defenses, all reusing existing machinery:
...
Effect: Schwab calls are bounded to ~`(number of open trades) x (quote + bars)` per TTL window, regardless of render volume. A storm of non-open-trade chart loads never touches Schwab; a storm of open-trade loads is bounded by the small open-trade count + the TTL cache. This is the literal "open-trade fetch scope -- NOT per-render, NOT universe-wide" the operator specified, and it is the design-against-render-storm requirement.

**4.5.2 429 / transient miss -> yfinance + cooldown, never a hammering retry.** The ladder already catches `SchwabRateLimitError` (429), `SchwabAuthError`, `SchwabApiError` -> redacted WARN -> yfinance fallback (`marketdata_ladder.py:317-347`). Two reuse-friendly additions in A-3's web hooks:
- **TTL cache as natural backoff:** a yfinance fallback result is cached for the cache TTL, so the same `(ticker, window_days)` is NOT re-attempted against Schwab until the TTL expires. No tight retry can occur on a single cache key.
- **Schwab-attempt cooldown gate (closure-local):** on a `SchwabRateLimitError` (or repeated transient Schwab failures), the hook sets a closure-local `_schwab_cooldown_until = monotonic() + cooldown` (reusing `cfg.web.circuit_breaker_cooldown_seconds`); while in cooldown, `_should_use_schwab` returns False for ALL tickers -> the whole web path pins to yfinance for the cooldown window, protecting the shared app-wide budget across distinct cache keys. This is the L9(b) "breaker trip," sized to the web surface; it is a few lines in the hook closure (the part A-3 writes), NOT a new fetcher (L5 preserved). The existing cache sliding-window breaker continues to handle general fetch failures unchanged.
- **NO retry/backoff on the market-data path.** Bounded backoff exists ONLY in P14.N7's checker wrap (a once-per-30s daemon loop), NEVER on a request-serving market-data fetch. A 429 means "stop," not "retry sooner."

**4.5.3 `rate_limit_remaining` observability (L9c -- SHOULD).** **Brief-vs-production correction:** the brief says the `rate_limit_remaining` audit field (`audit_service.py:127`) is "currently always None." In fact `marketdata.py:_extract_response_payload` (`:101-144`) ALREADY reads the header and the success + HTTP-failure audit-close paths ALREADY pass the value (`:564`, `:643`); `audit_service.py:127` is just the pass-through parameter. The reason it resolves to `None` in production is almost certainly a **header-name mismatch** -- the extractor tries `X-RateLimit-Remaining` / `X-Rate-Limit-Remaining` / `Schwab-Client-RateLimit-Remaining`, which may not be the header Schwab actually emits. L9c therefore reduces to a small, SHOULD-tier fix: confirm Schwab's actual rate-limit header name (live header inspection / Schwab docs at writing-plans) and add it to the extractor's candidate list so the EXISTING plumbing captures real headroom. No new audit write-path, no schema. If the header name cannot be confirmed without a live production session, defer the exact name to the writing-plans/executing-plans gate and land the extractor change behind the confirmed name (do NOT guess a name that silently stays None).

**4.5.4 Budget headroom sketch (for operator confidence).** With the open-trade scope gate, steady-state web Schwab traffic = at most `(open_trades) x 2 (quote+bars)` calls per max(TTL). With a handful of open trades and a 3600s OHLCV TTL / shorter price TTL, that is single-digit-to-low-tens of calls per hour from the web path -- a tiny fraction of the 120/min budget, even concurrent with a nightly pipeline run. The render-storm scenario is the only way the web path could spike, and 4.5.1 closes it. The cooldown gate (4.5.2) is the backstop if the budget is nonetheless breached by the shared load.
...
def resilient_update_tokens(force_access_token=False, force_refresh_token=False):
    ...
    liveness.record_tick()                                          # 30s heartbeat: loop is alive
    attempt = 0
    while True:
        try:
...
```
CheckerLiveness:
  last_tick_ts            # updated every wrapper invocation (~30s) -> proves the loop is alive
...
- Write: atomic (`NamedTemporaryFile(dir=<same dir>)` + `os.replace` -- same-volume, Windows-safe per the `os.replace` gotcha). Throttled: write on state transition (alive<->degraded) and at most once per N cycles (e.g. every 10th tick, ~5 min) to avoid a disk write every 30s.
...
- `render_status` reads the sidecar: present + recent `last_tick_ts` -> "Checker: ALIVE (last refresh <t>; <n> consecutive failures)"; stale `last_tick_ts` (older than ~3x the 30s cadence) or sidecar present-but-degraded -> "Checker: DEGRADED -- <redacted reason>"; sidecar absent -> "Checker: unknown (web server not running, or pre-N7 build)". ASCII-only output (gotcha #16/#32).
...
Before A-3, the web app issued ZERO Schwab calls (yfinance-only). After A-3, the web app becomes a new RUNTIME Schwab-call surface drawing on the **shared ~120-calls/min app-wide budget** alongside the pipeline, account/reconciliation ops, and the checker. The operator has sanctioned this expansion for Schwab's better data provenance; the spec's obligation is to make it **safe within the shared budget** -- which is exactly what L9 (section 4.5) delivers: open-trade scope + TTL cache bound steady-state volume to single-digit calls/hour from the web, and the cooldown gate + yfinance fallthrough absorb any 429 without hammering. The source-grep measures SITES (unchanged); L9 governs the runtime SURFACE/VOLUME (the part that actually expanded).
...
- **Slice 1 -- A-3 web ladder install + L9 scope/cooldown gates.** `_install_web_marketdata_caches` in `app.py`; mirror `_install_pipeline_marketdata_caches`; graceful-degradation client construction; BOTH hooks (full parity); `surface='pipeline'`; the open-trade-scope gate + the Schwab-cooldown gate in the hook closures. Tests: L7 production-path wiring (hooks installed under production, absent under sandbox; daily-bar kwargs reach the ladder) + the scope gate (non-open-trade ticker bypasses Schwab) + the cooldown gate (429 pins to yfinance for the cooldown window).
...
- **Slice 2 -- P14.N7 checker-resilience wrap** (`swing/integrations/schwab/checker_resilience.py`): the wrapper + the in-memory liveness record. Install it on the web client in `app.py` (right after A-3's construction). Test: DNS-failure-during-refresh survival (the loop does not die; liveness goes degraded then recovers; forced calls still raise).
- **Slice 3 -- liveness surfacing** (ephemeral sidecar writer + `render_status` checker line). Test: `swing schwab status` renders ALIVE / DEGRADED / unknown from a seeded sidecar; ASCII-only.
...
- **A-3 wiring test** exercises the REAL `create_app(cfg)` construction -- NOT a stubbed cache:
  ...
  - **L9 cooldown gate:** simulate a `SchwabRateLimitError` from the ladder; assert the next hook call (even for an open-trade ticker, within the cooldown window) bypasses Schwab; assert it resumes after the cooldown.
...
- **P14.N7 resilience test** simulates a real DNS failure during refresh -- NOT a stubbed checker:
  - Build a fake `tokens` whose `update_tokens()` raises a `ConnectionError` (with a NameResolution-shaped message) on the first call(s) then succeeds; install the resilient wrap; invoke it directly (simulating the checker's call) and assert: (1) it does NOT raise; (2) it returns `False` on failure / the refreshed bool on success; (3) liveness transitions `installed -> degraded(consecutive_failures>0) -> alive(consecutive_failures==0)`; (4) a forced call (`force_access_token=True`) passes through and DOES propagate the exception (CLI contract preserved).
...
- **Liveness render test** seeds a sidecar JSON (ALIVE / DEGRADED / stale / absent) and asserts `render_status` output lines + `out.isascii()`.
...
- **S4** the A-3 production-path wiring + L9 scope/cooldown tests (section 10.1) green -- hooks installed under production, absent under sandbox, Schwab scoped to open-trade tickers, 429 -> cooldown.
...
- **L9:** open-trade fetch scope + TTL cache bound steady-state web calls to single-digit/hour; 429 -> yfinance + closure-local cooldown, NEVER a hammering retry; `rate_limit_remaining` made observable via a header-name fix on existing plumbing; render-storm defused by the scope gate (section 4.5).
