...
- `render_status` reads the sidecar (states in priority order):
  - `last_daemon_tick_ts` within `STALE_THRESHOLD` -> "Checker: ALIVE (last refresh <t>; <n> consecutive failures)".
  - No daemon tick yet (only `last_seed_ts`) AND `installed_ts`/`last_seed_ts` younger than `STARTUP_GRACE` -> "Checker: STARTING".
  - No daemon tick yet (only `last_seed_ts`) but OLDER than `STARTUP_GRACE` -> "Checker: DEGRADED -- no daemon heartbeat since startup" (Codex R5 Major #1: STARTING MUST expire to DEGRADED, else a daemon that died before its first tick stays STARTING forever).
  - `last_daemon_tick_ts` older than `STALE_THRESHOLD`, or sidecar explicitly degraded -> "Checker: DEGRADED -- <redacted reason>".
  - Sidecar absent -> "Checker: unknown (web server not running, or pre-N7 build)".
...
- **Liveness render test** seeds a sidecar JSON for each state -- **ALIVE / STARTING (seed-only, within `STARTUP_GRACE`) / DEGRADED-no-daemon-heartbeat (seed-only, OLDER than `STARTUP_GRACE` -- the STARTING-expiry case, Codex R5 Major #1) / DEGRADED-stale (old daemon tick) / DEGRADED-explicit / absent** -- and asserts the new checker-liveness line(s) render correctly (the seed-only cases prove the false-ALIVE prevention AND the STARTING->DEGRADED expiry). **ASCII assertion scoped to the NEW line only (Codex R1 Minor #2):** `render_status` ALREADY emits an em dash in its PROVISIONAL/DEGRADED lines (`cli_schwab.py:829`/`:831`), so a whole-output `out.isascii()` would fail on pre-existing content. Assert `.isascii()` on the checker-liveness substring the test adds (mirrors the SB5 D2 lesson: scope the ASCII assertion to the new content, not the shared surface).
