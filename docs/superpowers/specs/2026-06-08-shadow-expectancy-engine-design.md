...
| H4 | mean R positive (smaller position) | watch∩risk_feasibility-only mean R |
...
1. **Denominator funnel — two levels (Codex R3-M2).** *Detection-level reconciliation:* total emitted detections → `collapsed_duplicate_detection` (non-canonical duplicates for a (run,ticker)) → unique `(run,ticker)` signals. *Signal-level outcomes (per hypothesis):* unique signals → joinable-to-candidate → triggered (entered) → {closed, open-at-horizon} → excluded, with **reasons** (`no_candidate_join`, `invalid_ohlc`, `inconsistent_detection_series`, `inconsistent_trigger_state`, `degenerate_risk`, `insufficient_forward_depth`, `missing_observations`, `lifecycle`, `never_triggered`). **Pre-attribution failures** (`no_candidate_join`, and `invalid_ohlc` / `inconsistent_detection_series` caught before the matcher runs) are reported in an **`unattributed`** bucket, NOT under any hypothesis (Codex R4-m1); only post-attribution signals appear in the per-hypothesis breakdown. Terminal trade statuses apply only at the signal level; the two levels are reported separately and reconciled, never mixed. Makes the selected-sample risk explicit (Codex R1-M3).
