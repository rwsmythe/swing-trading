...
**E11 (degrade-safe, Codex R1 Major 1):** a closed row whose `last_fill_at` is a malformed or mixed-shape string (a correction-path or legacy artifact, per the G4 caveat) MUST NOT crash the query and MUST NOT corrupt the count of the well-formed rows. The predicate is pure SQL string comparison (no `datetime.fromisoformat` parse), so a malformed value simply sorts lexicographically — at worst that one row is mis-ordered relative to `started_ts` (an accepted bounded approximation), never an exception. The test seeds one malformed-`last_fill_at` closed row alongside well-formed rows and asserts the call returns an `int` and the well-formed rows' contribution is unchanged.
