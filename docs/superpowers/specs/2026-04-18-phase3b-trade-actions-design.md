...
6. On success (two-call pattern; both share the same `PriceCache` so the second call's `get_many` is almost entirely cache hits):
   ...
   Why two calls rather than one: `DashboardVM` does not expose per-row `OpenPositionsRowVM`s (its contract is the split `open_trade_last_prices` + `open_trade_advisories` mappings). Rather than break that contract, the success handler calls both helpers; the second call's `get_many` is partially or fully cache-hit depending on dashboard warmth — the first call only warms the entered ticker, while `build_dashboard` still needs prices for the other open positions + watchlist_top5 (see §4.7 for the cold-cache bound).

   Exit POST: same two-call pattern, but only emits `#status-strip` OOB (not `#watchlist-top5` — exits don't touch the watchlist).
   Stop-adjust POST: calls `build_open_positions_row` only (no OOB fragments needed; status-strip and watchlist unchanged by a stop move).
...
4. If `SizingResult.feasible` is False (e.g., `constraint="no_equity"` when equity ≤ 0, `constraint="infeasible"` when min-1-share would exceed max risk), render the "dim" fragment with the specific reason ("No equity recorded — add a cash_movement or set account.starting_equity" / "Risk cap too tight for 1 share at this stop"). If `compute_shares` somehow raises an unexpected exception (not expected — the helper returns feasibility flags rather than raising for known infeasible cases), catch it, log at WARNING, and render the fallback guidance "Sizing unavailable — check values". Do NOT 500.
...
Entry and exit success handlers make TWO calls that both hit `PriceCache`: `build_open_positions_row` for the primary row, then `build_dashboard` for the OOB fragments. Stop-adjust makes only the first call. Both calls share the same cache instance, so the second call's `get_many` is a cache hit for any ticker the first call already warmed.

Cost profile:

- **Cached prices case (typical):** The user just loaded the dashboard, so `PriceCache` is warm. `cache.get_many` returns snapshots from the in-memory dict — no yfinance round-trips. Submit latency is dominated by DB writes (Phase 2's `record_entry` etc.) + template rendering — ≲ 50ms on SQLite.
- **Cold cache case:** User's first action after a long idle. `get_many` issues yfinance fetches under the shared executor with `price_fetch_deadline_seconds` cap (6s default). Submit latency is bounded by that deadline; on timeout, `_fallback_snapshot` supplies `last_close` from DB, so the row renders with stale-marked prices rather than blocking indefinitely. The `build_dashboard` call piggy-backs on the freshly-warmed cache.
...
The POST handler does not block on price fetches beyond the configured deadline. The per-submit work is bounded: one row build + one dashboard rebuild (cache-hit dominated). This is deliberately heavier than a hypothetical "only rebuild what the fragment needs" path, but it keeps OOB fragment field-sets in lockstep with the dashboard template (§4.3 step 6 rationale). For the 3a `POST /prices/refresh` path the rebuild work is the explicit point of the endpoint; here it is a tolerated side effect because the alternative (bespoke per-fragment builders) was shown in adversarial review (R3 Major 1) to drift from the template field set.
...
| `tests/web/test_routes/test_trades_route.py` | 24 | GET each form returns expected fragment with ticker/trade_id context. POST entry success → primary-target row + `#status-strip` OOB + `#watchlist-top5` OOB (test asserts each OOB fragment carries the hx-swap-oob attribute). POST entry soft-warn → confirm fragment → POST again with `force=true` → success. POST entry hard-cap → 400. POST entry duplicate → 400 with drift-recovery fragment (§5.1 case 1). POST exit full → row disappearance fragment + `#status-strip` OOB only (no watchlist OOB). POST exit partial → updated row with derived remaining-shares. POST exit shares-too-many after CLI drift → form re-rendered with updated max (§5.1 case 2). POST stop-adjust success → row only (no OOB). POST stop-adjust regression → 400 with updated `current_stop` prefilled (§5.1 case 3). Form GET for a closed trade → HTMX-aware 404 fragment from §5.2 handler (not JSON). GET /trades/entry/sizing-hint happy path → fragment with numbers. GET /trades/entry/sizing-hint with missing/blank/invalid/stop-≥-entry params → 200 with "dim" guidance fragment (never 400/422/500). GET /trades/entry/sizing-hint with equity=0 → "dim" guidance fragment rendered from `SizingResult(feasible=False, constraint="no_equity")` (not an exception). GET /trades/entry/sizing-hint does NOT require HX-Request (it's a safe method). POST /trades/entry without HX-Request → 403 (strict mode). HTMX 404 response is a fragment; non-HTMX 404 still uses FastAPI default. |
...
5. **After submit (two-call pattern; shared cache)**: call `build_open_positions_row` for the primary target row, then `build_dashboard` for the OOB fragments' source data. Entry emits `#status-strip` + `#watchlist-top5` OOBs. Exit emits `#status-strip` only. Stop-adjust calls only `build_open_positions_row` (no OOB). Warm-cache fast path dominates; cold-cache is bounded by `price_fetch_deadline_seconds` (§4.7). Fragments render from authoritative VM fields — no bespoke per-fragment builders that could drift.
