...
| `tests/web/test_dashboard_banner.py` | **NEW** integration: plant a multi-leg auto-correction + assert banner fires; invoke tier-3 override on the chain head + assert banner count drops (AMENDMENT (Phase 12.5 #3 T-3.7): supersedes prior "assert banner UNCHANGED" per §9.3 S4 amendment; shipped helper drops the row from count when override flips parent `resolved_by`); insert a fresh `reconciliation_run` + ZERO auto-redirects in it + assert banner clears. | ~+40 LOC. |
