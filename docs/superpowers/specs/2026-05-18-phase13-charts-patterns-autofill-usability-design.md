...
| `geometric_score_json` | TEXT NULL | per-rule pass/fail breakdown (v2 brief §9.2 evidence-to-show-reviewer). **NULLABLE per Codex R4 M#2**: rule-tier evaluation requires T2.SB3+/SB4 detectors; T2.SB1 silver-tier exemplars persist with `geometric_score_json=NULL` until backfill. Cross-column CHECK invariant: `(label_source IN ('claude_silver', 'codex_silver') AND geometric_score_json IS NULL) OR (label_source IN ('curated_gold', 'closed_loop_review', 'organic_trade_history', 'synthetic', 'perturbation') AND geometric_score_json IS NOT NULL)` — silver-tier AI labels can be NULL pre-detector; all other label_sources require rule-tier evidence at insert time (operator who curates a gold exemplar OR closes-loop on a candidate has the detector geometric_score available; synthetic/perturbation generators emit evidence by construction). At T2.SB3+/SB4 dispatch, a backfill task recomputes `geometric_score_json` against silver-tier rows that have NULL. |
| `labeler_evidence_json` | TEXT NULL | NEW per Codex R4 M#2 — the Claude/Codex subagent's structural-evidence narrative output (distinct from rule-tier `geometric_score_json`). Populated when `label_source IN ('claude_silver', 'codex_silver')`; NULL otherwise. Preserves the silver-tier audit trail without conflating with rule-tier evidence. |
...
| `parent_exemplar_id` | INTEGER NULL | FK to `pattern_exemplars(id)` `ON DELETE SET NULL` (closes Codex R4 M#3). Populated when `label_source='codex_silver'` AND the row was inserted because Codex disagreed with a parent `claude_silver` row; preserves the disagreement-chain linkage for cohort queries + template-match double-counting prevention. NULL for all other rows. |
