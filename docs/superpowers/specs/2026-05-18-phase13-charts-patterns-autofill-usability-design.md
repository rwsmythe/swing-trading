...
Library of curated + AI-labeled + organic-trade-history pattern instances. Anchors v2 brief §6.4 label schema. ~15-18 columns expected:
...
| `label_source` | TEXT NOT NULL | CHECK enum: `curated_gold` / `claude_silver` / `codex_silver` / `organic_trade_history` / `synthetic` / `perturbation` (per v2 brief §8 5-source taxonomy; 6 values total). Rejection of an exemplar proposal is encoded via `final_decision='rejected'` (source-neutral name per Codex R3 M#2 + R3 m#2 fix); `label_source` itself remains the SOURCE of the exemplar's origin, NOT the decision verb. |
...
| `geometric_score_json` | TEXT NOT NULL | per-rule pass/fail breakdown (v2 brief §9.2 evidence-to-show-reviewer) |
...
| `organic_trade_history` | `confirmed` / `watch` / `rejected` / `relabeled` (operator decisions in T2.SB6 closed-loop surface; closes Codex R3 M#2) |
| `synthetic` | `generated` only (machine-generated; no operator decision applicable) |
| `perturbation` | `generated` only (same as synthetic) |

Encoded as: `(label_source = 'curated_gold' AND final_decision = 'confirmed') OR (label_source IN ('claude_silver', 'codex_silver', 'organic_trade_history') AND final_decision IN ('confirmed', 'watch', 'rejected', 'relabeled')) OR (label_source IN ('synthetic', 'perturbation') AND final_decision = 'generated')`.
...
| `ticker` | TEXT NOT NULL | |
| `pattern_class` | TEXT NOT NULL | CHECK enum same as `pattern_exemplars.pattern_class` |
...
4. **Selective Codex 2nd reviewer** (per L9 + §9 OQ-5): T2.SB1 uses **ONLY random 15% sampling** (closes Codex R3 M#4 — the rule/silver-disagreement clause is DEFERRED to T2.SB3+/SB4 when rule-tier `geometric_score` becomes available; T2.SB1 does NOT yet have rule detectors). At T2.SB1 the Codex MCP fires on the random-sample subset only; persists `codex_reviewed=1` + `codex_agreement=<bool>` + a separate row with `label_source='codex_silver'` if Codex disagreed. The full SELECTIVE policy (random 15% + high-stakes individual labels) ACTIVATES at T2.SB3+/SB4 sub-bundles when detector geometric_score is available.
...
**Form action** (rewritten post-Codex R3 M#2 + R3 m#2 source-neutral decision names): POST writes to `pattern_exemplars` with `label_source='organic_trade_history'` + `proposed_pattern_class=<the detector's class>` + `final_decision` mapped from operator decision per source-neutral enum: `confirm` -> `final_decision='confirmed'`; `watch` -> `final_decision='watch'`; `reject` -> `final_decision='rejected'`; `relabel` -> `final_decision='relabeled'` + `final_pattern_class=<operator-corrected-class>`; `pattern_present_outside_window` and `multiple_overlapping_patterns` route through a separate window-shift / multi-exemplar emit per v2 brief §9.3. `gold_validated_at=<now>` for all branches except `rejected` which retains `gold_validated_at=NULL`. All `(label_source='organic_trade_history', final_decision)` pairs are valid per the §3.1 source-vs-decision matrix CHECK invariant (closes Codex R3 M#2).
...
1. **Schema-CHECK + Python-constant + dataclass-validator paired atomic landing** (Phase 12 C.A T-A.2). Applies to every CHECK enum widening in v20 (pattern_class / label_source / fill_origin / surface).
