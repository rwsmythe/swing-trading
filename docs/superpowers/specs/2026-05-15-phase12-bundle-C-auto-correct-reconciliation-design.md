...
| `schwab_returned_no_match` | Schwab API queried for the relevant period returned NO matching record for a journal fill (`actual={"matched": null}`); journal fill might be entirely manual-entry-mistake OR a non-Schwab-routed broker fill that operator typed-from-memory after-the-fact | (a) `mark_unmatched` (operator-acknowledged "Schwab has no record"; NO journal mutation; resolution → `operator_resolved_ambiguity` per §6.2.1 Codex R7 Major #1 cleanup — `acknowledged_immaterial` is NOT a tier-2 outcome); (b) `operator_truth` (REQUIRES `--custom-value`; operator types real values from off-Schwab broker source); resolution → `operator_resolved_ambiguity`. The `acknowledged_immaterial` resolution is RESERVED for tier-3 Schwab-is-wrong overrides per §1.3 lock #4 + §6.4; reachable from `schwab_returned_no_match` only by first running tier-2 (e.g., `mark_unmatched`) then operator decides Schwab itself was wrong → tier-3 override via `override-correction` CLI. |
