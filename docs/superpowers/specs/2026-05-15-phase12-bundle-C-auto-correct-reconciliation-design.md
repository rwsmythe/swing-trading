...
- LOGIC (V1, source_grain='order' — all tier-2):
  - If source_payload has ZERO matching Schwab orders → tier=2, `ambiguity_kind='schwab_returned_no_match'`. Choices: operator marks unmatched OR supplies source-of-truth (`--choice operator_truth --custom-value ...`) OR acknowledges as tier-3.
  ...
  - If source_payload has TWO OR MORE Schwab orders whose totals DON'T match journal_row.quantity → tier=2, `ambiguity_kind='multi_match_within_window'`. Choices: operator picks Nth Schwab record OR marks unmatched.
...
- S6: operator picks (interactively, no automation) — `swing journal discrepancy resolve-ambiguity 39 --choice <picked> --reason ...` — and the disposition lands per §10.2 post-state (resolution → `operator_resolved_ambiguity`). **Codex R6 Major #3 acceptance test (LOCKED):** S6 MUST include AT LEAST ONE payload-required choice invocation among {DHC 39, VSAT 40} to acceptance-test the §6.3 per-choice payload contract. Specifically: operator picks one of `consolidate_using_operator_vwap` / `split_into_partials` / `operator_truth` / `custom` (any payload-required choice) with a concrete `--custom-value '...'` payload + verifies (a) the CLI rejected an INTENTIONALLY-omitted `--custom-value` invocation first (parse-time payload-required error message surfaced), then (b) the correct invocation lands the resolution + the operator-supplied payload appears in `reconciliation_corrections.applied_value_json` of the resulting audit row. If both DHC + VSAT operators pick no-mutation choices (e.g., both `keep_journal_as_is` or both `mark_unmatched`), the gate fails because the payload contract was never exercised. (Mitigation if operator's actual data calls for no-mutation: a contrived `--choice custom --custom-value '{"reason_hint": "no-op"}'` invocation on one of the two cases satisfies the acceptance test; writing-plans clarifies the minimum acceptance-test recipe.)
