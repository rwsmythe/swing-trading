...
- **disc 39 DHC `unmatched_open_fill`:** journal `fills.fill_id=2` entry @$7.58 × 39 on 2026-04-27; Schwab Trader API returned `actual={"matched": null}` indicating no single Schwab order matched at original-run time. **Tier-2 candidate** — Pass-2 re-fetch via `get_account_orders` (§8.4) returns order-level data only; per §8.4 Pass-2-tier-1-FORBIDDEN lock, classifier emits `(tier=2, ambiguity_kind='multi_partial_vs_consolidated', correction_target=None)` if 2+ orders summing to qty=39 are returned (or other tier-2 ambiguity_kind per response shape; §10.2 walkthrough). Service does NOT auto-apply; sets discrepancy `resolution='pending_ambiguity_resolution'` + populates `ambiguity_kind` column; operator resolves via CLI surface (§6) by picking one of: `keep_journal_as_is` (V1 default) / `consolidate_using_operator_vwap` (requires `--custom-value` with operator-computed VWAP from broker execution statement) / `split_into_partials` (requires `--custom-value` with execution-level partial-fill payload) / `custom`.

- **disc 40 VSAT `unmatched_open_fill`:** journal `fills.fill_id=6` entry @$65.69 × 2 on 2026-05-06; `manual_entry_confidence='low'` (operator flagged uncertain at entry); Schwab `actual={"matched": null}`. **Classifier evaluates per-row based on actual Schwab Pass-2 payload at classification time — tier-2 ALWAYS per §8.4 Pass-2-tier-1-FORBIDDEN lock:** if Schwab returns 1 single matching order with different price/qty → tier-2 `unknown_schwab_subtype` (V1 mapper exposes only order-level price; cannot tier-1 redirect to entry_price_mismatch on order-level data); if Schwab returns 2+ orders summing to journal qty → tier-2 `multi_partial_vs_consolidated`; if Schwab returns 0 orders → tier-2 `schwab_returned_no_match`. Discriminator is the Schwab payload + the V1-mapper limitation; the journal-side `manual_entry_confidence` flag is operator-honesty metadata at entry-time (NOT classification input).
...
| **Validator chain** | The set of dry-run journal-table validators the auto-correction service consults before applying a tier-1 correction: Phase 7 fills schema-CHECK + FK + `_recompute_aggregates` aggregate invariants mirrored via the new `swing/trades/reconciliation_validators.py` shim (§5.5). Phase 6 `review_log` invariants are NOT validators (they're frozen aggregates retained-and-marked-superseded per §9.1). Phase 9 `risk_policy` references are NOT validators (advisories surface at Phase 10 dashboard time, not validator-reject time, per §1.6 + §5.5). If the dry-run rejects the correction, classifier downgrades tier-1 → tier-2 with `ambiguity_kind='validator_rejected'`. | §4.6; §5.5 |
...
| `field_name` | TEXT | NOT NULL | the column name within `affected_table` that the correction modified (e.g., `'price'`, `'quantity'`, `'fill_datetime'`); single-column-per-correction-row simplifies forensic trail (multi-column atomic corrections produce N rows under same `discrepancy_id` chained by `correction_set_id` — §3.1.1; Codex R7 Minor #2 typo fix — earlier `applied_in_correction_set_id` was a typo) |
...
|---|---|---|
| `multi_partial_vs_consolidated` | Schwab side has N (N ≥ 2) partial fills for a single journal-side consolidated fill (or vice-versa) matching by `(ticker, date, total_qty)` | (a) Split journal into matching partials (REQUIRES `--custom-value` with execution-level partial-fill payload); (b) Keep journal consolidated + use operator-supplied VWAP as `fills.price` (REQUIRES `--custom-value '{"price": X.XX}'`); (c) Keep journal as-is (no mutation); (d) Operator-custom (Codex R5 M#1 cleanup — descriptions match the §6.2.1 menu where `split_into_partials` + `consolidate_using_operator_vwap` BOTH require operator-supplied execution payloads since V1 mapper exposes only order-level prices per `swing/integrations/schwab/mappers.py:223-229`) |
| `multi_match_within_window` | Multiple Schwab orders could plausibly match one journal fill within the matching window (e.g., 2 Schwab orders at same price + qty within the same day for the same ticker) | (a) `pick_schwab_record_<N>` — operator selects Nth Schwab candidate AND supplies operator-confirmed execution-level field values per Codex R7 M#2 + R8 M#3 lock (REQUIRES `--custom-value` because Pass-2 candidates are `SchwabOrderResponse` order-grain rows whose price is order/limit/stopPrice not execution per `swing/integrations/schwab/mappers.py:223-229`); (b) `mark_unmatched` (no journal mutation — no Schwab record corresponds); (c) `custom` (REQUIRES `--custom-value`) |
| `unknown_schwab_subtype` | Schwab transaction type is unrecognized by the classifier's `schwab_subtype` lookup (e.g., a new Schwab transaction kind classifier has no `discrepancy_type` mapping for) OR Pass-2 returns a single order at order-grain that V1 mapper cannot disambiguate to execution-grain per §8.4 single-order-multi-execution case | (a) `acknowledge` + log for code update (V2 sub-bundle adds classifier coverage); (b) `operator_truth` (REQUIRES `--custom-value` with field-value payload; Codex R5 Major #4 + R6 Major #2 fix); (c) `custom` (REQUIRES `--custom-value`) |
...
| `schwab_returned_no_match` | Schwab API queried for the relevant period returned NO matching record for a journal fill (`actual={"matched": null}`); journal fill might be entirely manual-entry-mistake OR a non-Schwab-routed broker fill that operator typed-from-memory after-the-fact | (a) `mark_unmatched` (operator-acknowledged "Schwab has no record"; NO journal mutation; resolution → `operator_resolved_ambiguity` per §6.2.1 Codex R7 Major #1 cleanup — `acknowledged_immaterial` is NOT a tier-2 outcome); (b) `operator_truth` (REQUIRES `--custom-value`; operator types real values from off-Schwab broker source); resolution → `operator_resolved_ambiguity`. **Codex R9 Minor #1 wording cleanup:** `acknowledged_immaterial` is NOT reachable from this `schwab_returned_no_match` ambiguity. The tier-3 override CLI sets `resolution='operator_overridden'` (not `'acknowledged_immaterial'`) per §5.7 + §6.4; the `'acknowledged_immaterial'` enum value persists only as Phase 9 back-compat for the existing 30 production rows (§3.2 + §1.3 lock #4). |
| `validator_rejected` | Tier-1 correction target would violate a Phase 7 fills schema-CHECK / FK / `_recompute_aggregates` invariant as mirrored via the `swing/trades/reconciliation_validators.py` shim (§5.5); classifier downgrades automatically. Per Codex R9 Minor #2 cleanup — Phase 6 review_log + Phase 9 risk_policy are NOT validators in this chain; they're handled by retain-and-mark-superseded (§9.1) + advisory surface (§5.5) respectively. | (a) Acknowledge (correction would violate journal invariants; system cannot apply); (b) Operator-supplied alternative value (must still pass validator chain at apply time); (c) Operator-supplied policy override (writes a new risk_policy version + retries — V2 path; V1 = NOT supported) |
...
Coverage check against §1.4 production discrepancies (Codex R6 Major #2 LOCK alignment with §8.4 Pass-2-tier-1-FORBIDDEN): CVGI 41 hits tier-1 (no ambiguity_kind needed; tier-1 from PERSISTED entry_price_mismatch JSON per §10.1). DHC 39 + VSAT 40 are BOTH tier-2 always under V1 because Pass-2 returns order-grain data only; specific `ambiguity_kind` is one of `multi_partial_vs_consolidated` / `unknown_schwab_subtype` / `schwab_returned_no_match` depending on Pass-2 response shape (per §10.2 + §10.3 walkthroughs). No tier-1 redirect from Pass-2 order-grain data anywhere. All three production cases covered without `unsupported` fallback unless Pass-2 itself fails (in which case `unsupported` is the persisted state per §8.4 #3).
...
- INPUT: discrepancy (carrying `actual_value_json={"matched": null}` or similar), journal_row (the journal `fills` row), source_payload (a Pass-2-re-fetched `list[SchwabOrderResponse]` from `get_account_orders` per §8.4 — order-grain only in V1; potentially multiple, potentially zero), source_grain hint (= `'order'` for V1 since the only Pass-2 source is the orders endpoint).
- **§8.4 Pass-2-tier-1-FORBIDDEN LOCK enforcement (Codex R6 Major #1 fix):** because V1 source_payload is ORDER-GRAIN (not execution-grain), the sub-classifier NEVER emits tier-1 from Pass-2 data — every path below ends in tier-2 regardless of N. Future V2 source-grain hint `'execution'` (after mapper expansion exposes per-execution detail) MAY allow tier-1 redirect; V1 SHALL NOT.
- LOGIC (V1, source_grain='order' — all tier-2):
  - If source_payload has ZERO matching Schwab orders → tier=2, `ambiguity_kind='schwab_returned_no_match'`. Choices (per §6.2.1 Codex R7 Major #1 lock — all land as `operator_resolved_ambiguity`): `mark_unmatched` (no mutation) OR `operator_truth` (`--custom-value` with real values). Per Codex R9 Minor #1 cleanup: `acknowledged_immaterial` is NOT a reachable outcome of this sub-classifier or any subsequent tier-3 override; tier-3 override lands `resolution='operator_overridden'` (§5.7 + §6.4).
  - If source_payload has ONE Schwab order matching (ticker, date) but with different price OR quantity → tier=2, `ambiguity_kind='unknown_schwab_subtype'` (per §8.4 single-order-with-aggregated-price case; cannot redirect to `entry_price_mismatch` because order-grain price ≠ execution-grain price). Choices: `acknowledge` / `operator_truth` (--custom-value) / `custom`.
  - If source_payload has TWO OR MORE Schwab orders whose total quantity matches journal_row.quantity → tier=2, `ambiguity_kind='multi_partial_vs_consolidated'`. Candidate choices (per §6.2.1 menu): `keep_journal_as_is` (V1 default; no mutation) OR `consolidate_using_operator_vwap` (requires `--custom-value` with operator-computed VWAP from broker execution statement) OR `split_into_partials` (requires `--custom-value` with execution-level partial-fill payload) OR `custom`.
  - If source_payload has TWO OR MORE Schwab orders whose totals DON'T match journal_row.quantity → tier=2, `ambiguity_kind='multi_match_within_window'`. Choices (per §6.2.1 menu): `pick_schwab_record_<N>` (REQUIRES `--custom-value` with operator-supplied execution-level field values for the Nth Schwab record per Codex R7 M#2 lock; Pass-2 candidates are order-grain only) OR `mark_unmatched` (no mutation) OR `custom`.
...
4. For multi-fill split-into-partials (DHC 39 path), the handler:
   - Validates the operator-supplied `--custom-value` payload (per §6.2.1 Codex R5 M#3 lock — operator supplies per-execution qty + price + fill_datetime; V1 mapper cannot expose execution-leg detail).
   - DELETEs the consolidated journal fill (`fills` table; `_recompute_aggregates` runs).
   - INSERTs N partial fills from the OPERATOR-SUPPLIED payload via the existing `insert_fill_with_event` path (each gets its own `fill_id`).
...
Where `<choice_code>` is a per-ambiguity_kind code per §6.2.1 table. **`--custom-value` requirement is per-choice (Codex R5 Major #2 fix — LOCKED per-choice payload contract):** suffix-based rules (e.g., "ends in `_custom`") are insufficient because per Codex R4 + R5 verification, several non-`_custom` choices REQUIRE `--custom-value` (e.g., `consolidate_using_operator_vwap`, `split_into_partials`, `operator_truth`, `operator_alternative`, `pick_schwab_record_<N>` for some kinds). The §6.2.1 table is the binding contract: each choice row's Description column explicitly states "REQUIRES `--custom-value`" when the choice cannot be applied without operator-supplied payload. The CLI validates per the table at command-parse time + emits a clear error message naming the missing required flag. `--reason` is REQUIRED on all resolutions per §5.6.
...
|---|---|---|---|
| `multi_partial_vs_consolidated` | `split_into_partials` | Replace journal consolidated fill with N partial fills. **REQUIRES `--custom-value` with execution-level partial-fill payload** (operator supplies per-execution `[{"qty": Q1, "price": P1, "fill_datetime": "..."}, ...]` from broker statement; V1 mapper cannot expose execution-leg detail per Codex R5 M#3 lock — Schwab `SchwabOrderResponse.price` is order-level only). | DELETE consolidated fill; INSERT N partial fills from operator-supplied payload; `correction_set_id` group; resolution → `operator_resolved_ambiguity` |
...
| `multi_match_within_window` | `pick_schwab_record_<N>` | Pick the Nth Schwab candidate (N is an index into the candidate list from `show-ambiguity`). **REQUIRES `--custom-value`** with operator-supplied execution-level field values per Codex R7 Major #2 lock — the Pass-2 candidates are `SchwabOrderResponse` order-grain rows; their `price` field is order/limit/stopPrice not execution; operator consults broker execution statement and supplies real fill values (qty + price + fill_datetime) for the Nth Schwab record. The choice records WHICH Schwab record the operator selected (audit metadata) AND the operator-supplied execution-level field-value payload. | UPDATE journal fields to match operator-supplied payload (corresponding to Nth Schwab record); resolution → `operator_resolved_ambiguity` |
...
| `unknown_schwab_subtype` | `operator_truth` | Operator supplies the real journal field values (Codex R5 Major #4 fix — added to menu for the case where operator has broker-statement execution detail and wants to correct journal price/qty for a single-order-multi-execution V1-mapper case per §8.4) | REQUIRES `--custom-value` with field-value payload; service applies as multi-column correction; resolution → `operator_resolved_ambiguity` |
| `unknown_schwab_subtype` | `custom` | Operator-custom transformation | REQUIRES `--custom-value`; resolution → `operator_resolved_ambiguity` |
...
3. Operator picks a `<choice_code>` + supplies `--reason` (REQUIRED) + supplies `--custom-value` per the §6.2.1 per-choice payload contract (Codex R5 Major #2 fix — `--custom-value` requirement is per-choice not suffix-based; the §6.2.1 Description column is the binding contract).
...
2. **Pass 2 — re-fetch Schwab when Pass 1 emits `ambiguity_kind='unsupported'` or persisted JSON is provably insufficient.** For `unmatched_open_fill` / `unmatched_close_fill` discrepancies with `actual_value_json={"matched": null}` (the persisted shape on shipped emitter), Pass 1 cannot distinguish partials-vs-no-match-vs-multi-match. **Pass 2 calls `get_account_orders` at `swing/integrations/schwab/trader.py:329` (NOT `get_account_transactions` — Codex R2 Critical #1 verification: `SchwabTransactionResponse` at `swing/integrations/schwab/models.py:207-218` carries only transaction_id+date+type+net_amount+description with NO symbol/quantity/price; only the orders endpoint's `SchwabOrderResponse` at `models.py:133-203` carries `instrument_symbol`+`quantity`+`price`+`instruction` needed for fill-level partial-fill matching).** Each Pass 2 fetch consumes one `schwab_api_calls` audit row with `surface='cli'` (operator-initiated backfill) + `linked_correction_id` set when a correction is ultimately written. Per Phase 11 sandbox gating (§9.7), under `environment='sandbox'` Pass 2 returns no domain data; the classifier emits tier-2 `unsupported` with rationale "sandbox: cannot re-fetch source-canonical payload".
   ...
   **V1 mapper limitation — tier-1 redirect FORBIDDEN when source is order-level (Codex R3 Critical #1 + Major #1 fix — LOCKED):** the shipped V1 `SchwabOrderResponse.price` field maps from the order's top-level `price` (limit-price) OR `stopPrice` per the shipped mapper at `swing/integrations/schwab/mappers.py:223-229`, NOT from per-execution fill prices. A limit/stop price can differ from the actual execution price, so auto-correcting `fills.price` to match `SchwabOrderResponse.price` would silently corrupt the journal — a textbook violation of the §4.4 determinism principle. Pass 2 returning `SchwabOrderResponse` data SHALL NOT be used for tier-1 price corrections. The classifier behavior under Pass 2 with order-level source data is **tier-2 always**:
   ...
   - 2+ orders returned summing to journal qty → tier=2, `ambiguity_kind='multi_partial_vs_consolidated'` (operator picks per §6.2.1 menu: `keep_journal_as_is` (V1 default; no mutation) / `consolidate_using_operator_vwap` (REQUIRES `--custom-value` with operator-computed VWAP) / `split_into_partials` (REQUIRES `--custom-value` with execution-level partial-fill payload) / `custom`).
...
- Pass 2 receives a `list[SchwabOrderResponse]` filtered to DHC. Sub-classifier (§4.3.2 unmatched_open_fill) re-evaluates against the response shape — **tier-2 ALWAYS per §8.4 V1-mapper-limitation lock** (Pass-2-tier-1-FORBIDDEN for order-level source data):
  - If Schwab returns 2 SEPARATE orders summing to qty=39 (e.g., 20 + 19) → tier=2, `ambiguity_kind='multi_partial_vs_consolidated'` (operator picks per §6.2.1 menu: `keep_journal_as_is` (V1 default; no mutation) / `consolidate_using_operator_vwap` (REQUIRES `--custom-value`) / `split_into_partials` (REQUIRES `--custom-value`) / `custom`; `SchwabOrderResponse.price` is order-level not execution-level so V1 cannot auto-derive prices).
...
- Pass 2 OUTPUT (assume the 2-separate-orders case): `ClassificationResult(tier=2, ambiguity_kind='multi_partial_vs_consolidated', correction_target=None, candidate_choices=[...the 2 SchwabOrderResponse rows surfaced as ORDER-GRAIN EVIDENCE rows (not directly usable as execution partials per §8.4 V1-mapper limitation; operator must consult broker execution statement for actual fill prices when picking split_into_partials or consolidate_using_operator_vwap)...])`.

**Classifier (§4.3.2 unmatched_open_fill sub-classifier) — final under Pass 2:**
- INPUT: discrepancy, source_payload from Pass 2 re-fetch (2 SchwabOrderResponse rows from `get_account_orders` — order-grain evidence, not execution-grain partials per §8.4 V1-mapper LOCK — summing to qty=39 across the 2 orders), journal_row = single fill_id=2 with qty=39.
- LOGIC: source_payload has 2 SchwabOrderResponse rows whose total quantity matches journal_row.quantity → tier=2, `ambiguity_kind='multi_partial_vs_consolidated'`.
- OUTPUT: `ClassificationResult(tier=2, ambiguity_kind='multi_partial_vs_consolidated', correction_target=None, correction_reason="unmatched_open_fill on (DHC, fill_id=2): journal consolidated qty=39 @ $7.58; Schwab returns 2 separate orders summing to qty=39; V1 mapper exposes order-level price only (per §8.4 Pass-2-tier-1-FORBIDDEN lock) — operator must consult broker execution statement and choose keep_journal_as_is (no mutation) OR consolidate_using_operator_vwap (requires --custom-value with operator-computed VWAP) OR split_into_partials (requires --custom-value with execution-level partial payload).", candidate_choices=[{"code": "keep_journal_as_is", "description": "Acknowledge Schwab partial-fill aggregation; no journal mutation (V1 default recommendation per §14.OQ-4)"}, {"code": "consolidate_using_operator_vwap", "description": "Keep journal consolidated; update price to operator-supplied VWAP. REQUIRES --custom-value '{\"price\": X.XX}' (operator computes VWAP from broker execution statement; V1 mapper cannot auto-derive execution-level prices)"}, {"code": "split_into_partials", "description": "Replace journal fill with N partial fills. REQUIRES --custom-value with execution-level partial-fill payload (operator supplies per-execution qty + price from broker statement)"}, {"code": "custom", "description": "Operator-supplied arbitrary payload via --custom-value"}])`.
...
Per §6.2.1 Codex R5 M#3 LOCK: this choice REQUIRES `--custom-value` with execution-level partial-fill payload (operator supplies per-execution qty + price from broker execution statement, since V1 Schwab mapper exposes only order-level price). Operator runs: `swing journal discrepancy resolve-ambiguity 39 --choice split_into_partials --custom-value '[{"qty": 20, "price": 7.57, "fill_datetime": "2026-04-27T14:23:00"}, {"qty": 19, "price": 7.59, "fill_datetime": "2026-04-27T14:23:42"}]' --reason "Schwab broker statement shows 2 partial executions; operator split journal accordingly"`.
...
- DELETEs fill_id=2.
- INSERTs N new fills from the operator-supplied payload, all with `action='entry'` per the original fill's action.
- `_recompute_aggregates` after each.
- Writes N + 1 `reconciliation_corrections` rows (1 deletion sentinel + N insertion sentinels) bundled under one `correction_set_id`. Each row's `field_name` is `'__delete__'` / `'__insert__'` sentinel; `applied_value_json` carries the full inserted fill payload for forensic trail. Operator-decision-anchored; `source_canonical_value_json` carries `{"_source": "operator_supplied_from_broker_statement"}` marker mirroring the consolidate_using_operator_vwap discipline.
...
**Backfill Pass 2 (§8.4) — re-fetch Schwab via `get_account_orders` for (VSAT, 2026-05-06):** consumes one `schwab_api_calls` audit row with `surface='cli'`, `endpoint='accounts.orders.list'` (per §8.4 Codex R2 C#1 lock — orders endpoint, NOT transactions endpoint). Three downstream classifier paths depending on actual Schwab payload:
...
- Tier=2, `ambiguity_kind='schwab_returned_no_match'` → operator decides: `mark_unmatched` (no mutation) OR `operator_truth` (`--custom-value`). Per §6.2.1 + Codex R7 Major #1 lock, all tier-2 outcomes land `operator_resolved_ambiguity`; `acknowledged_immaterial` is tier-3 only.

(All three cases are tier-2; no tier-1 redirect is permitted because Pass-2 source data is order-level not execution-level.)

**Why VSAT 40 is per-row data-dependent:** Pass 1 (§8.4) cannot break the ambiguity from `{"matched": null}` alone — same persisted-shape gap as DHC 39. Pass 2 re-fetches Schwab via `get_account_orders` for (VSAT, 2026-05-06) to get the full order list (consuming a `schwab_api_calls` audit row with `endpoint='accounts.orders.list'` attributed `surface='cli'`; under sandbox this short-circuits per §9.7 and classifier emits tier-2 `unsupported`).
...
- S6: operator picks (interactively, no automation) — `swing journal discrepancy resolve-ambiguity 39 --choice <picked> --reason ...` — and the disposition lands per §10.2 post-state (resolution → `operator_resolved_ambiguity`). **Codex R6 M#3 + R7 M#3 + R8 M#1+M#2 acceptance test (LOCKED — revised again):** the payload-required `--custom-value` contract MUST be acceptance-tested but ENTIRELY via an isolated synthetic-fixture pathway, NOT against production DHC/VSAT cases. Acceptance-test mechanic (final): **(a)** Pre-S6: the gate runs the payload-contract acceptance test against an **isolated test database** (fresh tmp `swing.db` produced by writing-plans-drafted gate fixture per S1's pytest infrastructure; NOT operator's production `~/swing-data/swing.db`); the gate plants a synthetic test discrepancy + first exercises parse-time payload-required error (operator omits `--custom-value` on a payload-required choice → CLI exits non-zero with clear missing-flag error), then dispositions via `--choice operator_truth --custom-value '{"price": 1.23}'` + verifies the `reconciliation_corrections.applied_value_json` row contains the operator-supplied payload. The choice picked at gate-time MUST be one that is valid for the synthetic discrepancy's `ambiguity_kind` — fixture deliberately plants a discrepancy of `ambiguity_kind='unknown_schwab_subtype'` so `operator_truth` is a valid choice (per §6.2.1 menu). Synthetic-fixture DB is discarded at gate end. **(b)** S6 then proceeds with operator's REAL disposition of production DHC 39 — whatever the operator's actual data calls for (`keep_journal_as_is` if no broker-statement consultation OR `consolidate_using_operator_vwap` / `split_into_partials` / `operator_truth` / `custom` with real `--custom-value` if operator has execution-level data). NO contrived contortions on the real production case; the payload-contract acceptance-test surface is satisfied by (a)'s synthetic-fixture run alone. Append-only audit semantics on `reconciliation_corrections` (§3.1) are preserved because (a) runs against an isolated fixture DB that gets discarded.
- S7: `swing journal discrepancy show-ambiguity 40` then `resolve-ambiguity 40 --choice <picked> --reason ...` for VSAT 40 per §10.3 (tier-2 case A/B/C dispositioned per actual Pass-2 outcome). **Codex R8 Minor #2 clarification:** the payload-contract acceptance-test surface is already satisfied by S6's pre-step isolated-fixture run (per S6 LOCKED revised mechanic above) — S7 does NOT need to exercise a payload-required choice on the production VSAT 40 case; operator dispositions per their actual data without contortion.
