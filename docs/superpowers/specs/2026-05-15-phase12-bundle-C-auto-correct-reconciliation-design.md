...
- S6: operator picks (interactively, no automation) — `swing journal discrepancy resolve-ambiguity 39 --choice <picked> --reason ...` — and the disposition lands per §10.2 post-state (resolution → `operator_resolved_ambiguity`). **Codex R6 Major #3 + R7 Major #3 acceptance test (LOCKED — revised):** the payload-required `--custom-value` contract MUST be acceptance-tested somewhere in the gate, but NOT via a contrived no-op on the production DHC/VSAT discrepancies (Codex R7 M3 — `custom` is an operator-supplied transformation the service APPLIES; using it on a real discrepancy to fake a no-op would either fail the handler's payload-validation contract or record a meaningless operator_resolved_ambiguity audit row that contaminates production audit trail). Acceptance-test mechanic (revised): **(a)** the gate FIRST exercises the parse-time payload-required error case on a real DHC/VSAT discrepancy — operator runs `resolve-ambiguity <id> --choice consolidate_using_operator_vwap --reason ...` (INTENTIONALLY omitting `--custom-value`) + verifies CLI exits non-zero with a clear error naming the missing required flag; **(b)** the gate THEN proceeds to the operator's actual disposition (whatever it is — `keep_journal_as_is` if operator's data calls for it, OR a payload-required choice with real `--custom-value` if operator's data calls for that); **(c)** an ADDITIONAL fixture/test-only run exercises the payload-applied happy path against a SYNTHETIC scratch discrepancy planted at gate-time (operator runs `swing journal reconciliation insert-test-discrepancy` per a NEW gate-only utility CLI OR via test fixture directly — writing-plans drafts) + dispositions it with `--choice operator_truth --custom-value '{"price": 1.23}'` + verifies the `reconciliation_corrections.applied_value_json` row contains `{"price": 1.23}`. The synthetic discrepancy is then cleaned up post-gate. The acceptance-test surface is exercised; the production DHC/VSAT cases get their REAL operator disposition without contamination.
- S7: `swing journal discrepancy show-ambiguity 40` then `resolve-ambiguity 40 --choice <picked> --reason ...` for VSAT 40 per §10.3 (tier-2 case A/B/C dispositioned per actual Pass-2 outcome). Same payload-contract acceptance test condition as S6 applies — at least one of S6/S7 must exercise a payload-required choice with `--custom-value`.
