...
> **Terminology (Codex R1 m#1) -- used consistently hereafter:** there are **5 total renderers**; **4 are candlestick "detail" adopters** (`ticker_detail`, `position_detail`, `market_weather`, `theme2_annotated`) and **1 is the line "thumbnail" surface** (`watchlist_row`, + future P14.N1 thumbnails which reuse it). "Detail surfaces" = the 4 adopters; "thumbnail surface" = the 1 line renderer.
...
mplfinance is a pure-Python matplotlib wrapper that emits matplotlib Figures (serializable to SVG) -- it satisfies Sec 9.1 Q5 ("matplotlib SVG only; no JS"). It is **already a declared dependency** (`pyproject.toml` `dev` + `charts` extras; installed `0.12.10b0`) and **already used in production** at `swing/rendering/charts.py:render_chart`. The packaging change is adding it to the `web` extra so a `pip install -e ".[web]"`-only deployment of `swing web` has it (the documented profile `[dev,web]` already includes it transitively via `dev`).

> **Packaging completeness (Codex R1 M#10).** Today `swing/web/charts.py` runs without mplfinance (line renderers only); after this sub-bundle it imports mplfinance at module load, so EVERY profile that runs `swing web` must have it. Writing-plans MUST: (a) audit all supported install/run profiles (`[web]`, `[dev,web]`, any requirements/lock/container path) and add `mplfinance>=0.12` wherever `swing web` can be launched; AND (b) mirror the EXISTING guard pattern in `swing/rendering/charts.py:85-89` -- a clear, ASCII `RuntimeError`/`ChartingUnavailableError` at the import boundary (the module already hard-imports matplotlib with a `RuntimeError` fallback at `charts.py:49-57`; mplfinance follows the same shape) so a missing-dep deployment fails loudly at startup, not with a deep stack trace on first chart render. Decide at writing-plans whether the web SVG path hard-fails or degrades to the line renderer when mplfinance is absent (recommend hard-fail at import -- it is a declared runtime dep, and silent line-chart degradation would mask the very candlestick regression this sub-bundle fixes). **(Codex R2 m#1) -- blast radius + smoke test:** because `swing.web.charts` is imported by route/VM modules, a missing-mplfinance import failure breaks EVERY page that imports it, not just chart routes. Add a smoke test that imports `swing.web.charts` under the actual `[web]` profile (and asserts the dashboard/app module imports cleanly) so a packaging miss fails in CI, not in production at first page load.
...
**mpf axes layout (with `volume=True`).** `mpf.plot(df, returnfig=True, volume=True, ...)` returns `(fig, axes)` where `axes` is a list; conventionally `axes[0]` is the price (candle) panel and `axes[2]` is the volume panel (mpf inserts a twin y-axis at `axes[1]`/`axes[3]` for right-side labels). **The helper MUST NOT hard-code `axes[2]` (Codex R1 m#6)** -- mpf's axis count/order shifts with `style`, extra panels, or secondary axes. The helper resolves the volume axis by introspection (e.g. the panel whose `get_ylabel()`/configured `ylabel_lower` is the volume label, or the lowest non-twin panel when `volume=True`) and returns an explicit `(fig, price_ax, vol_ax)` triple with named handles -- callers never see an mpf index. The T4.SB Item 3 volume y-tick strip becomes `vol_ax.set_yticks([])` on that resolved axis. **Pin tests:** assert the helper returns a non-None `vol_ax` when `volume=True`, that it is identified by label/role (not a fixed index), and that its y-tick labels are empty.

**Single coordinate helper for overlays (Codex R1 M#7).** Rather than scatter `bars.index.get_loc(pd.Timestamp(d))` positional assumptions across every renderer, ALL date->x conversion routes through one helper `_x_for_date(price_ax, bars, date) -> int|float`. V1 returns the integer bar position (mpf positional x-axis); the helper is the SINGLE place coupled to mpf's coordinate convention, so an mpf upgrade that switches candles to date coordinates is a one-function change + its pin test, not a renderer-wide break. Pattern-window `axvspan`, fill markers, and any vline use it. **Pin test:** assert `_x_for_date` returns the expected integer for a known date in a known fixture (the contract pin).
...
**Figure DPI / sizing.** mpf accepts `figsize` (inches) -- pass `_figsize_inches(_<SURFACE>_SIZE_PX)` so pixel dimensions match today's surfaces. `_svg_bytes_from_fig(fig)` (with `bbox_inches="tight"`) serializes the mpf-returned fig unchanged; `plt.close(fig)` discipline preserved. **(Codex R1 m#5 -- viewBox drift):** `bbox_inches="tight"` recomputes the figure extent, and mpf's panel layout differs from the old `plt.subplots`, so the emitted SVG `width`/`height`/`viewBox` will likely CHANGE vs today's line charts. Writing-plans MUST capture the actual SVG `width`/`height`/`viewBox` per surface post-conversion and reconcile with the template/CSS embedding (the `.ticker-detail-chart` / thumbnail containers) so charts are not clipped or mis-scaled in the browser -- this ties directly to the §4.5 embedding diagnosis (a viewBox change could itself be the V2.G1 "not displaying" cause). The operator-witnessed gate (§11.3 S3-S6) confirms final scaling.

### §4.2 Data-shape precondition (no escalation)

mpf requires `Open/High/Low/Close` columns + a DatetimeIndex. The `bars` DataFrame is `OhlcvCache.get_or_fetch` / `_bars_or_none` output -- the SAME archive shape `render_chart` already consumes successfully. So **no OHLCV-bar-shape change is forced** (the brief §7 escalation trigger does NOT fire).

**A normalization/validation barrier precedes every `mpf.plot` (Codex R1 M#8).** mpf is sensitive to frame shape; cached bars can in practice arrive unsorted, with duplicate timestamps, a timezone-aware index, lowercase column names, or a `group_by='column'` MultiIndex (the documented yfinance footgun). A shared `_normalize_ohlc_for_mpf(bars) -> pd.DataFrame` runs before plotting and: (a) squeezes MultiIndex columns (extends the existing `_close_series` defense to all OHLC columns); (b) Title-cases / maps column names to `Open/High/Low/Close/Volume` -- **failing fast with a typed error if title-casing would produce DUPLICATE canonical names (e.g. both `close` and `Close` present)** rather than silently picking one (Codex R2 m#2); (c) sorts the index ascending; (d) drops duplicate timestamps (`keep='last'`, mirroring the archive `write_window` convention -- gotcha #26 family); (e) makes the index tz-naive; (f) raises a typed, ASCII error if required OHLC columns are absent (NOT a deep mpf `KeyError`). **Tests** for unsorted, duplicate-timestamp, tz-aware, MultiIndex, lowercase-column, and missing-column inputs (the last asserts the typed error). The thumbnail (line) renderers keep using `_close_series`/`_volume_series`.
...
All title/label/annotation text routes through the existing `_assert_*` gates. **Two distinct gates, not one (Codex R1 m#10 -- the spec must not conflate them):** (a) the TITLE/suptitle gate (`_assert_title_no_math`) forbids `$`/`^`/`_`/`\` -- pattern slugs with `_` (`flat_base`, `cup_with_handle`) MUST NOT flow into titles; (b) BODY annotations via `ax.text(...)` only require `_assert_ascii_only` (ASCII, no math-metachar check) -- matplotlib treats `_`/`^` as LITERAL characters in body text outside `$..$` math mode (verified empirically + documented at `charts.py:90-98`), so `ax.text(0.98, 0.95, "flat_base")` is SAFE and is the existing mechanism for rendering slugs. So: underscores are FORBIDDEN in titles, ALLOWED in `ax.text` body annotations. Candlestick adoption introduces no new user-facing strings except the BULZ zone legend (§7, ASCII, no metachars) and any axis labels mpf adds. Manual visual verification of rendered text is non-optional (operator-witnessed gate).

### §4.5 V2.G1 "not rendering" sub-symptom (writing-plans investigation item)

The candlestick adoption closes the *type* expectation. The orthogonal "hyp-rec/watchlist expanded charts do not display while position_detail does" sub-symptom (V2.G1 detail block bucket (c): template CSS/sizing clipping) is browser-only and not reproducible in TestClient.

**This diagnosis runs EARLY -- before/at the START of renderer conversion, NOT after (Codex R1 M#15).** If the operator's real complaint is invisible/clipped SVGs (a template embedding bug), converting to mplfinance does NOT fix it and could even worsen it (mpf + `bbox_inches="tight"` changes `width`/`height`/`viewBox` -- see §11 m#5). So **T2 begins with** an operator-witnessed capture of the failing surface in a real browser: diff the embedding of `ticker_detail`/`watchlist_expanded` SVG (in `hypothesis_recommendations_expanded.html.j2` + the watchlist expanded partial) against the WORKING `position_detail` embedding (DOM inspect: container CSS, `overflow`, `max-height`, SVG `viewBox`/`preserveAspectRatio`). Capture the minimal failing route. If an embedding/CSS bug is confirmed, FIX IT FIRST (separate commit) so the candlestick conversion lands on a surface that actually renders. The decomposition (§10) sequences this as T2-step-0. The operator-witnessed gate at executing-plans is the verdict (browser-only).
...
> **DDL must be derived from the LIVE v22 schema, not hand-recreated from the `0020` excerpt (Codex R1 M#1).** This is an **authoring-time** step, NOT runtime generation (Codex R2 M#2): the `0023_*.sql` migration stays a STATIC SQL file -- writing-plans READS the live v22 `chart_renders` definition (`SELECT sql FROM sqlite_schema WHERE name='chart_renders'` against a migrated-to-v22 fixture DB) and PASTES the derived DDL into `0023_*.sql`, changing ONLY the `surface` CHECK enum value. The migration does not introspect the DB at apply time. This guarantees no column default, FK action (`ON DELETE CASCADE` on `pipeline_run_id`), `AUTOINCREMENT`, column order, or constraint is silently dropped.
>
> **Schema-parity assertion test (Codex R2 M#1 -- PRAGMAs are insufficient).** `PRAGMA table_info`/`index_list`/`index_info`/`foreign_key_list` do NOT capture the table `CHECK` expression text or the partial-index `WHERE` predicates -- exactly the risky parts of the rebuild. The parity test therefore compares the **normalized `sqlite_schema.sql`** strings (whitespace/case-normalized) for `chart_renders` AND each of its 3 partial indexes, pre-migration (v22) vs post-migration (v23), asserting they are IDENTICAL except for (a) the table name during the intermediate rebuild and (b) the single `hyprec_detail` -> `ticker_detail` enum token. The PRAGMA comparisons are retained as a secondary cross-check. The SQL block below is ILLUSTRATIVE of the shape; the binding source is the derived live schema.

> **`BEGIN;...COMMIT;` is the proven migration shape (Codex R1 M#3).** The runner's `_apply_migration` (`db.py:214-222`) runs the file via `conn.executescript()` with `PRAGMA foreign_keys=OFF`, which issues an implicit COMMIT before the script; migration `0022_phase14_temporal_log.sql` already wraps its body in explicit `BEGIN; ... UPDATE schema_version SET version=22; COMMIT;` and ships green. The v23 migration mirrors that exact structure -- NOT a nested transaction (the connection is in autocommit at `executescript` entry per the `db.py:209-212` docstring). The rollback-through-runner test (§5.3) proves the discipline on an injected mid-script failure (gotcha #9).
...
- **FK-enforcement-restored test (Codex R1 M#2).** `_apply_migration` saves `prior_fk`, sets `foreign_keys=OFF`, and is DOCUMENTED to restore the prior value after (`db.py:195-216` docstring + `db.py:209-212`). Writing-plans MUST (i) read the full function past the `raise` at line 222 and CONFIRM the restore fires on BOTH the success path AND the except/rollback path (the excerpt in this spec ends at the `raise`); (ii) add a runner-level test asserting `PRAGMA foreign_keys` equals its pre-migration value after a successful v23 migration AND after a forced mid-script failure+rollback. A migration that silently leaves FK enforcement OFF would let later writes corrupt `pattern_detection_events.chart_render_id`.
...
**L5 gate -- two-tier, not one blunt grep (Codex R1 M#14):**
- **Runtime-FORBIDDEN paths** (must return ZERO `hyprec_detail`/`render_hyprec_detail_svg`/`hyprec-detail-chart`): `swing/**` EXCEPT `swing/data/migrations/0020_*.sql` (frozen history) and `swing/data/migrations/0023_*.sql` (which legitimately contains the old token in the `CASE WHEN surface='hyprec_detail'` rename clause), plus `swing/web/templates/**`.
- **ALLOWED / EXPECTED paths** (old token MAY appear, by design): the `0020` + `0023` migration SQL; tests that assert the migration RENAMES the old value or that the dataclass validator REJECTS `'hyprec_detail'` (these MUST reference the literal); historical `docs/**` (provenance -- not rewritten).
- The gate is a test with explicit allowlist globs, not a bare repo-wide grep. **m#2 (template filename):** the file `hypothesis_recommendations_expanded.html.j2` keeps its name -- it describes the hyp-rec card it serves (a real domain surface), not the chart surface enum; renaming it would ripple to every `{% include %}` for zero semantic gain. The L5 token gate targets the `hyprec_detail` *surface string* + renderer name + CSS class, NOT the template filename. This exemption is explicit (the filename is not an orphaned surface reference).

### §5.6 Neutral, cache-safe title (kills the leakage)

V2.G2 root cause: `render_hyprec_detail_svg` builds `suptitle = f"{ticker} | hyp-rec detail | last {len(close)} bars"` (`charts.py:275`), and this surface is reused for the watchlist-expanded chart -- so a watchlist expand shows "hyp-rec detail".

**Resolution (Codex R1 M#4 -- a single cached SVG cannot carry caller-specific titles).** Because the `ticker_detail` surface caches ONE SVG row per `(ticker, surface, pipeline_run_id)` and BOTH the hyp-rec-expand and watchlist-expand callers read that same row, the title MUST be **caller-agnostic** -- it cannot embed a per-caller label without either (a) thrashing the cache, or (b) showing the wrong caller's label. The fix is therefore a **neutral title that names neither surface**: `f"{ticker} | last {N} bars"` (drop the "hyp-rec detail" / any surface-descriptor segment entirely). This kills the leakage (no surface name in the title) AND stays cache-safe (identical bytes for both callers). The renderer takes NO `context_label` kwarg. ASCII-gated.

> An EARLIER draft proposed a caller-supplied `context_label`; that is REJECTED -- it is incompatible with the single-cached-row contract (§6) and would reintroduce cache thrash or wrong-label display. If a future requirement genuinely needs per-caller titles, that forces per-caller cache keys (a separate surface) -- out of V1 scope.

> **Cache-key caution (L3):** the title is fixed by `(ticker, N_bars)` which is identical for both callers given identical bars. The cache key stays `(ticker, surface, pipeline_run_id)` for the run-bound `ticker_detail` surface; nothing caller-specific enters `source_data_hash`. Discriminating test: two callers, identical bars, assert ONE cached row + byte-identical SVG (call_count parity).
...
- **Cache-key decision:** do NOT add new surface enum values (`open_position_thumbnail`/`hyprec_thumbnail`) in V1 -- that would require a SECOND v-bump and widen the rename blast radius (YAGNI; no operator requirement for divergent thumbnails today). Reuse the `watchlist_row` surface with the existing run-bound key `(ticker, surface, pipeline_run_id)`.
- **Thumbnail identity contract (Codex R1 M#11) -- documented explicitly so the reuse is safe + the divergence boundary is known:** a `watchlist_row` thumbnail is defined SOLELY by `(ticker, pipeline_run_id)` and renders identical bytes regardless of which table embeds it. Contract: **dimensions** 200x100; **MA overlays** the caller `ma_lines` (default `[20,50]` per `chart_jit._WATCHLIST_THUMBNAIL_MA_LINES` -- both callers MUST pass identical `ma_lines`, Expansion #10c); **chart type** close-line (OD-2); **asof/run binding** the run-bound key; **NO per-table badges/overlays** (a thumbnail carries no entry/stop/target/flag markers -- those belong on the detail surfaces). **Invalidation:** a new `pipeline_run_id` produces a new row (the existing run-bound semantics). **Divergence boundary (explicit deferral):** IF Sub-bundle 4 needs table-specific thumbnails (different dimensions, an open-position P/L badge, a hyp-rec tier marker), it introduces distinct surface enum value(s) THEN (a v24 rename), at which point the identity contract above is the migration's starting point. Sub-bundle 3 does NOT speculatively add those enums.
- **Deliverable:** the thumbnail renderer is confirmed reusable + the `get_or_render_surface(surface="watchlist_row", ...)` JIT path is the substrate the row VMs will call in Sub-bundle 4. Sub-bundle 3 adds a test proving a non-watchlist ticker renders + caches a `watchlist_row` thumbnail via the JIT path. **(Codex R1 m#9):** that JIT test proves renderer+cache reuse but NOT that an open-position/hyp-rec row VM can OBTAIN the right `(ticker, bars, pipeline_run_id)` context to call the substrate. Sub-bundle 3 therefore ALSO adds a thin VM-level test asserting the open-position + hyp-rec row VMs expose (or can resolve) the ticker + run binding the thumbnail substrate needs -- so Sub-bundle 4's row-template wiring is unblocked, not blocked on a missing VM field. (Still substrate-only: no row TEMPLATE changes in V1.)
...
- **Long-position-only in V1 (Codex R1 M#12).** The zones assume `stop < entry < target` (a long). BULZ is long; the project's trade model is long-biased. V1 draws zones ONLY when the long-shape invariant holds; for a truly **invalid/unsupported shape** -- short position, `stop >= entry` (a stop raised above entry into profit), `target <= entry`, or missing/zero entry/stop/target -- the renderer SKIPS that zone + WARN-logs (per-ticker isolation; never raises). It does NOT draw an inverted band.
- **Off-range valid zones are DRAWN, not silently skipped (Codex R2 M#3).** A geometrically-VALID zone whose level sits outside the visible close range (a distant target, a far stop) is NOT silently hidden -- silently skipping it removes the very risk/reward context the operator opened the chart to inspect, and changes semantics. Instead `axhspan` is drawn for the valid level (which intentionally autoscales the price y-axis to include it) OR, if writing-plans finds autoscaling compresses the candles unacceptably, an edge marker + a legend note ("target above range: $X") is rendered. The binding rule: **never silently drop valid long-only entry/stop/target geometry**; only invalid shapes (above) are skipped+logged. Writing-plans picks autoscale-span vs edge-marker at the operator-witnessed gate.
- **Data source (open-trade row):** entry = **average entry fill price computed from `fills`** via a tested formula (sum(entry-leg `price * shares`) / sum(entry-leg `shares`)) -- handles partial/multiple entries; fall back to `trade.entry_price` only if denormalized + no fills. stop = `current_stop` (already a renderer param). target = the trade's target/objective field. **Writing-plans MUST verify the exact `Trade`/`Fill` field names** (#4 SQL/field verification) -- candidates: `trade.target_price` / a derived R-multiple target / `trade.entry_price`. If no target field exists, V1 draws only the risk zone + documents target-zone as deferred (do NOT invent a target).
...
- **asof_date is anchored on the RENDER's data context, NOT wall-clock `now()` (Codex R1 M#5).** A cached weather SVG must be consistent with the bars it draws + reproducible in tests/backfills. Per call site:
  - **Pipeline (`runner.py:2882`):** use the run's `data_asof_date` (the bars' last-completed session for that run) -- the same anchor the run uses elsewhere. NOT `now()`.
  - **JIT (`chart_jit.py`):** use the `data_asof_date` already passed into `get_or_render_surface` (it is a required param) -- consistent with the bars the JIT path fetched.
  - **Interactive refresh (`dashboard.py:117`):** `last_completed_session(datetime.now())` IS correct here -- it is a true interactive refresh with no better anchor, and it already matches the handler's own `data_asof_date` write at `dashboard.py:127`. Honors "Weather lookup must NOT query by action_session" + "session-anchor read/write" gotchas.
...
- **Dependency-injection contract (Codex R1 M#6).** `current_stage(conn, ticker, asof)` needs a DB connection + benchmark + a populated `weather_runs` row. Writing-plans MUST define the exact signature + DI point at EACH site and confirm each has `conn` + `cfg` available:
  - Pipeline + refresh handler already hold `conn` + `cfg`.
  - The JIT path (`get_or_render_surface`) receives `conn` (it already writes via `refresh_chart_render`) -- thread `cfg.rs.benchmark_ticker` in OR compute the state in the CALLER and pass it as the `trend_template_state` renderer_kwarg (preferred -- keeps `chart_jit` surface-agnostic). Pin which at writing-plans.
- **Fail-soft at every site:** wrap in try/except mirroring `_compute_trend_template_state` -> fall back to `"undefined"` (ASCII; not `"n/a"`) + WARN log; never 500 the refresh / abort the pipeline step. **Tests for: missing `cfg`/benchmark, missing `weather_runs` row, no active pipeline run** -> all yield `"undefined"`, not a crash. **(Codex R1 m#3 -- `"undefined"` rendering):** the badge text reads `trend: undefined` so it is visibly distinct from a real stage (`stage_2`) -- it is an honest "not computed", not a fake state; the WARN log carries the reason (which precondition was missing). This matches the existing `review_form` convention (`"undefined"` is already the documented fallback there). No special styling required in V1; the literal word + the log are the affordance.
...
> **OQ-5** (resolved by OD-3): real source = `current_stage(conn, benchmark, <render-context asof>)`. Pipeline + JIT also switch to this (closing the V2-banked literal); confirm `current_stage` is import-safe in the pipeline/runner context (it reads weather rows; no Schwab/yfinance call -- L6 preserved).
...
**A per-surface annotation layout policy, not an ad-hoc move (Codex R1 M#13).** Option (c) can itself collide: the upper-right region already holds the pattern-class slug (`0.98, 0.95`) + exemplar footnote (`0.98, 0.02`), and a multi-line `_annotate_*` block plus a zone legend (§7) plus 5 MA legend entries can overrun on the smaller surfaces. Writing-plans defines a SMALL reserved-region map per surface (e.g. legend=upper-left, pattern slug=top-right corner, annotation stack=right-edge starting `0.92` descending, footnote=bottom-right) and the `_annotate_*` family writes into the annotation-stack region with a bounded line budget. **Visual checks use worst-case inputs:** the pattern with the most annotation lines (vcp contraction list), longest labels, AND the full MA set, on the SMALLEST adopting surface, to prove no overlap. (theme2_annotated is 800x600 single-axis -- the roomiest; market_weather 400x150 is the tightest and carries only the trend badge + MA lines, no `_annotate_*` stack.)
...
| **T7** | Closer: pyproject `web`-extra mplfinance + full-suite + ruff + L2 source-grep verify + operator-witnessed visual gate enumeration | Lint uses the repo's canonical command `ruff check swing/` (0 E501) per CLAUDE.md Quick Start; if repo policy lints `tests/` too, mirror it (Codex R1 m#7) -- do not narrow below the established scope. Est. ~15-25 commits / ~40-80 tests is an INDICATIVE envelope (Codex R1 m#8 -- not a churn target; task boundaries are risk-based per T1-T7). |
...
### §11.2c Operationalizing the binding visual gate (Codex R1 M#9)

The visual gate is the strongest correctness check and MUST NOT be left as an easily-skipped "look at it." Executing-plans MUST produce, per changed surface, a concrete artifact + command so the gate is reproducible + recorded:

- **Artifact:** a rendered SVG (and a PNG for diffing) written to `exports/diagnostics/phase14-sb3/<surface>-<ticker>.svg` for each changed surface (`ticker_detail`, `position_detail`, `market_weather`, `theme2_annotated` per pattern class, `watchlist_row` thumbnail unchanged-baseline).
- **Command:** an exact, documented invocation (a small repro script or `python -m` one-liner that calls the renderer on a fixed fixture ticker/bars) recorded in the executing-plans return report -- so the operator regenerates the artifacts deterministically.
- **Route equivalents:** the live routes the operator opens (`/dashboard`, hyp-rec expand, watchlist expand, `POST /dashboard/weather-chart/refresh`, a `/patterns/{id}/review` flat_base case).
- **Checklist + ownership:** a per-surface acceptance checklist (the §11.3 ladder) recorded in the executing-plans return report / PR, with the operator as the named gate owner; merge is BLOCKED until each row is checked (Sec 9.1 Q6).
- **Baseline storage:** the pre-change rendered bytes are NOT a regression baseline (the change is intentional); instead the artifacts are kept as the post-change reference the operator signs off on.
...
| **OQ-mav-color** | Centralized MA color set -- which 5 colors? | Pin the colorblind-safe palette in the helper at writing-plans BEFORE implementation (Codex R1 m#4 -- MA distinguishability is central to the visual gate, so colors are not a closer detail); ASCII-safe; distinct. |
