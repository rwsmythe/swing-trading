...
## §8 The post-ritual addendum must re-verify

Suite green + probe on the post-ritual head (the ritual's sweep is a moved-path event — D21 binds: tests-grep before, suite after); the docs count dropped; CLAUDE.md line-3 recompacted with the streak figure refreshed (4,253 at this audit); the §6 charter log compacted if near ceiling. The audit is CLEAN subject to that addendum.
