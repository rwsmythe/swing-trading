...
**F1 — the guard first (the exemplar corpus is a MEASUREMENT INPUT). CHOSEN, and it is the FIRST commit:** in the review confirm path, when `evaluation.pattern_class == "double_bottom_w"` and the decision is a confirm without an operator-corrected start, `start_date` is derived from `structural_evidence_json["trough_1_date"]`, never from `window_start_date`. If the evidence lacks a parseable `trough_1_date` (a zero-envelope row cannot be confirmed as a W in any case) the route REFUSES the confirm with a typed, named-recovery message (use `pattern_present_outside_window` with a corrected start) — authorize-then-abort rule: the service refuses before any write. Test: confirm a DBW evaluation whose `window_start_date` is trough 2 and whose evidence carries trough 1 → the exemplar row's `start_date` == trough 1; the boundary twin: `pattern_present_outside_window` with a typed start still wins. Rejected: a process instruction alone (the guard class this project keeps losing).
