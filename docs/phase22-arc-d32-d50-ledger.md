...
### F4 disposition -- out-of-inventory root files (CHARC by the operator's word, 2026-09-15 18:1xZ)

Operator-directed in the CHARC session ("Delete the may image and the orphan wal"). Looked before deleting: `swing-CORRUPTED-post-phase7-migration-20260505T162747Z.db` (7,188,480 bytes, 2026-05-05) opened read-only on plain sqlite3 -- `schema_version` 14, `integrity_check` ok; schema 14 has other images in the inventory (RD's QA rows 45/48), so not a sole image. `s9step0-copy.db-wal` (0 bytes, 2026-09-02) and its sibling `s9step0-copy.db-shm` -- the `.db` they belonged to is gone; both are orphans of the S9 step-0 copy. Deleted all three with `rm`; verified absent afterwards. Neither was in the inventory's scope (R0-6 scoped the root scan to `swing-pre-*.db`); this record is the only place they are named.
Addendum, same sitting: my read-only probe of the May image RECREATED `…162747Z.db-shm` (32 KB) and `…162747Z.db-wal` (0 bytes) beside it -- the CLAUDE.md gotcha exactly (a read-only connection recreates the `-shm` on open and cannot delete either file on close). Both were orphans of a file I had just removed; deleted them too, verified no `swing-CORRUPTED*` or `s9step0*` remains. Root now holds `swing.db` and the two Schwab token DBs only.
