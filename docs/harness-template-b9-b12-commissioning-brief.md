...
- **Gap 2 (build-gate; DEPENDS on B-9):** a new director's `<role>-context.md` + `<role>-state.md` carry domain vocab → fail the genericity guard + manifest unless registered in `genericity_lists.py::INSTANCE_SURFACE_RELPATHS` (`charc-state.md` already is; `<role>-state.md` is its exact parallel). Register the pattern + add the §8 checklist line. (This rides B-9's instance-surface model.)
