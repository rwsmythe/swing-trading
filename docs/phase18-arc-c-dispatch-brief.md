...
**[DISCHARGED 2026-06-14 — CHARC architecture seat CLEARS.]** C1–C4 verified independently on the shipped diff (branch `phase18-arc-c`, baseline `30a4b838` → HEAD `0206f9d9`):
- **C1 ✓** — the diff touches NO measurement-logic module (validate / hypothesis / evaluation / candidates / temporal_metadata / pattern_detection / weather / trades); `_record_yf_download` (yfinance_audit.py:220-269) returns the EXACT `fetch_fn()` on every path (fetch called once; timer + post-fetch classification wrapped; `busy_timeout=0` on start + finish); the runner change is an observational OUTERMOST wrap (`runner.py:628`) with a non-throwing degrade-to-disabled entry (`runner.py:551-563`); migration 0030 is additive-only.
- **C2 ✓** — Task 0 is ONE commit (`9d4f7ad7`); the full §9 SET NULL 7-point delta landed: FK `ON DELETE SET NULL`, SQL run-linkage CHECK DROPPED, dataclass run-linkage validator DROPPED + all others KEPT (enum / bool-is-int / numeric / shape / non-empty-ticker), context-install guard KEPT (the relocated R12 catch) importing the shared `_YFINANCE_VALID_SURFACES` (no copy).
- **C3 ✓** — no functional `environment` read (only a docstring naming the no-gate design); records under sandbox.
- **C4 ✓** — strict `current_version == 29` backup gate (NOT `<=`); `EXPECTED_SCHEMA_VERSION = 30`; in-file `BEGIN; … COMMIT;` + `version = 30`; FK → `pipeline_runs(id)`; migrate-twice no-op test present.

**Three eyes converged** (orchestrator QA PASS + RD measurement-integrity PASS/merge-unblocked + CHARC C1–C4 PASS). **Merge conditions (all in the orchestrator merge plan):** (1) **MANDATORY** — reword `1bf9f511`'s `Runner:` body to plain prose so `%(trailers)` is `[]` across the range (the zero-`Co-Authored-By` streak is INTACT — scan-confirmed; this is trailers-audit hygiene, `feedback_commit_message_trailer_parse_hazard`); (2) binding no-false-green full fast suite on the MERGED head + ruff; (3) operator-witnessed v29→v30 live-migration + backup gate. **Squash-vs-preserve the iteration commits: PRESERVE** (CHARC custodian call — history surgery on a converged, gate-passed branch exceeds the cosmetic benefit; the commits are conventional + trailer-clean post-reword); adopt the forward recipe refinement (writing-plans commits the plan ONCE at convergence) instead.
