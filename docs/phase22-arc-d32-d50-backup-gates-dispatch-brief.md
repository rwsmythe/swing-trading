...
**Author:** CHARC, 2026-09-14. **Status:** COMMISSIONING DRAFT — two forks (§2 F2, §2 F4) await the OPERATOR's ruling in chat; dispatch after. **Executor:** orchestrator → implementer cell (**opus-high minimum**: `swing/data/db.py`, migration-adjacent, a wrong gate is confident and only found at the next live migration).
...
- `swing/cli.py` `db_migrate`: pass `backup_dir`; F2's conditional copy IF the operator chooses (b); echo both paths when both fire.
