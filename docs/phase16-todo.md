...
**Candidate root causes of the +$100.44 journal undercount (the arc decides — possibly several):**
1. **Missing Schwab cash credit not recorded as a `cash_movement` (strong suspect).** Only 3 cash_movements exist, ALL deposits — **no interest or dividend rows.** Schwab pays sweep interest on the ~$2k cash (held for months) and equities pay dividends; ~$100 of uncredited interest/dividends would exactly explain the journal undercounting the broker. Directly relates to the banked **"Schwab since-inception Account Statement ingestion"** (seed `cash_movements` from the Schwab statement incl. interest/dividends).
2. **Realized-P&L computation error.** The journal computed `−$73` realized across 16 trades; if fees/commissions or fill prices are off, the realized could be understated by ~$100.
3. **`starting_equity` baseline** ($1200, config) vs the true account inception value.
4. **NLV vs settled/unsettled** timing on the recent closes (NLV at snapshot time vs the ledger's settled view).

- [ ] **4a — Pin the cause.** Reconcile the three numbers: `current_equity` ($1927) vs the Schwab snapshot ($2027.44) vs the live balance. Check for missing interest/dividend `cash_movements` (the Schwab Account Statement — the prime suspect for the ~$100) and audit the realized-P&L-from-fills computation against the broker. Likely a combination.
- [ ] **4b — Add an account-equity RECONCILIATION (the actual gap behind "not reconciling").** Build the equity analogue of the trade reconciliation: compare the journal-computed `current_equity` to the Schwab snapshot/live balance, flag drift beyond a tolerance, and help attribute it (missing cash movement vs P&L error). Decide what the ACCOUNT tile SHOULD authoritatively show (journal ledger vs broker), and reconcile that the tile and the sizing-denominator currently read DIFFERENT equities.
