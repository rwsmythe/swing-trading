...
- **~~THREE research-side instances of the run-level-stamp shape (gotcha #30)~~ — DISCHARGED 2026-07-28, ahead of the deadline. Trace: [`docs/rd-d27-provenance-trace.md`](rd-d27-provenance-trace.md) (`ff405567`).** Result: **the August read's chain is measured CLEAN on this axis** — proceed without qualification.
  ...
  - **D27 is a NAMING defect, not a value defect.** The two columns agreed **113/113 across the FULL paired history** (not 25), including 17 runs started during US market hours — and **the agreement is STRUCTURAL, not a healthy-nightly coincidence**: both bar consumers cap at the clock session (`PriceFetcher.get` slices `<= effective`; `OhlcvCache.get_or_fetch` strips `> end`). **Refinement that matters: the DANGEROUS direction (bar-derived NEWER than clock-derived) is impossible by construction; only the benign direction is unenforced.** The 21-G survey's §0 headline states the uncertainty symmetrically — worth correcting before it lands. Still worth fixing the name (two quantities, one column; my monitors read both). Scoped to the Phase-21 close or Phase 22.
