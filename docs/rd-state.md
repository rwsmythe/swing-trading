...
- **VSTS did NOT re-qualify A+** — `watch`, pivot risen 15.06 → 16.90; it ran ~11% without re-entering the gate.
...
- Phase-21 scoping comes via the fresh CHARC generation. Inputs now: **the latch dashboard (operator-declared 07-23)** + D5 suite-runtime (the audit's headline candidate) + my riders (0032 taxonomy type; execution↔fill identity; the bound-b stats choke point). RD gates: the latch derivation rules + anything measurement-touching.
