...
## Pending RD
- **T5 FTRE: ANSWERED + VERIFIED 07-23** (vacation; order never fired — RIGHT through 07-21 incl. a ONE-CENT miss; but 07-22 CROSSED 18.34 with the order dead → no live entry while the SHADOW window will trigger it). **LATCH POSTURE ADOPTED 07-23 (operator):** A+ fire → GTC buy-stop-limit at the fire's pivot + buy-zone limit cap; latch clears ONLY on fill / setup invalidation / horizon (~20 sessions, the shadow-parity bound) — never bucket flicker. **FTRE latch live: a resting GTC buy-limit 18.89** (stop-limit broker-rejected at price 19.475 — the zone cap correctly held the operator out of a +6.2% chase; fill = `standard`, stop plan decided pre-fill; cancel on invalidation or at the ~mid-August horizon). **The latch dashboard is now an OPERATOR-DECLARED Phase-21 input** (posted CHARC `20260723T161518Z`; semantics settled, RD gates the derivation rules).
