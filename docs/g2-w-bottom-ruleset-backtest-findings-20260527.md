...
The implication: D2 Amendment 5's prior-arc outcome was SUBSTRATE-FRESHNESS-BOUND. The cohort fixture has drifted (max_observed_asof_date timestamps updated by an intervening regenerate-cohort pass), shifting some verdicts out of the 365d recency window. The 29 verdicts now in the substrate that closed for E are NOT the same 5 that drove the +1.220R Amendment 5 mean.

This is a substantive methodology finding: above-zero backtest outcomes on tight-window substrates may not REPLICATE under substrate refresh. Forward-binding lesson for any future research arc citing prior-arc outcomes: cite the SHA + N + window of the PRIOR substrate; do NOT assume current re-run will reproduce.
