...
**The pass (binding conditions):**
1. **Tripwire CROSSED** — a `LATCH_CLEAR_REASONS` widening. SQLite cannot edit a CHECK → a **table-rebuild migration** (copy the 0033 rebuild pattern; explicit BEGIN/COMMIT per gotcha #9; row-parity assertion; next-free number).
2. **Gotcha #11 binds in full:** the schema CHECK + EVERY Python mirror of the reasons tuple land in ONE commit — grep ALL of `swing/` for copies before authoring; the D29 lesson applies (a grep that names files returns only those files — grep the PATTERN).
...
**Phases:** writing-plans (real design surface: the state machine + default inversion) → executing. **Cells:** opus-xhigh planning / opus-high executing. **Gates:** RD merge-blocking (measurement) + operator witness on the latch surface (VSTS is the live worked example).
