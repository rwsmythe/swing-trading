...
- **OOF sentinel `ref`:** a deterministic, recognizable marker (e.g. an `oof:`-prefixed key encoding ticker + date) — NOT a real `transaction_id`. It (a) marks origin for the §4 matcher rule and (b) gives idempotency via `ux_cash_ref` — which MUST be **fail-loud, NOT a silent no-op** (RD merge-blocking measurement concern, 2026-06-19; CHARC-adopted): on a sentinel-ref collision the command REJECTS with a clear `ClickException` (actionable message) + an escape hatch (`--force`/disambiguator) that records a genuinely-distinct second same-key buy under a DISTINCT `oof:` ref. A silent "already recorded" no-op would SILENTLY DROP a legitimately-distinct second same-ticker/same-day OOF buy from the ledger (the #27 silent-ledger-loss drift this arc exists to prevent — the sentinel key is ticker+date, so distinct same-day buys collide). A bare re-run (no `--force`) is rejected loudly → no double-record. The exact sentinel format is a writing-plans detail; it MUST be shaped so it can never collide with a real Schwab `transaction_id`.
