...
**[DISCHARGED 2026-06-14 — CHARC architecture seat CLEARS.]** C-E1..C-E5 verified independently on the shipped diff (`7acae794..phase18-arc-e-exec`, 10 commits, trailers `[]`):
- **C-E1 ✓** — `sessions_behind` `+26/-0` in `dates.py` (additive; no existing fn changed), reuses `_NYSE.previous_session` (no fork), bounded walk, no calendar-day fallback.
- **C-E2 ✓** — monitor module-top imports = `sqlite3`/`dataclasses`/`datetime` ONLY (no pandas); readers lazy-imported inside the checks; the Schwab n/a short-circuit sits ABOVE the `cli_schwab` import (schwabdev off the bare/unconfigured path); read-only.
- **C-E3 ✓** — envelope 1:1 + HARDENED: `monitor='tool_health'`, `worst_of` by severity-rank (not lexical), `grey` rejected at construction, checks coerced immutable, `overall==worst_of(checks)` enforced at construction, bare-call `compute_tool_health(conn)` preserved.
- **C-E4 ✓** — ASCII (subprocess `decode('ascii')` test); freshness summary honestly states write-recency ("last WRITTEN N days ago"); the full tz chain correct (mtime→naive-Hawaii-local before the diff; `_normalize_now_to_naive_local` boundary; `_now_to_utc`/`_issued_to_utc` split).
- **C-E5 ✓** — scope = `scripts/` + `swing/monitoring/` (new) + the one additive `dates.py` helper + tests; NO `swing/data`/`trades`/`pipeline`/`web`, no migration, no pyproject (the lone `-1` is a test file). No measurement-chain touch.

Two-eye gate satisfied (orchestrator QA + CHARC). Remaining: **operator CLI witness** of the ASCII report vs the live DB + authorization; the orchestrator owns the no-false-green merged-head re-run + ruff + ff-merge + close. RD FYI (envelope conformed 1:1). **Live-DB note for the witness:** the monitor currently surfaces `weather_freshness = RED "no weather run recorded"` — EITHER the monitor correctly catching a real weather-collection gap (18-E justifying itself on day one) OR a false-read to investigate; the operator's witness + their knowledge of whether weather is running disambiguates. If real, it's a separate operational finding, NOT an 18-E code defect/blocker.

**Two rejected Codex MAJORs — correctly rejected (flagged-not-fixed):** "configured Schwab + no tokens DB → red" + "empty cache-dir → red" both contradict the ratified contract (§4.2 + Task 4 "never false-red on a missing input"). The implementer correctly did NOT re-design mid-execution.

**Banked future refinement (18-H candidate — NOT a blocker, NOT folded into 18-E):** "configured-but-tokens-absent → YELLOW" (client_id set, tokens DB gone). VALID — a coverage gap where the worst token state reads green/n/a, when `schwab_token_ttl` exists to catch a dying token. It's a CONTRACT CHANGE (revises §4.2), so it does NOT re-open this converged arc. CHARC lean: MILD (yellow is defensible, but an operator may legitimately run configured-yfinance-only — a genuine product call). A small follow-on touch (`_check_schwab_token`: split client_id-set + tokens-absent → yellow) if the operator sequences it.

**[REOPENED 2026-06-14 — the discharge above is SUPERSEDED. The operator live-DB witness caught a blocking false-RED defect OUTSIDE the C-E1..C-E5 scope.]** `get_latest(conn)` defaults to `ticker='QQQ'`; the live system records weather under `cfg.rs.benchmark_ticker='SPY'` (39 SPY rows, zero QQQ) → the monitor read no row → a FALSE `weather_freshness = RED` on a system whose weather is current (asof 2026-06-12). The arc unit test passed ONLY because its fixture seeded weather under the DEFAULT (QQQ) ticker — synthetic-vs-production drift (`feedback_adversarial_review_verify_data_shapes` + the synthetic-fixture-vs-production-emitter gotcha). Every production caller does it right: `get_latest(conn, ticker=cfg.rs.benchmark_ticker)`. **CHARC ownership (squarely):** (1) my §4.3 cited the bare `get_latest(conn)` — I named the weather gotcha but did NOT transcribe its full `ticker=` prescription, an incomplete reuse-citation that seeded the bug; (2) my C-E review verified the monitor LOGIC ("no row → red is correct") but NOT the data-shape ("does `get_latest(conn)` return the right row for THIS system's data") — the data-vs-DB gap, applied to a gate review. C-E1..C-E5 were accurate for what they covered but INCOMPLETE (no data-shape-vs-live-DB condition). The binding operator-witness gate — included FOR this failure mode — caught it; my review didn't. The other three families (pipeline/schwab/ohlcv) witnessed correct on the live DB.
**Fix (orchestrator-dispatched, reusing `.worktrees/phase18-arc-e-exec`) — CONCUR:** thread `cfg` into `_check_data_freshness`; weather → `get_latest(conn, ticker=cfg.rs.benchmark_ticker)`; `cfg` None → weather n/a green (the ratified degradation); a distinguishing test seeding weather under a NON-default benchmark ticker (fails pre-fix, passes post-fix — the production-shape fixture); re-run Codex to convergence on the full diff. Contained (`swing/monitoring/` + a test; the reader REUSED with the correct arg; no new file/dep/schema).
...
**[RE-DISCHARGED 2026-06-14 — CHARC seat CLEARS on the fixed diff.]** Codex Round-4 re-converged post-cap (`NO_NEW_CRITICAL_MAJOR`, substantive). **C-E6 verified on disk** (fix `d32ec4d4`): `_check_data_freshness` threads `cfg` + calls `get_latest(conn, ticker=cfg.rs.benchmark_ticker)` (the production SPY path; the bare QQQ-default gone); `cfg None → green n/a` (bare-call preserved); the distinguishing test `test_weather_uses_cfg_benchmark_ticker_not_qqq_default` seeds a CURRENT row under SPY ONLY + asserts green (RED pre-fix / GREEN post-fix — a correct both-ways test exercising the production read shape), and EVERY weather fixture now seeds under `cfg.rs.benchmark_ticker` (the synthetic-vs-production drift killed at the fixture). Contained to `swing/monitoring/tool_health.py` + 3 test files → C-E1..C-E5 preserved. **C-E1..C-E6 PASS.** Two-eye gate satisfied (orchestrator re-QA + CHARC). **The binding final leg is the operator RE-WITNESS against the live DB** (weather flips from the false RED to its true status — the real net, the lesson applied) + authorization → ff-merge + no-false-green merged-head re-run + ruff + close.
