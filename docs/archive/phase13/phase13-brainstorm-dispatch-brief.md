...
**AMENDMENT (2026-05-18 post-Phase-12.5-arc-close)**: Q4 operator close-tracking flag for watchlist symbols FOLDED INTO THEME 4 (operator-decided 2026-05-18 post-Q2-gate-PASS). Banked at `docs/phase3e-todo.md` "Item Q4" entry. **Use case (operator framing)**: operator looking at PTEN as top watchlist symbol for 5/19 process run; PTEN just closed past its pivot value → high-probability of opening a position when markets open. NOT flagged by hyp-rec (the existing algorithm doesn't elect it as a recommendation) but visually looks like a good candidate. Operator wants a visual mechanism to flag such symbols for personal close-tracking, persisting across pipeline runs even if the watchlist algorithm decides the symbol no longer meets criteria (false-negative guard). **Two sub-use-cases**: (a) at-breakout (PTEN today): symbol just crossed its pivot/trigger; immediate-action candidate; flag retains visual prominence on watchlist. (b) approaching-breakout: symbol trending in correct direction but not yet at pivot; flag breaks it out visually + ensures it's not dropped from the surface if the watchlist algorithm next-run decides criteria not met. **Architectural decisions for brainstorm Codex chain to resolve** (per phase3e-todo Q4 entry; brainstorm proposes + Codex reviews + operator confirms at brainstorm-output): (1) **Schema**: NEW column on existing table (likely `candidates` or `evaluation_results`) OR NEW table for operator-flag metadata. Likely v20 → v21 migration (Phase 13 already locks v19 → v20 per §1.5; Q4 might extend to v21 OR fold into v20). (2) **Setting / unsetting UI**: web UI toggle button on watchlist row? CLI command `swing watchlist flag <ticker> --close-track`? Both? (3) **Persistence semantics**: per-session? per-pipeline-run? persistent until operator clears? auto-expire after N days? auto-clear on operator-opens-position? (4) **Visual rendering**: badge on watchlist row (e.g., 🎯 or `[FLAGGED]` ASCII marker)? separate "Actively tracked" section above main watchlist? bold/colored row background? (5) **Filtering interaction**: operator-flagged symbol MUST be retained on watchlist surface (false-negative guard) even when algorithm would drop it. (6) **Relation to hyp-rec**: do flagged symbols get hyp-rec treatment too (forced through scoring + expanded panel)? Or watchlist-surface-only? (7) **Audit trail**: per-flag-event row (operator set/cleared timestamp + ticker + reason text)? Brainstorm spec MUST surface design proposal for each + leave operator-decision-pending items at brainstorm §15.B style for orchestrator-pre-writing-plans triage.

T4.SB closer Sub-bundle implementation now bundled WITH the rest of the Theme 4 usability list. Estimated additional scope: ~+15-30 fast tests + likely schema work (column on `candidates` OR new `watchlist_close_track_flags` table) + 1 new toggle action on watchlist row + 1 visual surface (badge or section) + 1 new CLI subcommand (possible). Brainstorm Codex chain expected to add 1-2 rounds vs pre-amendment baseline.
