...
Operators may proactively clean up by deleting `~/swing-data/prices-cache/CVGI.schwab_api.parquet` (and any other `*.schwab_api.parquet` file with similar contamination). On the next pipeline run, the Schwab daily-kwargs path will recreate `{TICKER}.schwab_api.parquet` from scratch on success; the yfinance-fallback path persists to `{TICKER}.yfinance.parquet` (a separate Shape A file, NOT the `.schwab_api.parquet` artifact). The legacy `*.parquet` files are unaffected (always daily-bar from yfinance).
...
- **V2-F: Shape A archive backfill cleanup** — sweep the operator's `~/swing-data/prices-cache/*.schwab_api.parquet` files for contamination. Heuristic should be multi-signal: (a) duplicate `asof_date` rows (catches first-write minute-frequency contamination); (b) row-count vs unique-date-count ratio anomalies (catches partially-merged remnants where `write_window`'s `drop_duplicates(keep='last')` already collapsed intraday rows to one last-minute bar per date); (c) volume-scale heuristic per-row vs nearby legacy `.parquet` values (catches per-minute volumes orders-of-magnitude smaller than daily); (d) optional: audit `schwab_api_calls` history for `endpoint='marketdata.pricehistory'` calls that omitted period/frequency kwargs. Not necessary for V1 fix (next run's `write_window` merge with daily-kwargs payload resolves the duplicate-date case naturally) but operator-friendly UX for surfacing latent partial contamination.
