...
> **STATUS (2026-06-02): COMMISSIONED as the FIRST Phase-15 arc -- dispatched.** Phase 14 CLOSED 2026-06-02 at schema v23 (all SB1-SB5 + SB5.5 + the close-out polish + follow-on bundles SHIPPED end-to-end; ~7038 fast tests green; L2 LOCK zero re-anchor so far -- THIS arc is the first-ever baseline move). The operator deferred this through Phase 14 (path A: keep 2.5.1) and has now commissioned it. Scoping basis: `docs/schwabdev-v3-upgrade-investigation-findings.md` (`9d4f6a4`; orchestrator-verified, re-corroborated 2026-06-02 -- pin still `schwabdev>=2.4.0,<3.0.0` at `pyproject.toml:20`; installed 2.5.1; PyPI LATEST 3.0.5; L2 baseline still `bf7e071`; the L2 grep pattern still `schwabdev.Client.`). Tracked at `docs/phase3e-todo.md` `#9` + `#19`. (An earlier 2026-05-31 commit `d69f136` briefly framed this as a Phase-14 prerequisite; that was corrected -- this is Phase 15.)
...
**Mission:** Produce a brainstorming design spec for migrating the project off **schwabdev 2.5.1** to **3.0.5** (latest; confirmed on PyPI 2026-06-02). This is the **FIRST commissioned Phase-15 arc** -- Phase 14 is CLOSED, so this is NOT a close-out-tail item and SB5.5 has already SHIPPED. Chosen deliberately for v3's better data-handling + the optional Fernet token encryption (the pull-forward forcing function). **A deep scoping investigation already exists -- BUILD ON IT, do not re-investigate from scratch:** `docs/schwabdev-v3-upgrade-investigation-findings.md` (committed `9d4f6a4`; orchestrator-verified, re-corroborated 2026-06-02). Your job is to turn that findings doc into a migration DESIGN SPEC: the migration strategy, the token-storage rewrite, the breaking-change fixes, the **removal of the now-SHIPPED P14.N7 daemon-checker resilience wrapper + the disposition (delete vs rewire) of the SHIPPED F-1 web checker-liveness badge** (see L6/§2.6), the test strategy, the operator re-setup UX, the L2-LOCK re-anchor, and the design OQs.

**Brief:** `docs/schwabdev-v3-upgrade-brainstorming-dispatch-brief.md` (this file).

**Context:** Phase 14 CLOSED 2026-06-02 (SB1-SB5 + SB5.5 + the close-out polish + follow-on bundles SHIPPED end-to-end) at schema v23; main HEAD at this dispatch: see the dispatch metadata §9 (branch from it). v3's sync `Client` removes the daemon `checker` thread by construction (sync per-request refresh), so this arc DELETES the now-SHIPPED **P14.N7** daemon-checker resilience wrapper (`swing/integrations/schwab/checker_resilience.py`) AND must decide the fate of the SHIPPED F-1 web checker-liveness badge + sidecar (`swing/web/view_models/schwab_checker_badge.py`; the `~/swing-data/schwab-checker-liveness.<env>.json` sidecar -- no daemon writes it on v3, so the badge would be permanently the A-7 "UNKNOWN" state). NOT a new-Schwab-feature arc; a dependency migration on the L2-LOCKED Schwab surface.

**Cumulative discipline at dispatch:** 38+ CLAUDE.md gotchas BINDING (the entire **Schwab / schwabdev block** is the migration checklist) + the process disciplines in `docs/orchestrator-context.md`; **~700+ cumulative ZERO Co-Authored-By**; **NO swing-DB schema change expected** (schwabdev's tokens DB is its OWN SQLite, separate from `swing.db`; `EXPECTED_SCHEMA_VERSION` stays 23 -- confirm at brainstorm); **the L2 LOCK is re-anchored by this arc** (the first baseline move -- requires explicit operator sign-off; see L2 below).
...
- **L1** Scope = **the schwabdev 2.5.1 -> 3.0.5 migration ONLY** -- re-pin; fix every breaking change the findings doc enumerates (tokens-storage rewrite; `account_linked`->`linked_accounts`; `Client.__init__` signature; public->private token methods); REMOVE the now-dead P14.N7 daemon-checker resilience wrapper + decide the disposition of the SHIPPED F-1 web checker-liveness badge (L6/§2.6); update the signature-pin tests; the L2 re-anchor; the operator re-setup UX; (optionally) Fernet encryption. Do NOT add new Schwab endpoints/features, do NOT touch unrelated surfaces (A-3/SB5.5 already SHIPPED -- leave them, EXCEPT the F-1 badge disposition which is in scope).
...
- **L6** **P14.N7 + the F-1 web checker-liveness badge are now SHIPPED surfaces this arc must RECONCILE** (NOT "obviate a planned item" -- the original framing predated SB5.5/F-1 shipping). v3's sync `Client` has no daemon checker, so: (a) the P14.N7 daemon-checker resilience wrapper (`swing/integrations/schwab/checker_resilience.py`) becomes DEAD CODE -> remove it + its wiring + its tests; (b) nothing writes the `~/swing-data/schwab-checker-liveness.<env>.json` sidecar on v3, so the F-1 web badge (`swing/web/view_models/schwab_checker_badge.py`) would be permanently the A-7 "UNKNOWN" state -> the spec MUST design its disposition (DELETE the badge, or REWIRE it to a v3 "last-successful-refresh" signal stamped on each per-request `update_tokens()`). This is an IN-SCOPE design decision for THIS arc (OQ-5), not a future re-scope.
...
### §2.6 P14.N7 + F-1 web-badge reconciliation (the SHIPPED-surface removal/rewire) (OQ-5; L6)
- The **P14.N7 daemon-checker resilience wrapper** (`swing/integrations/schwab/checker_resilience.py`) guards the 2.5.1 daemon `checker` loop against `ConnectionError`/DNS death. v3's sync `Client` has no such loop -> the wrapper is DEAD. Remove it + its construction-site wiring (`auth.py`/`app.py`) + its tests in this arc.
- The **F-1 web checker-liveness badge** (`swing/web/view_models/schwab_checker_badge.py` + `evaluate_liveness_state`) renders from the `~/swing-data/schwab-checker-liveness.<env>.json` sidecar that the daemon checker writes. On v3 nothing writes it -> the badge is permanently the A-7 "Schwab? UNKNOWN" state. Design the disposition: DELETE the badge + its `app.py` wiring + the `cli_schwab.py` status checker-liveness reporting, vs REWIRE all three to a v3 last-successful-refresh timestamp. Reconcile ALL sidecar readers/writers in ONE task (atomic-consistency, gotcha #11 spirit).
- Whatever is chosen, the gate MUST witness the UNSEEDED default state (memory `feedback_seeded_gate_masks_default_state` -- a seeded sidecar is exactly the gap that produced A-7).
...
5. **OQ-5 P14.N7 + F-1 web-badge disposition** (now SHIPPED surfaces, L6/§2.6) -- DELETE the daemon-checker wrapper + the web checker-liveness badge entirely, vs REWIRE the badge to a v3 "last-successful-refresh" signal (stamped on each per-request `update_tokens()`). Operator-binding UX decision.
...
- A-3 web market-data wiring + SB5.5 (already SHIPPED in Phase 14; do NOT re-touch -- EXCEPT the F-1 web checker-liveness badge, whose disposition IS in scope per L6/§2.6)
...
- **Worktree:** YES -- branch `schwabdev-v3-upgrade-brainstorming`. Dir `.worktrees/schwabdev-v3-upgrade-brainstorming/`. **Branch from main HEAD = the commit that REFRESHES this brief for Phase-15 dispatch** (the orchestrator states the exact SHA in the inline prompt).
