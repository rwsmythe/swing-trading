...
## Orchestrator Amendment 0 -- Two-Codex-chain default refinement (PRE-CHAIN-#1; meta-discipline)

Per operator-identified meta-lesson during G2 dispatch + NEW CLAUDE.md gotcha #36 banked at the same housekeeping commit (orchestrator-side, applied retroactively pre-Chain-#1): applied research dispatches default to **TWO** Codex MCP chains, not one. G2 brief Sec 7 + Sec 8 originally prescribed a SINGLE chain after Slice 5 ship. This Amendment supersedes that prescription. The G2 implementer DID adopt the two-chain pattern + the chain split was validated (chain #1 caught 3 CRITICAL + 13 MAJOR + 12 MINOR BEFORE smoke artifact emission).

**Why:** historical applied research arcs (V2 executing-plans, D1, R2-A, R2-D) defaulted to a single Codex chain invoked AFTER the smoke artifact + findings draft were shipped. This means (a) the smoke artifact was generated by IMPLEMENTATION code that Codex had not yet reviewed; (b) Codex-caught implementation defects required re-emission of the smoke artifact OR propagation of wrong numbers into findings narratives; (c) single chain reviewed implementation + methodology + numerical accuracy + narrative claims at the same time, overloading the review surface. V2-mechanic arc (merge `64e0099`) shipped the two-Codex-chain split via Brief Amendment 1 and surfaced 2 CRITICAL + 12 MAJOR + 3 MINOR in implementation BEFORE smoke artifact emission; chain #2 then reviewed methodology with implementation already validated. The two-chain pattern is materially better than single-chain-at-end.
...
The 44th cumulative C.C lesson #6 validation slot SPANS BOTH chains (one slot; two reviews; mirrors V2-mechanic 43rd slot precedent). Forward-binding for all future applied research dispatches via CLAUDE.md gotcha #36.
...
*End of G2 W-bottom-ruleset backtest dispatch brief (including Orchestrator Amendment 0 + Brief Amendments 1-4 above). Mission: test the joint hypothesis (V2-expanded population + W-bottom-derived ruleset = positive portfolio expectancy at acceptable trade frequency). Introduces 9-metric scorecard (expectancy_R + win_rate + avg_win_R + avg_loss_R + profit_factor + trigger_conversion_rate + median_time_in_trade + open_at_tail_count + estimated_dollar_per_period) as the headline output. Replaces single-metric verdict-gating with narrative across the scorecard. Sibling-module strategy continues; A-F + substrates LOCKED via byte-stability; gotcha #33 banned-verdict-terms LOCK preserved. 35 cumulative CLAUDE.md gotchas BINDING for 44th cumulative C.C lesson #6 validation slot. NEW gotcha #35 FIRST CANONICAL APPLICATION post-banking; gotcha #36 (two-Codex-chain default; banked simultaneously) FIRST CANONICAL APPLICATION validated by chain #1 catching 3C+13M+12m pre-smoke. ~578+ cumulative ZERO Co-Authored-By trailer drift to preserve.*
