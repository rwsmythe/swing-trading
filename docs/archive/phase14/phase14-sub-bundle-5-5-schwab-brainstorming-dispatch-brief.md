...
### §1.0 Operator rulings (2026-05-31 -- BINDING; OQ-1 + OQ-2 RESOLVED before dispatch)
- **OQ-1 RESOLVED -- A-3 is a SANCTIONED L2 EXTENSION** (operator 2026-05-31): the web-side market-data wiring is a DELIBERATE expansion of Schwab usage to gain Schwab's likely-better data provenance over yfinance -- NOT an accidental L2 violation, and NOT held. Design A-3 as an intended extension. **The L2 source-grep test still MUST stay green** (zero NEW `schwabdev.Client.*` call SITES -- reuse the EXISTING ladder; the runtime-surface expansion is the approved part). Do NOT add new call sites; DO design the web ladder install.
- **OQ-2 RESOLVED -- FULL PARITY** (operator 2026-05-31): A-3 wires ALL the pipeline's market-data consumers on the web path -- daily bars (`chart_jit.get_or_fetch`) + the SMA path + the last-close quote (`PriceCache`) -- NOT daily-bars-only. Match the pipeline's `_build_caches_with_ladder` install surface.
- **Schwab rate limit (web-verified 2026-05-31): ~120 API calls/minute, APP-WIDE (shared across the Market Data + Accounts/Trading endpoints), HTTP 429 on breach (~2 req/sec).** This budget is SHARED by the web path + the nightly pipeline + account/reconciliation ops + the schwabdev checker token refreshes -- it is NOT a dedicated web budget. (Sources: Schwab Developer Portal + community references, May 2026; treat as the working number and re-confirm at writing-plans if Schwab revises.)
...
- **L9 (rate-limit-aware; per the §1.0 ruling -- BINDING for full-parity A-3)** the ~120-calls/min APP-WIDE shared budget (web + pipeline + account ops + checker) is the runtime constraint. The web ladder install MUST: (a) ride the EXISTING TTL cache + open-trade fetch scope so Schwab calls are bounded (one fetch per `(ticker, window_days)` per TTL; open-trade-scoped -- NOT per-render, NOT universe-wide); (b) treat HTTP 429 + any transient miss as a fall-through to yfinance (F6 transient-empty discipline) + a breaker trip, NEVER a hammering retry; (c) SHOULD populate the existing `rate_limit_remaining` audit field (`swing/integrations/schwab/audit_service.py:127`, currently always `None`) from Schwab's response headers so real headroom becomes observable. **The failure mode to design against is a render-storm that bypasses the cache.**
...
1. **OQ-1 RESOLVED (§1.0 -- sanctioned L2 extension; operator 2026-05-31):** the web-side ladder wiring is a DELIBERATE, operator-sanctioned L2 extension (Schwab's likely-better data provenance over yfinance) -- no longer held. Design A-3 as intended: zero NEW `schwabdev.Client.*` call SITES (reuse the existing ladder; the L2 source-grep test stays green), the new runtime Schwab-call SURFACE is the approved part. STILL surface the call-site-count evidence + the L9 rate-limit framing in the spec.
2. **OQ-2 RESOLVED (§1.0 -- full parity; operator 2026-05-31):** A-3 wires ALL pipeline market-data consumers on the web path -- daily bars (`chart_jit.get_or_fetch`) + the SMA path + the last-close quote (`PriceCache`) -- governed by the L9 rate-limit-aware constraints (cache + open-trade scope; 429 -> yfinance + breaker).
