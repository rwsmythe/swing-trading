...
**NEW (this brainstorm):**
- **FB-N1 (ROOT-CAUSED + FIXED post-brainstorm 2026-05-28):** the copowers Codex MCP tool's "timed out after 1s" was a **Windows spawn failure**, NOT a real review timeout: `.mcp.json` used bare `command: "codex"`, and the raw (no-shell) MCP spawn picks the extensionless POSIX npm shim (`codex`) over `codex.cmd` and dies; `MCP_CONNECTION_NONBLOCKING=true` masked the dead server as a ~1s fast-fail. **FIX:** `command: "cmd", args: ["/c","codex","mcp-server"]` in BOTH `.mcp.json` copies (cache `plugins/cache/local/copowers/2.0.0/` + marketplace `plugins/marketplaces/local/plugins/copowers/`); restart Claude Code; re-apply on copowers upgrade. The MCP path works after restart. The `codex exec` / `codex exec resume --last -o <file> -` CLI is the transport-independent BACKSTOP (what this brainstorm used). Full root cause + fix + re-apply discipline saved to memory `feedback_copowers_codex_mcp_windows_launcher`; same Windows MCP-stdio failure family as `feedback_claude_mem_hook_blocks_disabled`.
...
(Codex transport note for orchestrator: the copowers Codex MCP "1s timeout" was ROOT-CAUSED + FIXED post-brainstorm 2026-05-28 -- a Windows spawn failure of the bare `command:"codex"` launcher; fixed to `cmd /c codex mcp-server` in both `.mcp.json` copies; restart Claude Code + re-apply on copowers upgrade. MCP works after restart; `codex exec` CLI is the backstop. Memory: `feedback_copowers_codex_mcp_windows_launcher`. Banked as FB-N1.)
