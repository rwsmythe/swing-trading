...
- [ ] **Operator-paired cohort CSV substrate authoring**: produce `exports/research/cohorts/tightness_1.005_flips_67.csv` with the 67 (ticker, asof_date, eval_run_id) entries extracted from V2 sensitivity smoke drill-down (per dispatch brief §1 + plan §C). Small ~30-min orchestrator-side task OR operator inline. Integration test at `tests/research/test_pattern_cohort_evaluator_integration.py` un-skips once CSV is present.
- [ ] **Operator-paired first-cohort smoke run**: `python -m swing.cli diagnose pattern-cohort-detect --cohort-csv exports/research/cohorts/tightness_1.005_flips_67.csv --db "$env:USERPROFILE/swing-data/swing.db" --output-dir exports/research/`. Produces detector verdicts per (ticker, asof_date, pattern_class, window) for the loosened-A+ cohort. Operator can cross-tabulate with V2 OHLCV backtest trigger outcomes (5 triggered / 12 untriggered + no-data) to test the original "Phase 13 detector over-filters" hypothesis.
