...
- [ ] **Option D dispatch**: cohort-detection harness mirroring V2 OHLCV evaluator pattern. Architecturally correct answer to operator's research question (does pattern detection filter loosened-A+ candidates productively?). Substantial scope (~4-8h V2 candidate dispatch). Mirrors `research/harness/aplus_v2_ohlcv_evaluator/` structure: NEW `research/harness/pattern_cohort_evaluator/` reads operator-specified candidate cohort (e.g., the +75 loosened-A+ flips at `vcp.tightness_range_factor=1.005`) + invokes `swing/patterns/*` detectors directly against cohort independent of production pool predicate + writes verdicts to `exports/research/` (NOT `pattern_evaluations` which is production-only). L2 LOCK preserved (detector reads OHLCV via `swing/data/ohlcv_archive.py`). Branch suggestion: `applied-research-pattern-cohort-detector-evaluator`.
