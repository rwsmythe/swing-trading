...
- **Worktree husk `.worktrees/phase8-daily-management/` ACL-locked at integration merge.** Same Windows ACL pattern as Phase 6/7 cleanup leftovers. Empty directory; needs elevated `cleanup-locked-scratch-dirs.ps1` run to fully remove. Per Phase 6/7 cleanup-script extension precedent (commit `5430c1c`); not orchestrator-blocking.
