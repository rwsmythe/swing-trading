...
- [ ] **Operator-paired V2 OHLCV harness output review** (DK CRITERION DRIFT triage — investigate V1↔V2 divergence for DK:62; full 63-eval-run operator reproduction; binding-threshold identification across all 15 inert threshold variables)
- [ ] **Research → shadow promotion gate per OQ-8 ladder DEFERRED pending DK CRITERION DRIFT triage outcome** — BLOCKING Tier-1 parity FAIL may invalidate baseline parity green gate condition; promotion gate cannot fire until V1↔V2 drift root-caused + resolved
