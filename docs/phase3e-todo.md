...
- [ ] **Next operator-paired arc decision** (post-NEGATIVE-backtest-verdict): (a) **RECOMMENDED**: pivot to `vcp.tightness_days_required +16` backtest via NEW dispatch (next-most binding variable; different criterion family — counts consecutive tight days vs measures range tightness; may produce different breakout-conversion characteristics); (b) low-cost archive-refresh + re-run current cohort to close the data-sparsity caveat; (c) V2.5/V3 multi-quarter walk-forward window for statistical power; (d) pivot to market-conditions / other-gates investigation per V2.1 §III scope (cause 2 / cause 3 may dominate; numeric A+ classification may not be the limiting factor); (e) Phase 14 commissioning per Path B sequencing (cfg-policy pathway paused pending more positive substrate); (f) execute Stage 2 + 3 (detector confirmation rate query + AI second-opinion eval on winners-without-detection cell per operator's earlier banked methodology) — but Stage 3 is gated on having WINNERS, which this backtest does not produce; deferred until a backtest produces positive substrate.
