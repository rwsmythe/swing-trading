...
- [ ] **OPTION A FIX (operator decision: dispatch OR inline)**: 1-line research-branch filter in `_compute_baseline_parity`: `if cand_row.persisted_bucket == "excluded": continue` + bucket='error' extension + drill-down filter + discriminating tests. ~3-5 commits + 3-6 tests. ZERO production swing/ writes; ZERO V1 changes. Branch suggestion: `applied-research-v2-baseline-parity-excluded-filter`.
- [ ] **V2 smoke re-run (post-Option-A) — verify Tier-1 FULL PASS** (DK:62 + DHC/UCO/VSAT × 60-64 all resolved).
  - [ ] (D.3) **Amend method-record** at `research/method-records/aplus-criteria-calibration.md` v0.2.0 → v0.2.1 with NEW Limitation L4 (parallel-archive freshness desync documented per investigation §5.4)
  - [ ] (D.4) **Bank V2 candidate**: V2 reader "prefer-fresher of (Shape A, legacy)" by mtime tiebreaker (V2.5/V3 candidate; preserves OQ-18 Shape A primacy as default but adds freshness gate)
- [ ] **Full 63-eval-run operator reproduction** (Shape A refresh per D.1 is precondition; DK:62 will remain SOLE Tier-1 baseline-parity FAIL until then per investigation drift-scope finding)
- [ ] **Research → shadow promotion gate per OQ-8 ladder UNBLOCKED post D.1+D.2** (root cause identified + remediation in operator's hands; gate condition "baseline parity invariant green" can fire once Shape A refresh + Tier-1 PASS confirmed)
