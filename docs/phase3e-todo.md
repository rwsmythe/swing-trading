...
- [ ] **Operator-paired next-arc decision** post-V2-selection-mechanic SHIP. Options enumerated:
  ...
  - **W-shape entry/exit rules extension (production arc) + downstream Finviz filter re-eval (short branch)** -- operator-banked Turn H 2026-05-27 PM #3 as the anticipated post-round-1 sequence (CORRECTED 2026-05-27 PM #4 — supersedes the earlier framing of this bullet). Operator framing: "I anticipate an extension of the enter/exit rules to include W-shape rather than strictly minnervini/qulamaggie VCP. This may lead to re-evaluating finviz filter, although that will likely be a short branch of the applied research arc." Two-stage sequence: **(stage 1, substantive)** extend the production entry/exit advisory rules to admit W-bottom patterns alongside the existing Minervini/Qullamaggie VCP framing — likely touches `swing/trades/advisory.py` + `swing/evaluation/` + possibly `swing/recommendations/`; consumes G2 W-bottom backtest output as evidence; standard production-dispatch workflow (brainstorming → writing-plans → executing-plans). **(stage 2, short branch downstream)** re-evaluate the upstream Finviz screen at `~/swing-data/user-config.toml` `[integrations.finviz]` IF the W-shape extension surfaces a population-of-candidates concern (current screen is VCP-calibrated; may exclude W-favorable substrate). Operator framing explicitly characterizes stage 2 as "a short branch" — kept scope-narrow at brainstorming; NOT pre-elaborated as a methodological cross-screen investigation. NOT actionable until G2 result + post-round-1 next-arc decision. Captured at memory `project_post_round1_applied_research_direction.md`.
