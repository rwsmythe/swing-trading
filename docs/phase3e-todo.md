...
| **Severity** | TBD by operator (orchestrator triage: significant — blocks the entire closed-loop review surface; either calibration is too strict OR market genuinely hasn't produced A+ in this window — diagnostic needed to distinguish) |
| **Operator framing** | TBD by operator — open question whether the 0-A+ outcome is expected market output or a calibration miss |
...
| **Severity** | TBD by operator (orchestrator triage: moderate — Path C V1 simplification banked at T2.SB6c return report §4.1 row 4 explicitly notes "cache-miss write-through skips when no completed pipeline run exists"; the deeper issue is that Path C cannot synthesize `rule_criteria` without `geometric_score_json` which only fresh detector-produced exemplars have) |
| **Operator framing** | TBD by operator (concurred to T4.SB triage 2026-05-22 PM); orchestrator framing: the rendering enhancement landed but cannot light up against existing operator data until either (a) fresh A+ candidates produce pattern_evaluations + exemplars promoted to gold OR (b) the labeler subagent emit contract widens to include `rule_criteria` + `narrative` at silver-label time |
...
| **Severity** | TBD by operator (orchestrator triage: functional gap — closed-loop review surface partially-blind for sub-A+ ticker workflow) |
| **Operator framing** | "UCTT in today's hyp-rec table lists 'Chart unavailable — ...' in lieu of a chart. This is incorrect, ALL hyp-recs should be charted. Additionally, ALL watchlist items should be charted." |
...
| **Orchestrator recommendation** | **Hybrid: keep pipeline-time pre-generation for the always-rendered "fast-render set" (A+ + open + dashboard top-5 watchlist visible by default) + add JIT cache-miss fallback for everything else (sub-A+ hyp-recs, watchlist rows 11+, expand-on-demand surfaces).** The substrate landed at T2.SB6a (`chart_renders` cache + `get_cached_chart_svg` + `refresh_chart_render`) makes JIT clean: first expand of UCTT triggers render-on-cache-miss → writes to cache → all subsequent renders are instant cache hits. **Why hybrid over pure pre-gen**: pre-generating ALL 57 watchlist rows + ALL hyp-recs at pipeline time bloats the pipeline runtime + cache footprint for tickers the operator never views; current pre-gen wins for the dashboard "first-paint" set only. **Why hybrid over pure JIT**: pure JIT puts first-render latency on the operator-visible critical path for the dashboard's top items; pre-gen for the fast-render set keeps that instant. **Implementation pattern**: existing `/patterns/exemplars` cache-miss live render at `view_models/patterns/exemplars.py:258` is the precedent — extend to dashboard hyp-rec expand path + watchlist row 11+ expand path. **Architectural footnote**: this is consistent with the §1.5.1 amendment's intent + closes the V1 simplification "exemplar cache-miss write-through skips when no completed pipeline run exists" (T2.SB6c return report §4.1 row 4) by making cache-miss live-render the canonical fallback path uniformly. |
...
| **Severity** | TBD by operator (orchestrator triage: **HIGH** — closed-loop trade analysis is core operator workflow; a metric reporting 0 while reality is 8 trades is silent-lying, which is worse than crashing) |
