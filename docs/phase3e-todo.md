...
## 2026-05-23 #3 Applied Research Tranche 1: V2 OHLCV criterion-evaluator harness EXECUTING-PLANS SHIPPED at `a43a921` — **Applied Research Tranche 1 arc COMPLETE end-to-end**
...
- [x] **V2 smoke re-run (post-Option-A) — Tier-1 FULL PASS verified** at `exports/diagnostics/aplus-sensitivity-v2-20260524T181554Z.{csv,md}` (5 eval_runs / 121s runtime / 516 universe / 351 candidates; 15 false-positive entries → 0; CRITERION DRIFT DETECTED section OMITTED entirely; tier-2 unchanged at 10/0; sensitivity matrix counts identical; both-exist banner AESI/DK/PL unchanged orthogonal).
  - [ ] (D.3) **Amend method-record** at `research/method-records/aplus-criteria-calibration.md` v0.2.0 → v0.2.1 with NEW Limitation L4 (parallel-archive freshness desync per investigation §5.4 + gotcha #24) + NEW Limitation L5 (V2 harness sentinel-bucket filter discipline per investigation `d7cdd51` §5.3 + gotcha #25). Small inline doc edit OR mini dispatch — operator decision.
  - [ ] (D.4) **Bank V2 candidates** in method-record: (a) V2 reader "prefer-fresher of (Shape A, legacy)" by mtime tiebreaker (V2.5/V3; preserves OQ-18 Shape A primacy as default but adds freshness gate); (b) sentinel-bucket parity-comparison discipline as research-branch BINDING template for future V1-vs-V2 parity harnesses; (c) Fix #3 drill-down COUNT filter extension (operator-visible matrix behavior; default recommendation FILTER per investigation §9.3).
- [ ] **Full 63-eval-run operator reproduction** (UNBLOCKED post-Option-A Tier-1 FULL PASS; extrapolated 100-200 excluded-class entries now filtered correctly per investigation §3.3 scope estimate; canonical operator-paired binding-variable identification step).
- [ ] **Research → shadow promotion gate per OQ-8 ladder UNBLOCKED** (V2 shipped + baseline parity green NOW SATISFIED + ≥1 study writeup SATISFIED at T-V2.5 closer); fires post-binding-threshold identification OR all-15-non-binding declaration via full reproduction.
