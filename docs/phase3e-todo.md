...
- [ ] **OPTION D REMEDIATION (operator-paired)** per investigation §4:
  - [ ] (D.1) **Operator: refresh Shape A** for the 3 both-exist tickers (AESI/DK/PL) via natural pipeline run / `resolve_ohlcv_window` invocation (lowest-blast-radius)
  - [ ] (D.2) **Operator: re-run V2 smoke** + verify DK:62 Tier-1 parity restored (PASS)
