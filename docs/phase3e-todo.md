...
- [ ] **Operator-paired research → shadow promotion gate decision** (condition-1 interpretation): either (a) treat as "baseline parity green to the extent V2 evaluator's correctness is verifiable against V1, with 3 documented L4 + L5 + L6 architectural limitations" → gate SATISFIED on all 3 conditions; OR (b) maintain strict baseline-parity invariant → PENDING immutable-archive-snapshot V2.5/V3 candidate. Conditions 2 (study writeup) + 3 (5 binding variables) SATISFIED.
- [ ] **Study writeup amendment** at `research/studies/2026-05-23-v2-ohlcv-criterion-evaluator.md`: append 5 binding-variables findings + L4/L5/L6 caveat language (provided in investigation findings §5.4) + cross-references to L4/L5/L6 method-record entries + full-reproduction smoke artifact path. Small inline doc edit OR mini dispatch — operator decision.
