...
- **Deficiency 1 (T-A.1.6 template rendering)** — DEFERRED per closer brief §1.2. Significant scope; operator has external workaround. Operator decision needed post-merge: (A) standalone web-refinement task OR (B) fold into T2.SB6 closed-loop surface.
...
- **Deficiency 1 (T-A.1.6 template rendering: chart + per-criterion table + narrative)** — DEFERRED. Operator decision needed: (A) standalone web-refinement task OR (B) fold into T2.SB6 closed-loop surface.
