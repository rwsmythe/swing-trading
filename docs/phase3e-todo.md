...
3. **CLAUDE.md + orchestrator-context.md maintenance pass** — addresses cap-drift (CLAUDE.md ~48 entries vs ~30 cap; orchestrator-context active "Lessons captured" section growing past retention discipline cap). Archive-split for both files following 2026-05-05 archive-companion precedent. Banks SHIPPED entries to archive companions + preserves index/cross-references in active files. May fold in V2.1 §VII.F amendment batch processing (~17 cumulative amendments pending across Phase 9/10/12 + post-Phase-12 brainstorm + writing-plans) per Phase 13 scope-brainstorm §3 OQ-6 disposition. **Estimated 1 standalone docs-only dispatch** (no code; no tests); ZERO Codex rounds (no architectural surface).
...
- **3 third** because docs-only; can be scheduled opportunistically; cheap; prevents cap-drift compounding.
