...
**Infra note (NOT SB4 scope; for the operator / copowers T/S instance):** CLAUDE.md Conventions still says "adversarial Codex MCP review (2-5 rounds)" -- now stale on two axes: (1) the MCP transport is dead in the VS Code extension (WSL Codex CLI fallback is the path; copowers v2.0.2); (2) the 5-round cap is suspended for swing-trading (run to convergence; memory `feedback_codex_round_limit_suspended`). Left unedited to avoid colliding with the T/S instance's CLAUDE.md/memory edits; flag for a future Conventions-section pass.
