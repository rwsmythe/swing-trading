...
- [ ] **Next operator-paired arc decision** (post-shadow-promotion): cfg-policy proposal for `vcp.tightness_range_factor` (the most binding variable; +75 max_delta_aplus) for shadow → production pathway per V2.1 §VII.C; OR market-conditions / other-gates investigation per V2.1 §III scope; OR Phase 14 commissioning consideration per Path B sequencing.
