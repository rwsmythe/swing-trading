...
- [ ] **Option D Phase 2 writing-plans dispatch**: plan at `docs/superpowers/plans/2026-05-24-pattern-cohort-detector-evaluator-plan.md` consuming the spec as BINDING substrate. Mirror V2 OHLCV writing-plans precedent (~2602-line plan; 15 sections §A-§O). Branch `applied-research-pattern-cohort-detector-evaluator-writing-plans`; dispatch brief at `docs/applied-research-pattern-cohort-detector-evaluator-writing-plans-dispatch-brief.md`. Expected duration ~5-8h. Codex MCP review OPTIONAL via `copowers:writing-plans` wrapper; 38th cumulative C.C lesson #6 validation slot available.
- [ ] **Option D Phase 3 executing-plans dispatch**: ships ~7 research modules + ~50-80 fast tests + CLI subcommand (OQ-13 carve-out) + method-record + first-cohort smoke + study writeup + return report. Branch `applied-research-pattern-cohort-detector-evaluator-executing-plans`. Expected duration ~10-15h. First-cohort target = +67 watch→aplus flips at `vcp.tightness_range_factor=1.005` (15 unique tickers per OQ-9 LOCK).
