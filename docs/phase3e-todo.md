...
- [ ] **Operator decision on bug #3 (archive depth-insufficiency) handling** — 3-tier options enumerated at gotcha #29: (a) operational fix (operator re-runs exemplar pre-fetch with yfinance `period="max"` for 13 tickers; small ad-hoc; re-runs smoke; verifies template Pass 2 populated); (b) architectural fix (extend banked V2 NEW `_step_exemplar_ohlcv` pipeline step to use depth-aware period derived from MIN(end_date) - 200d; affects production); (c) defer + characterize as gotcha #29 limitation in study writeup (ship A+B+D1 with geometric-only verdicts as substantive substrate; NOT-recommended because substantive verdict is robust without template Pass 2).
- [ ] **Turn F A: smoke output tracking** — .gitignore amendment for `exports/research/pattern-cohort-detection-*/{summary.md, results.csv, manifest.json}` (mirrors `exports/diagnostics/` pattern); commit prior 3 smoke runs (164425Z + 175553Z + 190514Z) for bug-discovery-sequence traceability. Operator-confirmed YES at Turn F session start.
- [ ] **Turn F B: study writeup Results-section amendment** at `research/studies/2026-05-24-pattern-cohort-detection.md` — append per-class confirmation rate table + substantive finding (cohort is double-bottom-w not VCP) + implication for cfg-policy direction reframe + 3-bug-chain caveats (bug #1 cache miss + bug #2 type coercion + bug #3 archive depth-insufficiency) + current archive caveat (L6 from V2 OHLCV). Small inline edit (~30 min) OR mini dispatch per operator preference.
- [ ] **Turn F D1: double_bottom_w-specific backtest dispatch** — substantial brief authoring (~1-2h orchestrator-side); implementer ship ~4-8h. Tests whether dominant detector signal has positive expectancy under mean-reversion-appropriate entry rules (W right-shoulder break NOT close > consolidation pivot); 3 exit rulesets (same as prior V2 backtest); cohort = subset of +67 with double_bottom_w composite>=0.7 (~670 windows; needs pattern-level dedup).
