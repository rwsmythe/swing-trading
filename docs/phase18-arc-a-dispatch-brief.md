...
### Executing-return gate checklist (durable — verify on the SHIPPED diff; captured here so it survives mailbox acks)
Decision #3 (serializer belt-raise) RATIFIED KEEP by all three eyes (orchestrator + RD `580be9d1`-style reply + CHARC) — executing is GO. The checks each eye owns at the executing return:
- **RD watch-item (a):** the skip must not corrupt `sessions_since_detection` / horizon counting when it creates a one-session hole — confirm s-numbering reflects ACTUAL recorded sessions, not assumed contiguity.
- **RD watch-item (b):** in the shipped diff, re-confirm the writer predicate and `validate_bars` stay byte-consistent on the OHLC key set + the volume-exemption (no drift introduced during execution).
- **RD watch-item (c) / CHARC C3 check:** the `warnings_json` skip entry must be STRUCTURED — carry `ticker` + `observation_date` + `reason="non_finite_ohlc"` (not free-text) — so 18-D (the health monitor) can consume it. This is the C3 boundary's forward signal.
- **CHARC:** C1 (one shared predicate, all three sites) / C2a (no migration/mutation) / C3 / layer rule verified on the shipped diff.
- **Orchestrator:** QA-against-disk + suite green on the merged head + ruff.
