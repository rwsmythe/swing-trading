...
- **NEXT: Task 10** (the four cohort readers, A2-91..A2-97c), with **G-T7FE-A+C item C folded in**:
  ...
  - Dispatch a FRESH `implementer-opus-high` off `e16f8976`. Use the same dispatch shape as cells 5-9: targeted reading, STRICTLY red-first, one task per unit, STOP at the gate. Include the commit-message trailer hazard paragraph and the digest check.
...
- **OPEN with CHARC (must be ruled before Reviewer B; non-blocking for Task 10):** G-T9F note 1. `resolve_remote_ref` sits OUTSIDE the digest (replay-only, writes no blob, but it chooses the ref the merge-base checks against). Options: (a) leave it outside, which is my recommendation; (b) make it an eighth root, which forces version `.4`. Posted `20260923T042115Z`. His ruling arrives in YOUR inbox. Transcribe it byte-for-byte (the script pattern is in every ruling section's header) and post the landed path back.
