...
- **BASELINE_SHA:** `7f3cfa6` (HEAD of `main` after the Bundle 2 brief BASELINE_SHA pin commit; use this as the worktree branching point so the in-worktree brief reflects the final pinned form).
...
- **Brief commit:** `5037c5c` (initial draft) + `7f3cfa6` (BASELINE_SHA pin §2.1+§8) + 1 follow-up commit pinning BASELINE_SHA = `7f3cfa6` so the in-worktree brief reflects the final form.
