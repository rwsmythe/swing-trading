...
make. Then four acts, in this order, nothing between them:
  ...
  3. TaskStop your inbox Monitor, then VERIFY the loop is gone: from PowerShell,
     ...
     CommandLine matching orchestrator/inbox must return nothing (measured
     2026-09-14: TaskStop killed the loop and its children within 3 s, so a
     surviving row is a fault to report, never a routine kill).
