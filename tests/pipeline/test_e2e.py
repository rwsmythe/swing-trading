...
@pytest.fixture
def real_finviz_fixture():
    """Phase 1 fixture with realistic ticker cardinality (62 tickers).
    Lacks Phase 2 schema columns (Sector/Industry/52-Week etc.), so the
    test that uses it monkeypatches validate_csv. Purpose: prove the
    orchestrator handles nontrivial batch sizes end-to-end."""
...
def test_pipeline_e2e_realistic_batch(tmp_path: Path, real_finviz_fixture, monkeypatch):
    """Adversarial review Batch 5 Round 2 Major 1: cardinality coverage.

    Uses the 62-ticker Phase 1 fixture to exercise the orchestrator at a
    realistic batch size — ranking, filtering, report shaping, and briefing
    generation on nontrivial cardinality. The Phase 1 capture predates the
    Phase 2 schema columns, so validate_csv is monkeypatched; the strict
    schema gate is proven separately by test_pipeline_e2e_smoke against the
    Phase-2-compliant fixture and by the unit tests in
    tests/pipeline/test_finviz_schema.py. Together the two tests cover both
    halves of acceptance: schema correctness at small cardinality and
    orchestrator robustness at realistic cardinality."""
    project = tmp_path / "project"
    project.mkdir()
    home = tmp_path / "home"
    home.mkdir()
    ...
    inbox.mkdir(parents=True, exist_ok=True)
    import shutil
    shutil.copy2(real_finviz_fixture, inbox / real_finviz_fixture.name)
