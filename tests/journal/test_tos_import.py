...
def test_extract_cash_movements_neutralizes_reserved_oof_ref():
    """Codex R4-MAJOR-1: the `oof:` ref namespace is RESERVED for
    `journal oof-buy`. A TOS REF # that happens to be `oof:`-prefixed (a
    hand-edited CSV; a real broker REF# is never oof:-prefixed) must NOT become a
    self-sourced-skippable row. extract_cash_movements neutralizes it to a
    ref-less movement -> the row keeps its PRE-arc disposition (the ref-less
    heuristic), so the step-7 self-reconcile branch never skips a non-OOF TOS row
    (Lock 1 holds across the TOS write path too).

    Pre-fix: ref='oof:SPCX:2026-06-18' would flow to the cash_movement, and the
    matcher would skip the row as self-sourced. Post-fix: ref is None.
    """
    rows = [
        {
            "TYPE": "WD", "DATE": "6/18/26", "AMOUNT": "-123.00",
            "REF #": '="oof:SPCX:2026-06-18"', "DESCRIPTION": "forged",
        },
        # A normal numeric REF# is preserved (the guard is scoped to oof:).
        {
    ...
    assert forged.ref is None          # reserved oof: ref neutralized
