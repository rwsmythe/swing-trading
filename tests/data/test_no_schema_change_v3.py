...
def test_no_new_migration_file_added() -> None:
    ...
    # 11 -> 12 by table rebuild); Demand C adds 0036 (the additive
    ...
    # trigger); 22-A2 adds 0039 (the provenance_corrections rebuild: the third
    # admission tier latch_ladder_tier2 and the seventh citation column,
    # CHARC F12 = A); 22-B adds 0040 (the trades rebuild widening
    # entry_intent, entry_intent_attestations, the N4 trigger + twins and the
    # four latch_view_events belts), so the ceiling is now 40.
    #
    ...
    assert versions[-1] <= 40, f"a new migration file was added: {versions[-1]} (L3 violation)"
