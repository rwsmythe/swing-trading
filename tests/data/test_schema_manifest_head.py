...
def test_write_then_load_manifest_round_trips(head_conn, tmp_path):
    ...
    mod.write_manifest(out_path, rows, schema_version=EXPECTED_SCHEMA_VERSION)
...
def test_committed_fixture_header_states_its_schema_version():
    """Brief section 2 (Naming bullet): "The fixture's header comment states
    the schema version it was generated at, as data." Verified RED against
    the pre-fix fixture (which stated the version only in generator PROSE,
    e.g. "generated by scripts/schema_manifest.py --write", never as a
    separately-greppable data line) -- see the commit message for the exact
    pre-fix header text and the failure it produced.

    Sourced from `swing.data.db.EXPECTED_SCHEMA_VERSION`, not a literal `38`
    hardcoded in this test: HEAD's own version constant is the ground truth
    the fixture must agree with, the same relationship `read_manifest`/
    `ensure_schema` already have to it."""
    header_lines = [
        line for line in _FIXTURE.read_text(encoding="utf-8").splitlines()
        if line.startswith("#")
    ]
    assert f"# schema_version {EXPECTED_SCHEMA_VERSION}" in header_lines


def test_write_manifest_sources_schema_version_from_the_given_db_not_a_constant(tmp_path):
    """`write_manifest`'s `schema_version` argument is written into the
    header VERBATIM -- it must be read by the CALLER from the DB `--write`
    just migrated, never hardcoded to `EXPECTED_SCHEMA_VERSION` inside
    `write_manifest` itself. Proven by passing a DELIBERATELY DIFFERENT
    value and observing THAT value in the header, not the constant."""
    out_path = tmp_path / "roundtrip.tsv"
    mod.write_manifest(out_path, [], schema_version=999)
    header_lines = [
        line for line in out_path.read_text(encoding="utf-8").splitlines()
        if line.startswith("#")
    ]
    assert "# schema_version 999" in header_lines
    assert f"# schema_version {EXPECTED_SCHEMA_VERSION}" not in header_lines
