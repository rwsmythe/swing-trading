...
# ---------------------------------------------------------------------------
# 22-A: the order<->mandate link's freeze tier, and the correction surface's
# admission tier.
#
# THEY LIVE HERE, in the cycle-free constants module, because BOTH the SQL
# CHECK's Python mirror (``swing/data/models.py``) and the resolver
# (``swing/trades/latched_origin.py``) must consume the SAME frozenset -- and
# ``swing.data.models`` cannot import from ``swing.trades`` without inverting
# the layering.  Two spellings of one enum is exactly the mirror family this
# arc measured at TWELVE sites; the single source is how it stays at one.
#
# The SQL halves live in migration 0037 -- ``latch_order_mandate_links
# .freeze_tier`` and ``provenance_corrections.admission_tier`` -- and a drift
# test runs both spellings and asserts they hold the same values.  That
# comparator is the only mirror that defends the set: it does not depend on
# anyone choosing the right grep.
# ---------------------------------------------------------------------------
