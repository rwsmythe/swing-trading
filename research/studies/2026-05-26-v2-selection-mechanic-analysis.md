...
The two tightness-family variables (tightness_range_factor + tightness_days_required) and adr_min_pct have DEGRADED-to-SUPPRESSED survival; proximity_max_pct + orderliness_max_bar_ratio have COMPARABLE survival. As a descriptive observation across the N=5 V2 binding variables analyzed, the cohorts selected by tightness/volatility-gating variables show LOWER canonical survival than the cohorts selected by proximity/orderliness-gating variables. The causal mechanism (whether the tightness/volatility V2 criteria themselves disqualify historical W patterns through the canonical filter, or whether the disqualification is incidental to the substrate's regime characteristics) is a HYPOTHESIS for future investigation, not a claim established by this analytical investigation. The R2-A rejecting Ruleset E verdict on the tightness_days_required substrate may be at least partially explained by this survival suppression: the surviving W patterns on those tickers may have systematically different quality profiles than the surviving W patterns on the baseline cohort.
...
- **D2 W-bottom ruleset comparison** (S&P 500 EXPANDED N=71 cohort): the bias-free baseline against which V2 substrates are characterized. D2's per-ruleset outcome on the N=26 most-recent recency-filtered S&P 500 candidates per D2 Amendment 5 established the analytical reference for "what Ruleset E looks like on a substrate-size-sufficient bias-free cohort" (specific verdict label per the original D2 study writeup; this investigation references the prior arc finding abstractly to avoid emitting banned verdict-style substrings per cumulative gotcha #33).
