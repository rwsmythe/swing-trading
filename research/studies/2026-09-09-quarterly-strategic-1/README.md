...
This is not an edge claim and it is not a commission. It is a routing note: the detector lives in `swing/patterns/` (CHARC's lane), and any DBW-dependent research is blocked on a bounded diagnostic — does the v1.0.0 detector fire on ANY synthetic or historical W-bottom at all? **Answered by CHARC by execution the same night (2026-09-10T08:41Z, thread `dbw-detector-silence`; registered D53 at `e0c646ac`): a DEFECT of composition.** The detector fires on the suite's own synthetic W (score 1.10, all eight criteria) when handed the trough-1 window, but `runner.py:2165` hands every detector `windows[-1]`, the most-recent swing low, which for any completed W is trough 2, and `double_bottom_w.py:356` demands trough-1 alignment. Each half deliberate (2026-05-20 and 2026-05-21, different tasks); the DBW tests build windows by hand and never call the generator. **So the 1,302 DBW rows are UNCOLLECTED, not negative: predicate 1 for the W substrate is UNINSTRUMENTED since 2026-05-21, not N = 0**, and the June exemplar 0/2 is the same mechanism. CHARC's recommended fix is detector-local (accept a trough-2-aligned anchor in zigzag mode) under a NEW `detector_version`, historical rows untouched; the operator commissions, I QA as measurement-adjacent. The W substrate's maturity clock starts at the fix.

**D53 MERGED `e311be8f` (2026-09-14); RD return QA, half 1 — the June exemplars re-run under v1.1.0.** Method: the same harness as the June study (`research/harness/minervini_exemplar_recall/run.py`, which resolves the PRODUCTION detector registry via `swing.pipeline.runner._pattern_detect_registry` — the installed `swing` verified to read `double_bottom_w@v1.1.0` at run time), same curated CSV and Tiingo archive, `--only twosmw-fig10-40-lulu,twosmw-fig11-6-dks`; run twice, production-faithful window selection (`windows[-1]`) and `--h2-all-windows` as a control. Artifacts under `artifacts/dbw-rerun-faithful-20260914T190048Z/` and `artifacts/dbw-rerun-allwin-20260914T190155Z/`.

| exemplar | June (v1.0.0) fired classes, isolated, window-sweep | v1.1.0, same mode |
|---|---|---|
| LULU (TWoSMW Fig 10.40, entry 2010-11-05) | high_tight_flag; vcp | **double_bottom_w**; high_tight_flag; vcp |
| DKS (TWoSMW Fig 11.6, entry 2003-04) | cup_with_handle; flat_base; high_tight_flag; vcp | cup_with_handle; **double_bottom_w**; flat_base; high_tight_flag; vcp |

**DBW recall, stage-isolated, window-sweep: 0/2 → 2/2.** The all-windows control returns the identical result, i.e. the runner's own window is now sufficient — the fix did what D53 specified. Two caveats travel with the hit: (1) **production-faithful (Stage-2 8/8-TT-gated) recall is 0/2, unchanged from June** — the Stage-2 gate rejects both exemplar windows exactly as it did for every class in June; the detector's own verdict is what changed, and it is now visible only when the gate is isolated; (2) the stage-isolated detectors fired on 10/10 random same-ticker control windows on this run (0.789 across the full June set), so **"fires on the exemplar" is proof the detector CAN fire, not a recall or specificity claim** — the June conclusion that the Stage-2 gate is the source of specificity stands. Also corrected: the June "0/2" was 0/2 evaluated under H2 but only LULU was ever SCREENABLE (DKS: 55–115 bars, insufficient history); that is unchanged.

**Half 2 of the return QA** (the first post-merge nightly's non-zero DBW count against the 1,302-at-zero baseline; temporal clause — only runs on the fixed code count) waits for run 176. Interim guard on the exemplar corpus (do not confirm `double_bottom_w` rows in review) stands until D53.1 F1 merges.

**Half 2 — DONE 2026-09-15 (run 176, the first nightly on the merged code; re-derived on the live DB `mode=ro`).** Run 176: complete, data_asof 2026-09-14, `export_status=ok` (the D42 archiver repair's forward test also passed). Six DBW evaluations, all `double_bottom_w@v1.1.0`, **one non-zero: evaluation 7835, NESR, geometric = composite 0.6667**, evidence trough_1 2026-07-29 / center peak 08-14 / trough_2 08-20, persisted `window_start_date` 2026-08-20 (== trough_2: an INTERIM row, as the F2 derivation predicts). Temporal log: detection 6645, same values, anchors-JSON `window` start = anchor = 2026-08-20 (`zigzag_pivot:swing_49_down`) with the evidence's trough_1 beside it — the (1a) prediction verified on the first live row. **Against the baseline: 0 of 1,563 evaluations (0 of 1,302 signals) non-zero before run 176; 1 of 6 on the first fixed run.** Emission shape unchanged (30 events = 6 signals × 5 classes; all-time 6,655 = 1,331 × 5 — no double emission). Warning classes: run 176's set is a subset of 175's. Temporal clause satisfied: run 176 executed the fixed code; runs ≤175 did not and are not counted. **T9 for the W substrate: instrumented from run 176; N = 1 name (NESR); the maturity clock starts at data_asof 2026-09-14.** D53 closes on this read (CHARC's ruling). The interim span (window_start_date = trough_2 on non-zero DBW rows) opens at run 176 and closes at the run before D53.1's first run.
...
1. **RHI (trade 24, `standard`): the journal stop was reconciled DOWN to the broker's.** `reconciliation_corrections` 41 (2026-08-25) moved `current_stop` 34.77 → 34.18, i.e. the broker stop sits $0.59 (6.3%) below the framework's initial stop; the daily-management records carry 34.18 unchanged since. A reconciliation correction on a `standard` trade is the T6 shape, and I find no adjudication of it in the record. **Resolved same day (operator, 2026-09-09): deliberate — 34.77 was entered in error; 34.18 is the intended stop.** Not a T6 event. **Residual record defect:** `trades.initial_stop` still reads 34.77, and that column is the R denominator, so RHI's realized R will be computed on a $9.43 risk unit instead of the true $10.02 (magnitude overstated ~6%) until corrected. No supported correction surface exists for `initial_stop` (the CLI corrects `entry_date` and cohort provenance only; the four `current_stop` corrections on record all came through reconciliation). Named for the October read's caveat list; the surface question is the operator's/CHARC's.
...
2. **DBW: routed and answered — a defect (D53).** The fix is CHARC's recommendation and the operator's commission; my only condition (historical rows stay as emitted; a new `detector_version` forward) is already in his fix shape.
3. **RHI stop (item 4.1): resolved — deliberate.** What remains is the `initial_stop` record defect above (no correction surface).
