...
At constant 1× capital ($7,500), Russell 3000 produces 0.0089 % per ticker-day vs SPX+NDX 0.0019 %; at 5× capital, Russell 0.0098 % vs SPX+NDX 0.0039 %. The ratio is ~4.6× at 1× and ~2.5× at 5×. Both ratio comparisons are statistically distinguishable from sampling noise at 95 % confidence (CIs do not overlap; see "CI overlap notes" above).
...
3. **A+ rate differs between universes at constant capital, consistent with a material universe-composition effect.** Russell 3000 produces ~4.6× the per-ticker-day A+ rate of SPX+NDX at 1× capital (Wilson CIs do not overlap); ~2.5× at 5× (CIs also do not overlap). The diagnostic does **not** isolate "universe shape" from the confounders documented in §"Universe-shape effect" (snapshot membership, survivorship profile, sector/liquidity mix, yfinance coverage); the finding is that *changing universe from SPX+NDX to Russell-3000-via-IWV* moves A+ rate by these factors, not that "small-mid-cap names are intrinsically better at producing setups."
...
- This diagnostic does **NOT** state whether `risk_feasibility`'s 1× → 5× shift is "binding" or "non-binding" in any operator-preference-relative sense — it reports the structural counts (48,155 → 3,367 SPX+NDX; 86,969 → 4,315 Russell) and the headline A+-rate change (whose Wilson CIs overlap and which is therefore not statistically distinguishable from sampling noise at 95 %). The threshold for "matters" is operator-set.
...
- **Sample size is small for headline-rate comparisons.** Wilson 95 % CIs on the four runs are reported in the results table. The cross-run ratios most-often quoted in the report (capital 1× → 5× SPX+NDX 2.0×, Russell 1.10×; universe SPX → Russell at 1× ~4.6×, at 5× ~2.5×) inherit this uncertainty: the within-universe ratios overlap CIs (capital effect not statistically distinguishable from noise) while the cross-universe ratios do not (universe effect is). Any operator interpretation that treats these point estimates as precise is over-confident.
