...
- **Harness git SHA at D3 run:** `110f7cc00e7c6527c8698719a21fd57e8aa0b0b3` (the D4 commit, which also contains all D2/D3 code plus the bridge fix `d5b17d0`). The original D3 run on 2026-04-25T06:05:00 was performed against a working tree that already contained the bridge fix (the `_CountingPriceFetcher` wrapper that produced the `cache_hits=595` value), but `git rev-parse HEAD` at that moment still returned the D2 SHA `59a86c6` — the bridge fix was uncommitted in the working tree. That manifest was therefore provenance-ambiguous; D5 round-1 adversarial review flagged this as a Major issue. The committed manifest is the re-run on 2026-04-25T06:13:55 against HEAD `110f7cc`, which is byte-identical to the original on every bucket/criterion outcome (80/80, 1,440/1,440, 595/0 cache hits/misses) but cleanly attributes the outputs to a committed SHA.
...
- **`as_of_date` mechanism differs between harness reconstruction and production.** Production calls `PriceFetcher.get(..., as_of_date=None)` and lets `swing.prices._resolve_asof` map None to `last_completed_session(datetime.now())` at run time. The parity reconstruction explicitly pins `as_of_date=date.fromisoformat(eval_row.data_asof_date)`. For eval 15, both mechanisms resolved to `2026-04-24` (Friday close), so the cache keys and bytes coincided. The mechanisms could diverge for evaluations that ran during or before the NYSE close on the same day (production's `last_completed_session(now)` would resolve to the prior session, while the eval row's `data_asof_date` is set after OHLCV is fetched and reflects the actual max bar date). The parity reconstruction's pinning to `data_asof_date` is the more reproducible choice but is not bytewise mechanism-equivalent to production; the empirical coincidence on this run does not generalize.
